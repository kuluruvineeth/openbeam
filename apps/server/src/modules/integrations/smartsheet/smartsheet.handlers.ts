...
import { createSmartsheetClient } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { apiKeyAuthRoute } from "./smartsheet.routes";
  ...
  try {
    ...
    if (!healthy) {
      return c.json(
        {
          success: false,
          message: "Cannot access Smartsheet. Check your API access token.",
        },
        400
      );
    }
    ...
    logger.error({ error }, "Smartsheet auth error");
    const message =
