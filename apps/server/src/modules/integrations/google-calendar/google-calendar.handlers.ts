...
import { GoogleCalendarAuth } from "@openbeam/services";
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await googleCalendarAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "Google Calendar OAuth start error");
    const message =
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await googleCalendarAuth.complete({
      code,
      state,
    });
    ...
    logger.error({ error }, "Google Calendar OAuth callback error");
