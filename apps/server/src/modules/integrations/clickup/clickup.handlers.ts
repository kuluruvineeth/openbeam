...
import { ClickUpAuth } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { oauthCallbackRoute, startOAuthRoute } from "./clickup.routes";

const clickUpAuth = new ClickUpAuth();
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await clickUpAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "ClickUp OAuth start error");
    const message =
      error instanceof Error
        ? error.message
        : "Failed to start ClickUp authentication";
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await clickUpAuth.complete({
      code,
      state,
    });
    ...
    logger.error({ error }, "ClickUp OAuth callback error");
