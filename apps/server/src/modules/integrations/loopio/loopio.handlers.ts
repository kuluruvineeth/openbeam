...
import { createLoopioClient } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { apiKeyAuthRoute } from "./loopio.routes";
  ...
  try {
    ...
    if (!healthy) {
      return c.json(
        {
          success: false,
          message: "Cannot access Loopio. Check your API key.",
        },
        400
      );
    }
    ...
    logger.error({ error }, "Loopio auth error");
    const message =
