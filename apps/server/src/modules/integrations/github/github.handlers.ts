...
import { GitHubAuth } from "@openplane/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { oauthCallbackRoute, startOAuthRoute } from "./github.routes";
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await githubAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "GitHub OAuth start error");
    const message =
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await githubAuth.complete({
      code,
      state,
    });
