...
import { BoxAuth } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { oauthCallbackRoute, startOAuthRoute } from "./box.routes";
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await boxAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "Box OAuth start error");
    const message =
      error instanceof Error
        ? error.message
        : "Failed to start Box authentication";
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await boxAuth.complete({
      code,
      state,
    });
    ...
    logger.error({ error }, "Box OAuth callback error");
