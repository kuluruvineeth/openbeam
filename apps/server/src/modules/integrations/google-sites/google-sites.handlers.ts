...
import { GoogleSitesAuth } from "@openbeam/services";
...
import type {
  oauthCallbackRoute,
  startOAuthRoute,
} from "./google-sites.routes";

const googleSitesAuth = new GoogleSitesAuth();
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await googleSitesAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "Google Sites OAuth start error");
    const message =
      error instanceof Error
        ? error.message
        : "Failed to start Google Sites authentication";
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await googleSitesAuth.complete({
      code,
      state,
    });
    ...
    logger.error({ error }, "Google Sites OAuth callback error");
