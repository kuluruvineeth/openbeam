...
import { LookerStudioAuth } from "@openbeam/services";
...
import type {
  oauthCallbackRoute,
  startOAuthRoute,
} from "./looker-studio.routes";

const lookerStudioAuth = new LookerStudioAuth();
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await lookerStudioAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "Looker Studio OAuth start error");
    const message =
      error instanceof Error
        ? error.message
        : "Failed to start Looker Studio authentication";
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await lookerStudioAuth.complete({
      code,
      state,
    });
    ...
    logger.error({ error }, "Looker Studio OAuth callback error");
