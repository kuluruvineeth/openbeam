...
import { GitLabAuth } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { oauthCallbackRoute, startOAuthRoute } from "./gitlab.routes";
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await gitlabAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "GitLab OAuth start error");
    const message =
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await gitlabAuth.complete({
      code,
      state,
    });
