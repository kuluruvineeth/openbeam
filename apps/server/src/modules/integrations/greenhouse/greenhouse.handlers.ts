...
import { createGreenhouseClient } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { apiKeyAuthRoute } from "./greenhouse.routes";
...
> = async (c) => {
  ...
  try {
      ...
      return c.json(
        {
          success: false,
          message:
            "Cannot access Greenhouse. Check your API key and permissions.",
        },
        400
      );
    }
    ...
    logger.error({ error }, "Greenhouse auth error");
    const message =
