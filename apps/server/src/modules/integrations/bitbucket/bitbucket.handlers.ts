...
import { BitbucketAuth } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { oauthCallbackRoute, startOAuthRoute } from "./bitbucket.routes";
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await bitbucketAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "Bitbucket OAuth start error");
    const message =
      error instanceof Error
        ? error.message
        : "Failed to start Bitbucket authentication";
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await bitbucketAuth.complete({
      code,
      state,
    });
    ...
    logger.error({ error }, "Bitbucket OAuth callback error");
