...
import { ZoomAuth } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { oauthCallbackRoute, startOAuthRoute } from "./zoom.routes";
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await zoomAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "Zoom OAuth start error");
    const message =
      error instanceof Error
        ? error.message
        : "Failed to start Zoom authentication";
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await zoomAuth.complete({
      code,
      state,
    });
    ...
    logger.error({ error }, "Zoom OAuth callback error");
