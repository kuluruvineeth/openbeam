...
import { createFifteenFiveClient } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { apiKeyAuthRoute } from "./fifteen-five.routes";
  ...
  try {
    ...
    if (!healthy) {
      return c.json(
        {
          success: false,
          message: "Cannot access 15Five. Check your API key.",
        },
        400
      );
    }
    ...
    logger.error({ error }, "15Five auth error");
    const message =
