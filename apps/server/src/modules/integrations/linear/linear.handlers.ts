...
import type { oauthCallbackRoute, startOAuthRoute } from "./linear.routes";
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await linearAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "Linear OAuth start error");
    const message =
      error instanceof Error
        ? error.message
        : "Failed to start Linear authentication";
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await linearAuth.complete({
      code,
      state,
    });
    ...
    logger.error({ error }, "Linear OAuth callback error");
