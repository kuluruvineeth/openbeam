...
import { AirtableAuth } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { oauthCallbackRoute, startOAuthRoute } from "./airtable.routes";
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await airtableAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "Airtable OAuth start error");
    const message =
      error instanceof Error
        ? error.message
        : "Failed to start Airtable authentication";
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await airtableAuth.complete({
      code,
      state,
    });
    ...
    logger.error({ error }, "Airtable OAuth callback error");
