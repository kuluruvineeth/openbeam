...
import { DocuSignAuth } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { oauthCallbackRoute, startOAuthRoute } from "./docusign.routes";
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await docuSignAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "DocuSign OAuth start error");
    const message =
      error instanceof Error
        ? error.message
        : "Failed to start DocuSign authentication";
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await docuSignAuth.complete({
      code,
      state,
    });
    ...
    logger.error({ error }, "DocuSign OAuth callback error");
