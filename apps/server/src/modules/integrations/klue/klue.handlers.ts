...
import { createKlueClient } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { apiKeyAuthRoute } from "./klue.routes";
  ...
  try {
    ...
    if (!healthy) {
      return c.json(
        {
          success: false,
          message: "Cannot access Klue. Check your API key.",
        },
        400
      );
    }
    ...
    logger.error({ error }, "Klue auth error");
    const message =
