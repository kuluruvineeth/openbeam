...
import { TeamsAuth } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { oauthCallbackRoute, startOAuthRoute } from "./teams.routes";
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await teamsAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "Teams OAuth start error");
    const message =
      error instanceof Error
        ? error.message
        : "Failed to start Microsoft Teams authentication";
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await teamsAuth.complete({
      code,
      state,
    });
    ...
    logger.error({ error }, "Teams OAuth callback error");
