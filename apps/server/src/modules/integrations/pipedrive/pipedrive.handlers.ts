...
import { PipedriveAuth } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { oauthCallbackRoute, startOAuthRoute } from "./pipedrive.routes";
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await pipedriveAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "Pipedrive OAuth start error");
    const message =
      error instanceof Error
        ? error.message
        : "Failed to start Pipedrive authentication";
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await pipedriveAuth.complete({
      code,
      state,
    });
    ...
    logger.error({ error }, "Pipedrive OAuth callback error");
