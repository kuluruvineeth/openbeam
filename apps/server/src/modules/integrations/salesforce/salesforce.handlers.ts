...
import { SalesforceAuth } from "@openbeam/services";
import type { AuthEnv } from "@/middleware/auth";
import { getTeamId } from "@/middleware/auth";
import logger from "@/utils/logger";
import type { oauthCallbackRoute, startOAuthRoute } from "./salesforce.routes";
...
> = async (c) => {
  ...
  try {
    const oauthUrl = await salesforceAuth.start({
      user: { id: user.id },
    ...
    logger.error({ error }, "Salesforce OAuth start error");
    const message =
      error instanceof Error
        ? error.message
        : "Failed to start Salesforce authentication";
...
> = async (c) => {
  try {
    ...
    const { connector, redirectUrl } = await salesforceAuth.complete({
      code,
      state,
    });
    ...
    logger.error({ error }, "Salesforce OAuth callback error");
