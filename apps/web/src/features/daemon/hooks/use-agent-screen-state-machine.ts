...
export function deriveAgentScreenViewState({
  ...
  const displayAgent =
    candidateAgent ??
    (nextMemory.hasRenderedReady ? nextMemory.lastReadyAgent : null);
  if (!displayAgent) {
    return {
      state: {
        tag: "boot",
        reason:
          input.missingAgentState.kind === "resolving"
            ? "resolving"
            : "loading",
        source: "none",
      },
  ...
  let source: "authoritative" | "optimistic" | "stale" = "stale";
  if (input.agent) {
    source = "authoritative";
  } else if (input.shouldUseOptimisticStream) {
    source = "optimistic";
  }
...
const INITIAL_MEMORY: AgentScreenMachineMemory = {
...
export function useAgentScreenStateMachine({
  ...
  const memoryRef = useRef<AgentScreenMachineMemory>({ ...INITIAL_MEMORY });

  if (routeKeyRef.current !== routeKey) {
    routeKeyRef.current = routeKey;
    memoryRef.current = { ...INITIAL_MEMORY };
  }
