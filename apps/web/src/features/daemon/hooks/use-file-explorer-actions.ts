...
export function useFileExplorerActions(serverId: string) {
  const rawClient = useSessionStore(
  ...
  type ExplorerClient = {
    exploreFileSystem: (
      ...
      directory?: {
        path: string;
        entries: import("../stores/session-store").ExplorerEntry[];
      };
      file?: import("../stores/session-store").ExplorerFile;
    }>;
  ...
  const hasExplorerMethods =
    rawClient &&
    typeof rawClient === "object" &&
    "exploreFileSystem" in rawClient &&
    "requestDownloadToken" in rawClient;

  const client = hasExplorerMethods ? (rawClient as ExplorerClient) : null;
  ...
  const requestDirectoryListing = useCallback(
    async (
      ...
      const shouldRecordHistory =
        options?.recordHistory ?? !!shouldSetCurrentPath;
