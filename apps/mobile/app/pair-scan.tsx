...
export default function PairScanScreen() {
  ...
  const targetServerId =
    typeof params.targetServerId === "string" ? params.targetServerId : null;
  ...
  const returnToSource = useCallback(
    (serverId: string) => {
      ...
      if (source === "editHost" && targetServerId) {
        ...
        // biome-ignore lint/suspicious/noExplicitAny: Expo Router route type mismatch
        router.replace(settingsRoute as any);
        return;
      }
  ...
  const handleScan = useCallback(
    async (result: BarcodeScanningResult) => {
      ...
      if (lastScannedRef.current === offerUrl) {
        return;
      }
      ...
      try {
        ...
        const isNewHost = !daemons.some(
          (daemon) => daemon.serverId === offer.serverId
        );
  ...
  if (Platform.OS === "web") {
    return (
      <View style={styles.container}>
        <View
          style={[styles.header, { paddingTop: insets.top + theme.spacing[2] }]}
        >
        ...
        >
          <View style={styles.permissionCard}>
            ...
            <Text style={styles.permissionBody}>
              QR scanning isn't supported in the web build. Use "Paste link"
              instead.
            </Text>
            <Pressable onPress={closeToSource} style={styles.permissionButton}>
