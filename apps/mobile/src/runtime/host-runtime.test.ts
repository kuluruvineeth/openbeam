...
class FakeDaemonClient {
  ...
  private readonly listeners = new Set<(status: ConnectionState) => void>();
  ...
  // biome-ignore lint/suspicious/useAwait: async signature required by interface
  async fetchAgents(options?: FetchAgentsOptions): Promise<{
    entries: [];
    pageInfo: {
      hasMoreBefore: false;
      hasMoreAfter: false;
...
describe("HostRuntimeController", () => {
  it("keeps known hosts in connecting when client reports idle during connect", async () => {
    ...
    expect(controller.getSnapshot().agentDirectoryStatus).toBe(
      "initial_loading"
    );
  });
  ...
  it("logs typed reason codes for connection transitions", async () => {
    ...
    try {
      ...
      const lastTransition = transitionPayloads.at(-1) ?? null;
