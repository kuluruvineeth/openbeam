...
function toReasonCode(reason: string | null): string | null {
  ...
  if (
    ...
    normalized.includes("client_closed")
  ) {
...
function findConnectionById(
  ...
  return (
    host.connections.find((connection) => connection.id === connectionId) ??
    null
  );
}
...
export class HostRuntimeController {
  ...
  private readonly deps: HostRuntimeControllerDeps;
  ...
  markAgentDirectorySyncError(error: string): void {
    const hasEverLoadedAgentDirectory =
      this.snapshot.hasEverLoadedAgentDirectory;
    this.updateSnapshot({
  ...
  markAgentDirectorySyncIdle(): void {
    this.updateSnapshot({
      agentDirectoryStatus: this.snapshot.hasEverLoadedAgentDirectory
        ? "ready"
        : "idle",
      agentDirectoryError: null,
    });
  ...
  async runProbeCycleNow(): Promise<void> {
    ...
    if (
      !(activeConnectionId && findConnectionById(this.host, activeConnectionId))
    ) {
    ...
    if (activeProbe && activeProbe.status === "available") {
      const available = Array.from(probeByConnectionId.entries())
        ...
        .map(([connectionId, probe]) => ({
          connectionId,
          latencyMs: (probe as { status: "available"; latencyMs: number })
            .latencyMs,
        }))
    ...
    const reason =
      event.type === "connect_failed"
        ...
        : // biome-ignore lint/style/noNestedTernary: readable inline conditional
          ...
          : null;
    const _reasonCode =
      event.type === "connect_failed" ? "connect_failed" : toReasonCode(reason);
  }
  ...
  private async switchToConnection(input: {
    ...
    this.unsubscribeClientStatus = client.subscribeConnectionStatus((state) => {
      ...
      const patch: Partial<
        Omit<HostRuntimeSnapshot, "serverId" | "clientGeneration">
      > = {
        ...toSnapshotConnectionPatch(this.connectionMachineState),
      };
  ...
  private resolveClientSessionKey(): Promise<string> {
    if (!this.clientSessionKeyPromise) {
      this.clientSessionKeyPromise = this.deps
        .getClientSessionKey()
        .then((value) => {
...
export class HostRuntimeStore {
  private readonly controllers = new Map<string, HostRuntimeController>();
  private readonly serverListeners = new Map<string, Set<() => void>>();
  private readonly globalListeners = new Set<() => void>();
  private version = 0;
  private readonly deps: HostRuntimeControllerDeps;
  private readonly lastConnectionStatusByServer = new Map<
    string,
    HostRuntimeConnectionStatus
  >();
  private readonly agentDirectoryBootstrapInFlight = new Map<
    string,
    Promise<void>
  >();
  ...
  subscribe(serverId: string, listener: () => void): () => void {
    const existing =
      this.serverListeners.get(serverId) ?? new Set<() => void>();
  ...
  runProbeCycleNow(serverId?: string): Promise<void> {
    if (serverId) {
      return (
        this.controllers.get(serverId)?.runProbeCycleNow() ?? Promise.resolve()
      );
    }
    return Promise.all(
  ...
  refreshAllAgentDirectories(input?: { serverIds?: string[] }): void {
    ...
    for (const [serverId] of this.controllers) {
      ...
      // biome-ignore lint/complexity/noVoid: fire-and-forget async call
      void this.refreshAgentDirectory({ serverId }).catch(() => {
        /* intentional no-op */
      });
    }
  }
...
export function getHostRuntimeStore(): HostRuntimeStore {
  ...
  if (runtimeGlobal[HOST_RUNTIME_STORE_GLOBAL_KEY]) {
    singletonHostRuntimeStore =
      runtimeGlobal[HOST_RUNTIME_STORE_GLOBAL_KEY] ?? null;
    if (singletonHostRuntimeStore) {
      return singletonHostRuntimeStore;
    }
  }
