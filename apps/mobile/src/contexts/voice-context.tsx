...
export function VoiceProvider({ children }: VoiceProviderProps) {
  ...
  const { client: runtimeClient, isConnected: activeRuntimeConnected } =
    useHostRuntimeSession(activeServerId ?? "");
  ...
  const lastVoiceModeSyncedClientRef = useRef<SessionState["client"] | null>(
    null
  );
  ...
  const speechInterruptTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null
  );
  ...
  const realtimeAudio = useSpeechmaticsAudio({
    ...
    onAudioSegment: ({ audioData, isLast }) => {
      if (!voiceTransportReadyRef.current) {
        console.log(
          "[Voice] Skipping audio segment: voice transport not ready"
        );
        return;
      }
      console.log(
    ...
    onError: (error) => {
      ...
      if (session?.client) {
        ...
        console.error(
          "[Voice] Cannot handle error - setMessages not available from SessionState"
        );
      }
    },
    volumeThreshold: REALTIME_VOICE_VAD_CONFIG.volumeThreshold,
    confirmedDropGracePeriod:
  ...
  useEffect(() => {
    ...
    if (realtimeAudio.isSpeaking) {
      ...
      speechInterruptTimerRef.current = setTimeout(() => {
        ...
        speechInterruptStartMsRef.current = null;
        if (
          !(isVoiceModeRef.current && vadStateRef.current.isSpeaking) ||
          speechStartInterruptSentRef.current
        ) {
          return;
        }
        ...
        console.log(
          "[Voice] Speech persisted beyond grace; interrupting turn",
          {
  ...
  useEffect(() => {
    ...
    const connectionRecovered =
      connected && !wasVoiceSocketConnectedRef.current;
    const clientChanged = lastVoiceModeSyncedClientRef.current !== client;
    if (
      connected &&
      (connectionRecovered || clientChanged) &&
      !voiceResyncInFlightRef.current
    ) {
      ...
      // biome-ignore lint/complexity/noVoid: fire-and-forget async call
      void client
        .setVoiceMode(true, activeAgentId)
        .then(
          () => {
  ...
  const startVoice = useCallback(
    async (serverId: string, agentId: string) => {
      ...
      try {
        ...
        await realtimeAudio.stop().catch(() => {
          /* intentional no-op */
        });
