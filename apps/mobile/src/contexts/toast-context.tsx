...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const show = useCallback((content: ReactNode, options?: ToastShowOptions) => {
    ...
    if (Platform.OS === "android" && nativeAndroid && nativeMessage) {
      const duration =
        durationMs <= 2500 ? ToastAndroid.SHORT : ToastAndroid.LONG;
...
function ToastViewport({
  ...
  const icon =
    ...
    (toast.variant === "success" ? (
      ...
      <AlertTriangle color={theme.colors.destructive} size={18} />
    ) : null);
    ...
    <View pointerEvents="box-none" style={styles.container}>
      ...
      >
        ...
        {typeof toast.content === "string" ? (
          ...
          <View style={styles.contentSlot} testID="app-toast-message">
            {toast.content}
          </View>
        )}
      </Animated.View>
