...
export type UpdateHostInput = Partial<
  Omit<HostProfile, "serverId" | "createdAt">
>;
...
const DaemonRegistryContext = createContext<DaemonRegistryContextValue | null>(
  null
);
...
function isDefaultLocalhostConnection(connection: HostConnection): boolean {
  return (
    ...
    connection.endpoint === DEFAULT_LOCALHOST_ENDPOINT
  );
}

export function hostHasDirectEndpoint(
  host: HostProfile,
  endpoint: string
): boolean {
  ...
  return host.connections.some(
    (connection) =>
      connection.type === "direct" && connection.endpoint === normalized
  );
}

export function registryHasDirectEndpoint(
  hosts: HostProfile[],
  endpoint: string
): boolean {
...
export function useDaemonRegistry(): DaemonRegistryContextValue {
  ...
  if (!ctx) {
    throw new Error(
      "useDaemonRegistry must be used within DaemonRegistryProvider"
    );
  }
  return ctx;
}
...
export function DaemonRegistryProvider({ children }: { children: ReactNode }) {
  ...
  const persist = useCallback(
    async (profiles: HostProfile[]) => {
      queryClient.setQueryData<HostProfile[]>(
  ...
  const readDaemons = useCallback(
    () =>
      queryClient.getQueryData<HostProfile[]>(DAEMON_REGISTRY_QUERY_KEY) ??
      daemons,
    [queryClient, daemons]
  );
  ...
  const removeHost = useCallback(
    async (serverId: string) => {
      ...
      const removedHost =
        existing.find((daemon) => daemon.serverId === serverId) ?? null;
      const remaining = existing.filter(
        (daemon) => daemon.serverId !== serverId
      );
      await persist(remaining);
      if (
        removedHost &&
        hostHasDirectEndpoint(removedHost, DEFAULT_LOCALHOST_ENDPOINT)
      ) {
  ...
  const removeConnection = useCallback(
    async (serverId: string, connectionId: string) => {
      ...
      const next = existing
        .map((daemon) => {
          if (daemon.serverId !== serverId) {
            return daemon;
          }
          const remaining = daemon.connections.filter(
      ...
      if (
        removedConnection &&
        isDefaultLocalhostConnection(removedConnection)
      ) {
        await markDefaultLocalhostBootstrapHandled();
      }
    },
  ...
  const upsertHostConnection = useCallback(
    async (
      ...
      const connectionIdx = prev.connections.findIndex(
        (c) => c.id === input.connection.id
      );
      const nextConnections =
        connectionIdx === -1
          ? [...prev.connections, input.connection]
          : prev.connections.map((c, i) =>
              i === connectionIdx ? input.connection : c
            );
      ...
      const nextProfile: HostProfile = {
        ...
        preferredConnectionId:
          prev.preferredConnectionId ?? input.connection.id,
        updatedAt: now,
      };
  ...
  useEffect(() => {
    ...
    if (localhostBootstrapAttemptedRef.current) {
      return;
    }
    ...
    isPending,
    markDefaultLocalhostBootstrapHandled,
    readDaemons,
  ...
  try {
    ...
    if (stored) {
      ...
      if (Array.isArray(parsed)) {
        const v2 = parsed.filter((entry) =>
          isHostProfileV2(entry)
        ) as HostProfile[];
        ...
        const migrated: HostProfile[] = parsed
          .map((entry): HostProfile | null => {
            ...
            const daemonPublicKeyB64 =
              typeof obj.daemonPublicKeyB64 === "string"
                ? obj.daemonPublicKeyB64.trim()
                : "";
            ...
            const preferredConnectionId: string | null =
