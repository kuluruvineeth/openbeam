...
export function useAudioPlayer(options?: AudioPlayerOptions) {
  ...
  async function play(audioData: Blob): Promise<number> {
    return new Promise((resolve, reject) => {
      ...
      const shouldSuppress =
        options?.isDetecting?.() || options?.isSpeaking?.();

      if (shouldSuppress) {
        console.log(
          "[AudioPlayer] Suppressing playback - voice detection/speaking active"
        );
  ...
  function startCheckingForClearFlags(): void {
    ...
    checkIntervalRef.current = setInterval(() => {
      ...
      if (!isStillBlocked && suppressedQueueRef.current.length > 0) {
        console.log(
          "[AudioPlayer] Flags cleared - moving suppressed queue to main queue"
        );
        ...
        // biome-ignore lint/style/useCollapsedIf: necessary for this context
        if (checkIntervalRef.current !== null) {
          clearInterval(checkIntervalRef.current);
          checkIntervalRef.current = null;
        }
  ...
  async function processQueue(): Promise<void> {
    ...
    while (queueRef.current.length > 0) {
      ...
      if (shouldSuppress) {
        console.log(
          "[AudioPlayer] Flags became active during processing - moving remaining queue to suppressed"
        );
  ...
  async function _processNextInQueue(): Promise<void> {
  ...
  async function playAudio(audioData: Blob): Promise<number> {
    // biome-ignore lint/suspicious/noAsyncPromiseExecutor: necessary for this context
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const inputRate = parsePcmSampleRate(audioData.type || "") ?? 24_000;
        const pcm16k = resamplePcm16(pcm, inputRate, 16_000);

        // Calculate total duration
        const samples = pcm16k.length / 2; // 16-bit = 2 bytes per sample
        const durationSec = samples / 16_000; // 16kHz sample rate
