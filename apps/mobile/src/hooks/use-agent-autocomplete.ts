...
export function useAgentAutocomplete(
  ...
  const canLoadCommands =
    Boolean(serverId) && (isRealAgent || !!queryDraftConfig);
  ...
  const options = useMemo<AgentAutocompleteOption[]>(() => {
    ...
    if (mode === "file" && activeFileMention) {
      const orderedEntries = orderAutocompleteOptions(
  ...
  const loadingText =
    mode === "file" ? "Searching workspace..." : "Loading commands...";
  const emptyText =
    mode === "file" ? "No files or directories found" : "No commands found";
