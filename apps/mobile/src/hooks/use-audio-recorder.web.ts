...
export function useAudioRecorder(config?: AudioCaptureConfig) {
  ...
  const startMetering = useCallback((stream: MediaStream) => {
    ...
    if (!AudioContextCtor) {
      console.warn(
        "[AudioRecorder][Web] AudioContext unavailable for metering"
      );
      return;
    }
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      throw new Error(
        `Failed to access microphone: ${error?.message ?? error}`
      );
    }
    ...
    try {
      ...
      cleanupStream();
      throw new Error(
        `Failed to initialize recorder: ${error?.message ?? error}`
      );
    }
  ...
  const stop = useCallback(async (): Promise<Blob> => {
    ...
    if (recorder.state === "inactive") {
      ...
      const mimeType =
        recorder.mimeType || detectSupportedMimeType() || "audio/webm";
