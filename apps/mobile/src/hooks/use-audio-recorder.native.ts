...
import { Directory, File, type FileInfo, Paths } from "expo-file-system";
...
async function getActualRecordingUri(createdAt: Date): Promise<string | null> {
  try {
    console.log(
      "[AudioRecorder] Searching for recording file created at:",
...
export function useAudioRecorder(config?: AudioCaptureConfig) {
  ...
  const recordingOptions: RecordingOptions = useMemo(
    () => ({
      ...
      web: {
        ...
        bitsPerSecond: config?.bitRate || 128_000,
      },
    }),
    [
      config?.sampleRate,
  ...
  useEffect(() => {
    ...
    if (!(currentConfig?.onAudioLevel && recorderState.isRecording)) {
      return;
    }
  ...
  const start = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      console.log(
        "[AudioRecorder] Recording started at:",
    ...
    recordingOptions.numberOfChannels,
    recordingOptions.bitRate,
  ]);
  ...
  const stop = useCallback(async (): Promise<Blob> => {
    ...
    if (startStopMutexRef.current) {
      await startStopMutexRef.current.catch(() => {
        /* intentional no-op */
      });
    }
    ...
    try {
      const stopPromise = (async () => {
        ...
        if (recordingStartTime && (!uri || uri === "")) {
          console.log(
            "[AudioRecorder] Using workaround to find actual recording file..."
          );
