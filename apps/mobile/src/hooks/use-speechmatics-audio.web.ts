...
const floatToInt16 = (sample: number): number => {
  ...
  return clamped < 0
    ? Math.round(clamped * 0x80_00)
    : Math.round(clamped * 0x7f_ff);
};
...
export function useSpeechmaticsAudio(
    ...
    segmenterRef.current = new SpeechSegmenter(
      ...
      },
      {
        onAudioSegment: (segment) =>
          callbacksRef.current.onAudioSegment?.(segment),
  ...
  useEffect(() => {
    ...
    const startTime =
      segmenterRef.current?.getSpeechDetectionStartMs() ?? Date.now();
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      let contextState: AudioContextState = context.state;
      if (contextState !== "running") {
        console.warn(
          "[Voice][Web] AudioContext not running after resume, state:",
          contextState
        );
        try {
          await context.resume();
          ...
          console.error(
            "[Voice][Web] Second resume attempt failed:",
            resumeError
          );
        }
        ...
        if (contextState !== "running") {
          console.error(
            "[Voice][Web] AudioContext stuck in state:",
            contextState,
            "— onaudioprocess will NOT fire. This is likely a WKWebView user-gesture restriction."
          );
        }
      }
      console.log(
        "[Voice][Web] AudioContext state:",
        context.state,
        "sampleRate:",
        context.sampleRate
      );
      ...
      let firstProcessEventLogged = false;
      processor.onaudioprocess = (event) => {
        ...
        if (!firstProcessEventLogged) {
          firstProcessEventLogged = true;
          let maxSample = 0;
          ...
          console.log("[Voice][Web] First onaudioprocess fired", {
            bufferLength: input.length,
            maxSample: Number(maxSample.toFixed(6)),
            contextState: context.state,
        ...
        const pcm16 = resampleToPcm16(input, context.sampleRate, 16_000);
  ...
  }, [stopInternal, config.confirmedDropGracePeriod]);
