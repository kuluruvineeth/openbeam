...
describe("useAgentFormState", () => {
  describe("__private__.combineInitialValues", () => {
    ...
    it("keeps other initial values without forcing serverId", () => {
      const combined = __private__.combineInitialValues(
        { workingDir: "/repo" },
        null
      );
    ...
    it("respects an explicit serverId override (including null) over initialServerId", () => {
      expect(
        __private__.combineInitialValues({ serverId: null }, "daemon-1")
      ).toEqual({
        serverId: null,
      });

      expect(
        __private__.combineInitialValues({ serverId: "daemon-2" }, "daemon-1")
      ).toEqual({
        serverId: "daemon-2",
      });
    });
  ...
  describe("__private__.resolveFormState", () => {
    ...
    it("resolves provider only from allowed provider map", () => {
      const allowedProviderMap = new Map<
        AgentProvider,
        AgentProviderDefinition
      >(
        AGENT_PROVIDER_DEFINITIONS.filter(
