  ...
  useEffect(() => {
    ...
    function runCleanup(unlisten?: () => void | Promise<void>) {
      ...
      try {
        ...
        void Promise.resolve(cleanupFn()).catch((error) => {
          console.warn(
            "[useFileDropZone] Failed to remove Tauri drag-drop listener:",
            error
          );
        });
      } catch (error) {
    ...
    async function setupTauriDragDrop(): Promise<boolean> {
      ...
      try {
        ...
        console.warn(
          "[useFileDropZone] Failed to listen for Tauri drag-drop:",
          error
        );
        return false;
      }
    ...
    function setupDomDragDrop() {
      ...
      async function handleDrop(e: DragEvent) {
        ...
        try {
          ...
          console.error(
            "[useFileDropZone] Failed to process dropped files:",
            error
          );
        }
      }
