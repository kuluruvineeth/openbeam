...
export function useAudioPlayer(options?: AudioPlayerOptions) {
  ...
  const shouldSuppressPlayback = (): boolean =>
    Boolean(options?.isDetecting?.()) || Boolean(options?.isSpeaking?.());
  ...
  const startCheckingForClearFlags = (): void => {
    ...
    checkIntervalRef.current = setInterval(() => {
      ...
      if (!isStillBlocked && suppressedQueueRef.current.length > 0) {
        ...
        !isStillBlocked &&
        suppressedQueueRef.current.length === 0 &&
        checkIntervalRef.current !== null
      ) {
        clearInterval(checkIntervalRef.current);
        checkIntervalRef.current = null;
  ...
  const playAudio = async (audioData: Blob): Promise<number> => {
    ...
    if (type.startsWith("audio/pcm")) {
      const sampleRate = parsePcmSampleRate(type) ?? 24_000;
      audioBuffer = pcm16LeToAudioBuffer(
        context,
  ...
  return useMemo(
    ...
    [isPlayingState, clearQueue, ensureContext, play, stop]
  );
}
