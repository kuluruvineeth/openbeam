...
type UseAgentFormStateResult = {
  ...
  availableThinkingOptions: NonNullable<
    AgentModelDefinition["thinkingOptions"]
  >;
...
const allProviderDefinitionMap = new Map<
  AgentProvider,
...
function resolveFormState(
  ...
  allowedProviderMap: Map<
    AgentProvider,
    AgentProviderDefinition
  > = allProviderDefinitionMap
): FormState {
  ...
  if (!userModified.provider) {
    if (
      ...
      allowedProviderMap.has(initialValues.provider)
    ) {
  ...
  if (availableModels) {
    ...
    if (thinkingOptions.length === 0) {
      ...
      if (
        !(result.thinkingOptionId && thinkingIds.has(result.thinkingOptionId))
      ) {
  ...
  if (!userModified.serverId) {
    if (initialValues?.serverId !== undefined) {
      ...
      validServerIds.has(preferences.serverId)
    ) {
...
function combineInitialValues(
  ...
  const serverIdFromOptions =
    initialServerId === null ? undefined : initialServerId;

  // If nobody provided initial values or an explicit serverId, let preferences drive defaults.
  if (
    !(initialValues || hasExplicitServerId) &&
    serverIdFromOptions === undefined
  ) {
    return;
  }
...
export function useAgentFormState(
  ...
  const [userModified, setUserModified] = useState<UserModifiedFields>(
  ...
  const { client, isConnected } = useHostRuntimeSession(
    formState.serverId ?? ""
  );
  ...
  useEffect(() => {
    ...
    if (formState.serverId === initialServerId) {
      return;
    }
    setFormState((prev) => ({ ...prev, serverId: initialServerId }));
  }, [initialServerId, formState.serverId]);
    ...
    () =>
      new Map<AgentProvider, AgentProviderDefinition>(
        providerDefinitions.map((definition) => [
          definition.id as AgentProvider,
          definition,
        ])
      ),
    [providerDefinitions]
  ...
  const [debouncedCwd, setDebouncedCwd] = useState<string | undefined>(
    undefined
  );
  useEffect(() => {
  ...
  const modelQueryEnabled =
    queryEnabled && providerDefinitionMap.has(formState.provider);
  ...
  const providerModelsQuery = useQuery({
    ...
    staleTime: 5 * 60 * 1000,
    // Avoid long "always loading" states when provider model discovery is slow/unavailable.
    retry: false,
  ...
  useEffect(() => {
    ...
    if (combinedInitialValues?.serverId !== undefined) {
      return;
    }
    if (formStateRef.current.serverId) {
      return;
    }

    const candidate =
      onlineServerIds.find((id) => validServerIds.has(id)) ?? null;
    if (!candidate) {
      return;
    }

    setFormState((prev) =>
      prev.serverId ? prev : { ...prev, serverId: candidate }
    );
  }, [
  ...
  useEffect(() => {
    ...
    if (preferences?.serverId === serverId) {
      return;
    }
  ...
  const setModelFromUser = useCallback(
    (modelId: string) => {
      ...
      // biome-ignore lint/complexity/noVoid: fire-and-forget async call
      void updateProviderPreferences(formState.provider, {
  ...
  const hasFetchedProviderModels = providerModelsQuery.data !== undefined;
  const isModelLoading =
    providerModelsQuery.isPending ||
    (providerModelsQuery.fetchStatus === "fetching" &&
      !hasFetchedProviderModels &&
      !providerModelsQuery.error);
  const modelError =
    providerModelsQuery.error instanceof Error
