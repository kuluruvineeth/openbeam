import type { CheckoutStatusPayload } from "../hooks/use-checkout-status-query";
import {
  buildHostAgentDraftRoute,
  parseHostAgentRouteFromPathname,
} from "./host-routes";

export function parseAgentKey(
  key: string | null | undefined
): { serverId: string; agentId: string } | null {
  if (!key) {
    return null;
  }
  const sep = key.lastIndexOf(":");
  if (sep <= 0 || sep >= key.length - 1) {
    return null;
  }
  const serverId = key.slice(0, sep).trim();
  const agentId = key.slice(sep + 1).trim();
  if (!(serverId && agentId)) {
    return null;
  }
  return { serverId, agentId };
}

export function resolveSelectedAgentForNewAgent(input: {
  pathname: string;
  selectedAgentId?: string;
}): { serverId: string; agentId: string } | null {
  return (
    parseHostAgentRouteFromPathname(input.pathname) ??
    parseAgentKey(input.selectedAgentId)
  );
}

function inferMainRepoRootFromOpenPlaneWorktreePath(
  cwd: string
): string | null {
  const normalizedPath = cwd.replace(/\\/g, "/");
  const marker = "/.openplane/worktrees";
  const markerIndex = normalizedPath.indexOf(marker);
  if (markerIndex <= 0) {
    return null;
  }
  const markerEnd = markerIndex + marker.length;
  const nextChar = normalizedPath[markerEnd];
  if (nextChar && nextChar !== "/") {
    return null;
  }
  // biome-ignore lint/performance/useTopLevelRegex: scoped regex acceptable here
  const inferred = cwd.slice(0, markerIndex).replace(/[\\/]+$/, "");
  return inferred.trim() ? inferred : null;
}

export function resolveNewAgentWorkingDir(
  cwd: string,
  checkout: CheckoutStatusPayload | null
): string {
  const explicitMainRepoRoot = checkout?.isOpenPlaneOwnedWorktree
    ? checkout.mainRepoRoot?.trim() || null
    : null;
  if (explicitMainRepoRoot) {
    return explicitMainRepoRoot;
  }

  return inferMainRepoRootFromOpenPlaneWorktreePath(cwd) ?? cwd;
}
export function buildNewAgentRoute(
  serverId: string,
  options?:
    | {
        workingDir?: string | null;
        newNoteAt?: string | null;
        autoDictateAt?: string | null;
      }
    | string
    | null
): string;
export function buildNewAgentRoute(
  serverId: string,
  workingDirOrOptions?:
    | string
    | null
    | {
        workingDir?: string | null;
        newNoteAt?: string | null;
        autoDictateAt?: string | null;
      }
): string {
  const baseRoute = buildHostAgentDraftRoute(serverId);
  if (baseRoute === "/") {
    return baseRoute;
  }

  const trimmedWorkingDir =
    typeof workingDirOrOptions === "string"
      ? workingDirOrOptions.trim()
      : (workingDirOrOptions?.workingDir?.trim() ?? "");
  const trimmedNewNoteAt =
    typeof workingDirOrOptions === "object" && workingDirOrOptions !== null
      ? (workingDirOrOptions.newNoteAt?.trim() ?? "")
      : "";
  const trimmedAutoDictateAt =
    typeof workingDirOrOptions === "object" && workingDirOrOptions !== null
      ? (workingDirOrOptions.autoDictateAt?.trim() ?? "")
      : "";

  const searchParams = new URLSearchParams();
  if (trimmedWorkingDir) {
    searchParams.set("workingDir", trimmedWorkingDir);
  }
  if (trimmedNewNoteAt) {
    searchParams.set("newNoteAt", trimmedNewNoteAt);
  }
  if (trimmedAutoDictateAt) {
    searchParams.set("autoDictateAt", trimmedAutoDictateAt);
  }

  const query = searchParams.toString();
  if (!query) {
    return baseRoute;
  }
  return `${baseRoute}?${query}`;
}
