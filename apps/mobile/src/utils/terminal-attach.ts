...
export function getTerminalAttachRetryDelayMs(input: {
  attempt: number;
}): number {
  const clampedAttempt = Math.max(0, input.attempt);
  const exponentialDelay = 250 * 2 ** clampedAttempt;
  return Math.min(2000, exponentialDelay);
}

export function isTerminalAttachRetryableError(input: {
  message: string;
}): boolean {
...
export async function waitForDuration(input: {
  durationMs: number;
}): Promise<void> {
