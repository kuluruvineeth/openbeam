...
function getWebNotificationConstructor(): {
  ...
  new (
    title: string,
    options?: { body?: string; data?: Record<string, unknown> }
  ): unknown;
} | null {
  // biome-ignore lint/suspicious/noExplicitAny: React Native type interop
  const NotificationConstructor = (globalThis as { Notification?: any })
    .Notification;
...
function dispatchWebNotificationClick(
  detail: WebNotificationClickDetail
): boolean {
  const dispatch = (globalThis as { dispatchEvent?: (event: Event) => boolean })
    .dispatchEvent;
