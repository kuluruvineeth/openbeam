...
import {
  deriveSidebarShortcutAgentKeys,
  parseSidebarAgentKey,
} from "./sidebar-shortcuts";

describe("parseSidebarAgentKey", () => {
  it("parses serverId and agentId", () => {
    expect(parseSidebarAgentKey("server:agent")).toEqual({
      serverId: "server",
      agentId: "agent",
    });
  });
...
describe("deriveSidebarShortcutAgentKeys", () => {
  it("skips collapsed projects and preserves visual order", () => {
    ...
    expect(
      deriveSidebarShortcutAgentKeys(sections, new Set(["p2"]), 9)
    ).toEqual(["s:a1", "s:a2"]);
  });
  ...
  it("limits to 9", () => {
    ...
    expect(deriveSidebarShortcutAgentKeys(sections, new Set(), 9)).toHaveLength(
      9
    );
    expect(deriveSidebarShortcutAgentKeys(sections, new Set(), 9)[0]).toBe(
      "s:a1"
    );
    expect(deriveSidebarShortcutAgentKeys(sections, new Set(), 9)[8]).toBe(
      "s:a9"
    );
  });
});
