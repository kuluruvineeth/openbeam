...
function pickBestReason(
  ...
  const genericReason =
    reason &&
    (reason.toLowerCase() === "transport error" ||
      reason.toLowerCase() === "transport closed");
  ...
  return new Promise<{
    ...
    unsubscribe = client.subscribeConnectionStatus((state) => {
      if (state.status === "disconnected") {
        ...
        finishErr(
          new DaemonConnectionTestError(message, { reason, lastError })
        );
      }
    });
...
function resolveTimeout(
  ...
  return connection.type === "relay" ? 10_000 : 6000;
}
...
export async function probeConnection(
  ...
  const { client, serverId, hostname } = await connectAndProbe(
    config,
    resolveTimeout(connection, options)
  );
...
export async function measureConnectionLatency(
  ...
  const config = await buildClientConfig(connection, options?.serverId);
  const { client } = await connectAndProbe(
    config,
    resolveTimeout(connection, options)
  );
  try {
