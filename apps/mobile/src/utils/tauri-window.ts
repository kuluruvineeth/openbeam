...
import {
  getIsTauriMac,
  TAURI_TRAFFIC_LIGHT_HEIGHT,
  TAURI_TRAFFIC_LIGHT_WIDTH,
} from "../constants/layout";
import { getCurrentTauriWindow, getTauri, isTauriEnvironment } from "./tauri";
  ...
  try {
    ...
    if (typeof invoke === "function") {
      tauriWindow = {
        ...
        show: () => invoke("plugin:window|show", { label: "main" }),
        unminimize: () => invoke("plugin:window|unminimize", { label: "main" }),
        setFocus: () => invoke("plugin:window|set_focus", { label: "main" }),
        isFullscreen: () =>
          invoke("plugin:window|is_fullscreen", { label: "main" }),
        onResized: (handler: (e: unknown) => void) =>
          tauri?.event?.listen?.("tauri://resize", handler),
      };
      return tauriWindow;
    }
...
export async function focusMainWindow() {
  ...
  if (typeof win.show === "function") {
    try {
      ...
      console.warn("[TauriWindow] show failed", error);
    }
  }
  ...
  if (typeof win.unminimize === "function") {
    try {
      ...
      console.warn("[TauriWindow] unminimize failed", error);
    }
  }
  ...
  if (typeof win.setFocus === "function") {
    try {
      ...
      console.warn("[TauriWindow] setFocus failed", error);
    }
  }
}
...
export function useTauriDragHandlers() {
  ...
  return {
    onMouseDown: (e: React.MouseEvent) => {
      ...
      if (typeof e.buttons === "number" ? e.buttons !== 1 : e.button !== 0) {
        return;
      }
