...
interface SessionStoreActions {
  ...
  setAgents: (
    ...
    agents:
      | Map<string, Agent>
      | ((prev: Map<string, Agent>) => Map<string, Agent>)
  ) => void;
  ...
  setPendingPermissions: (
    ...
    perms:
      ...
      | ((
          prev: Map<string, PendingPermission>
        ) => Map<string, PendingPermission>)
  ) => void;
  ...
  setFileExplorer: (
    ...
    state:
      ...
      | ((
          prev: Map<string, AgentFileExplorerState>
        ) => Map<string, AgentFileExplorerState>)
  ) => void;
  ...
  subscribeWithSelector((set, get) => ({
    ...
    initializeSession: (serverId, client, audioPlayer) => {
      set((prev) => {
        ...
        return {
          ...
          sessions: {
            ...prev.sessions,
            [serverId]: createInitialSessionState(
              serverId,
              client,
              audioPlayer
            ),
    ...
    setMessages: (serverId, messages) => {
      set((prev) => {
        ...
        logSessionStoreUpdate("setMessages", serverId, {
    ...
    setCurrentAssistantMessage: (serverId, message) => {
      set((prev) => {
        ...
        logSessionStoreUpdate("setCurrentAssistantMessage", serverId, {
    ...
    setAgentStreamTail: (serverId, state) => {
      set((prev) => {
        ...
        const nextState =
          typeof state === "function" ? state(session.agentStreamTail) : state;
        if (session.agentStreamTail === nextState) {
          return prev;
        }
        logSessionStoreUpdate("setAgentStreamTail", serverId, {
    ...
    setAgentStreamHead: (serverId, state) => {
      set((prev) => {
        ...
        const nextState =
          typeof state === "function" ? state(session.agentStreamHead) : state;
        if (session.agentStreamHead === nextState) {
          return prev;
        }
        logSessionStoreUpdate("setAgentStreamHead", serverId, {
          agentCount: nextState.size,
        });
        return {
          ...prev,
      ...
      set((prev) => {
        ...
        const nextState =
          typeof state === "function"
            ? state(session.agentTimelineCursor)
            : state;
    ...
    markAgentHistorySynchronized: (serverId, agentId) => {
      set((prev) => {
        ...
        const previousGeneration =
          session.agentHistorySyncGeneration.get(agentId);
    ...
    setInitializingAgents: (serverId, state) => {
      set((prev) => {
        ...
        const nextState =
          typeof state === "function"
            ? state(session.initializingAgents)
            : state;
        if (session.initializingAgents === nextState) {
          return prev;
        }
        logSessionStoreUpdate("setInitializingAgents", serverId, {
          count: nextState.size,
        });
        return {
          ...prev,
          sessions: {
    ...
    setAgents: (serverId, agents) => {
      set((prev) => {
        ...
        const nextAgents =
          typeof agents === "function" ? agents(session.agents) : agents;
        if (session.agents === nextAgents) {
          return prev;
        }
        logSessionStoreUpdate("setAgents", serverId, {
      ...
      set((prev) => {
        ...
        const nextPerms =
          typeof perms === "function"
            ? perms(session.pendingPermissions)
            : perms;
        if (session.pendingPermissions === nextPerms) {
          return prev;
        }
        logSessionStoreUpdate("setPendingPermissions", serverId, {
          count: nextPerms.size,
        });
        return {
          ...prev,
          sessions: {
    ...
    setFileExplorer: (serverId, state) => {
      set((prev) => {
        ...
        const nextState =
          typeof state === "function" ? state(session.fileExplorer) : state;
        if (session.fileExplorer === nextState) {
          return prev;
        }
        logSessionStoreUpdate("setFileExplorer", serverId, {
          agentCount: nextState.size,
        });
        return {
          ...prev,
    ...
    setQueuedMessages: (serverId, value) => {
      set((prev) => {
        ...
        const nextValue =
          typeof value === "function" ? value(session.queuedMessages) : value;
        if (session.queuedMessages === nextValue) {
          return prev;
        }
        logSessionStoreUpdate("setQueuedMessages", serverId, {
          agentCount: nextValue.size,
        });
        return {
          ...prev,
          sessions: {
