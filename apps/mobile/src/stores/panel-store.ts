...
function logPanelTransition(
  ...
  const stack =
    // biome-ignore lint/suspicious/useErrorMessage: necessary for this context
...
function resolveExplorerTabFromActiveCheckout(
  state: PanelState
): ExplorerTab | null {
    ...
    (set) => ({
      ...
      closeFileExplorer: () =>
        set((state) => {
          const nextState = {
            mobileView:
              state.mobileView === "file-explorer"
                ? ("agent" as const)
                : state.mobileView,
            desktop: {
              ...state.desktop,
      ...
      closeToAgent: () =>
        set((state) => {
          const nextState = {
            ...
            desktop: {
              agentListOpen:
                state.mobileView === "agent-list"
                  ? false
                  : state.desktop.agentListOpen,
              fileExplorerOpen:
                state.mobileView === "file-explorer"
                  ? false
