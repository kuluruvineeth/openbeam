...
const FIT_TIMEOUT_DELAYS_MS = [0, 16, 48, 120, 250, 500, 1000, 2000];
const OUTPUT_OPERATION_TIMEOUT_MS = 5000;
...
export class TerminalEmulatorRuntime {
  ...
  private readonly outputOperations: TerminalOutputOperation[] = [];
  private inFlightOutputOperation: TerminalOutputOperation | null = null;
  private inFlightOutputOperationTimeout: ReturnType<typeof setTimeout> | null =
    null;
  ...
  mount(input: TerminalEmulatorRuntimeMountInput): void {
    ...
    terminal.attachCustomKeyEventHandler((event) => {
      ...
      if (
        this.pendingModifiers.ctrl ||
        this.pendingModifiers.shift ||
        this.pendingModifiers.alt
      ) {
  ...
  unmount(): void {
    ...
    const pendingOperations = this.outputOperations.splice(
      0,
      this.outputOperations.length
    );
  ...
  private applyDocumentBoundsStyles(input: {
    root: HTMLDivElement;
  }): () => void {
  ...
  private applyViewportTouchStyles(input: {
    host: HTMLDivElement;
  }): () => void {
    const viewportElement =
      input.host.querySelector<HTMLElement>(".xterm-viewport");
    const screenElement =
      input.host.querySelector<HTMLElement>(".xterm-screen");

    const previousViewportOverscroll =
      viewportElement?.style.overscrollBehavior ?? "";
    const previousViewportTouchAction =
    ...
    const previousViewportPointerEvents =
    ...
    const previousViewportWebkitOverflowScrolling =
      ...
      "";
    const previousScreenPointerEvents =
  ...
  private setupTouchScrollHandlers(input: {
    ...
    const touchMoveHandler = (event: TouchEvent) => {
      ...
      const lineDelta = Math.trunc(
        touchScrollRemainderPx / touchScrollLineHeightPx
      );
      if (lineDelta !== 0) {
