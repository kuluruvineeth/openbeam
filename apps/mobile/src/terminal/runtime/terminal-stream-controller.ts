...
export class TerminalStreamController {
  ...
  // biome-ignore lint/correctness/noUnusedPrivateClassMembers: written in updateStatus, read via options.onStatusChange
    ...
    for (let attempt = 0; attempt < maxAttachAttempts; attempt += 1) {
      ...
      try {
        ...
        if (
          ...
          if (typeof attachPayload.streamId === "number") {
            ...
            void this.options.client
              .detachTerminalStream(attachPayload.streamId)
              .catch(() => {
                /* intentional no-op */
              });
          }
        ...
        if (attachPayload.error || typeof attachPayload.streamId !== "number") {
          lastErrorMessage =
            attachPayload.error ?? "Unable to attach terminal stream";
          terminalDebugLog({
            scope: "stream-controller",
          ...
          if (
            ...
            isRetryableError({ message: lastErrorMessage })
          ) {
        ...
        const unsubscribe = this.options.client.onTerminalStreamData(
          streamId,
          (chunk) => {
            this.handleChunk({
              terminalId: input.terminalId,
        ...
        lastErrorMessage =
          error instanceof Error
            ? error.message
            : "Unable to attach terminal stream";
        terminalDebugLog({
          scope: "stream-controller",
  ...
  private async detachActiveStream(input: {
