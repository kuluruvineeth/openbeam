...
export class SpeechSegmenter {
  ...
  constructor(
    ...
    callbacks: SpeechSegmenterCallbacks = {}
  ) {
    ...
    this.config = {
      ...
      pcmSampleRate: config.pcmSampleRate ?? 16_000,
  ...
  updateConfig(next: SpeechSegmenterConfig): void {
    ...
    this.config = {
      ...
      confirmedDropGracePeriodMs:
        next.confirmedDropGracePeriodMs ??
        this.config.confirmedDropGracePeriodMs,
      minChunkDurationMs:
        next.minChunkDurationMs ?? this.config.minChunkDurationMs,
  ...
  pushVolumeLevel(volume: number, nowMs: number): void {
    ...
    const speechDetectedForContinue =
      volume > this.config.volumeReleaseThreshold;
  ...
  private recomputeDerived(): void {
    this.pcmBytesPerMs =
      ...
      1000;
    this.minChunkBytes = Math.round(
      this.pcmBytesPerMs * this.config.minChunkDurationMs
    );
  }
