...
function computePosition({
  ...
  if (
    placement === "bottom" &&
    spaceBottom < contentHeight &&
    spaceTop > spaceBottom
  ) {
    actualPlacement = "top";
  } else if (
    placement === "top" &&
    spaceTop < contentHeight &&
...
export function DropdownMenuItem({
  ...
  if (isPending) {
    leadingContent = (
      <ActivityIndicator color={theme.colors.foregroundMuted} size={16} />
    );
  } else if (isSuccess) {
    leadingContent = (
      <CheckCircle color={theme.colors.palette.green[500]} size={16} />
    );
  } else if (leading) {
  ...
  return (
    ...
    >
      ...
      {trailingContent ? (
        <View style={styles.trailingSlot}>{trailingContent}</View>
      ) : null}
    </Pressable>
  );
}
