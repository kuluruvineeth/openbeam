    ...
    >
      {kind === "directory" || kind === "file" ? (
        <View style={styles.comboboxItemLeadingSlot}>
          {kind === "directory" ? (
            <Folder color={theme.colors.foregroundMuted} size={16} />
          ) : (
            <File color={theme.colors.foregroundMuted} size={16} />
          )}
        </View>
      ) : null}
      ...
      {selected ? (
        <View style={styles.comboboxItemTrailingSlot}>
          <Check color={theme.colors.foregroundMuted} size={16} />
        </View>
      ) : null}
    </Pressable>
  );
...
export function ComboboxEmpty({
  ...
  return (
    <Text style={styles.emptyText} testID="combobox-empty-text">
      {children}
    </Text>
  );
}
...
export function Combobox({
  ...
  useEffect(() => {
    ...
    const selectedIndex = orderedVisibleOptions.findIndex(
      (opt) => opt.id === value
    );
  ...
  const handleDesktopKey = useCallback(
    (
      ...
      if (key === "Enter") {
        ...
        event?.preventDefault();
        const index =
          activeIndex >= 0 && activeIndex < orderedVisibleOptions.length
            ? activeIndex
            : 0;
  ...
  if (!isOpen) {
    // biome-ignore lint/complexity/noUselessFragments: necessary for this context
    return <></>;
  }
  ...
  return (
    ...
    >
      ...
      >
        <Pressable onPress={handleClose} style={styles.desktopBackdrop} />
        <Animated.View
          collapsable={false}
