...
function normalizeTransportMessage(
...
function buildConnectionFailureCopy(
  ...
  if (rawLower.includes("timed out")) {
    ...
    detail =
      "Connection was refused. Is the daemon running on that host and port?";
    ...
    rawLower.includes("host is unreachable")
  ) {
...
export function AddHostModal({
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Invalid host:port";
    ...
    try {
      ...
      const { title, detail, raw } = buildConnectionFailureCopy(
        endpoint,
        error
      );
      const combined =
  ...
  return (
    ...
    >
      <Text style={styles.helper}>
        Connect to a daemon by entering host:port.
      </Text>
      ...
      <View style={styles.actions}>
        ...
        <Button
          disabled={isSaving}
          leftIcon={<Link2 color={theme.colors.palette.white} size={16} />}
          // biome-ignore lint/complexity/noVoid: fire-and-forget async call
