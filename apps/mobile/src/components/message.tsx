...
function canElementScrollInDirection(
  ...
  const overflow =
    axis === "x" ? computedStyle.overflowX : computedStyle.overflowY;
...
export const TurnCopyButton = memo(function TurnCopyButton({
  ...
  return (
    <Pressable
      ...
      onHoverIn={
        Platform.OS === "web" ? () => onHoverChange?.(true) : undefined
      }
      onHoverOut={
        Platform.OS === "web" ? () => onHoverChange?.(false) : undefined
      }
      onPress={handleCopy}
  ...
  const markdownRules = useMemo(
    () => ({
      ...
      code_inline: (
        ...
        const parsed = onInlinePathPress ? parseInlinePathToken(content) : null;
        ...
        const inlineCodeLinkUrl = getInlineCodeAutoLinkUrl(
          markdownParser,
          content
        );
          ...
          <View style={activityLogStylesheet.textContainer}>
            ...
            {metadata && (
              <View style={activityLogStylesheet.detailsRow}>
                ...
                {isExpanded ? (
                  <ChevronDown color="#71717a" size={12} />
                ) : (
                  <ChevronRight color="#71717a" size={12} />
                )}
              </View>
            )}
          </View>
...
export const TodoListCard = memo(function TodoListCard({
  ...
  const renderDetails = useCallback(
            ...
            items.map((item, idx) => (
              ...
              >
                ...
                >
                  {item.completed ? (
                    <Check
                      color={unistylesTheme.colors.primaryForeground}
                      size={12}
                    />
                  ) : null}
                </View>
    ...
    [items, unistylesTheme.colors.primaryForeground]
  );
...
const ExpandableBadge = memo(function ExpandableBadge({
  ...
  const shimmerDuration = Math.max(
    ...
    Math.min(
      2.3,
      1.25 + totalShimmerChars * 0.008 - shortTextDurationAdjustment
    )
  );
  ...
  const isNativeShimmer =
    shouldMeasureNativeShimmer && labelRowWidth > 0 && labelRowHeight > 0;
  ...
  if (isError) {
    iconNode = <TriangleAlertIcon color={iconColor} opacity={0.8} size={12} />;
  } else if (IconComponent) {
    iconNode = <IconComponent color={iconColor} size={12} />;
  }
        ...
        {({ hovered }) => (
          <>
            <View style={expandableBadgeStylesheet.headerRow}>
              ...
              <View
                onLayout={
                  shouldMeasureNativeShimmer ? handleLabelRowLayout : undefined
                }
              ...
              >
                <Text
                  numberOfLines={1}
                  onLayout={
                    shouldMeasureWebShimmer ? handleLabelLayout : undefined
                  }
                  style={[
...
function areExpandableBadgePropsEqual(
  ...
  if (previous.secondaryLabel !== next.secondaryLabel) {
    return false;
  }
  ...
  if (previous.isLastInSequence !== next.isLastInSequence) {
    return false;
  }
  if (previous.disableOuterSpacing !== next.disableOuterSpacing) {
    return false;
  }
  if (previous.testID !== next.testID) {
    return false;
  }
  if (Boolean(previous.onToggle) !== Boolean(next.onToggle)) {
    return false;
  }
