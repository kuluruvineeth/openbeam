    ...
    >
      {file.status === "too_large" || file.status === "binary" ? (
        ...
        <ScrollView
          ...
          waitFor={
            isAtLeftEdge && closeGestureRef?.current
              ? closeGestureRef
              : undefined
          }
        >
          <View
            style={[
              ...
              scrollViewWidth > 0 && { minWidth: scrollViewWidth },
            ]}
          >
...
export function GitDiffPane({ serverId, agentId, cwd }: GitDiffPaneProps) {
  ...
  const [diffModeOverride, setDiffModeOverride] = useState<
    "uncommitted" | "base" | null
  >(null);
  const [actionError, setActionError] = useState<string | null>(null);
  const [postShipArchiveSuggested, setPostShipArchiveSuggested] =
    useState(false);
  ...
  const statusErrorMessage =
    ...
    (isStatusError && statusError instanceof Error
      ? statusError.message
      : null);
  ...
  const computeHeaderOffset = useCallback(
    (path: string): number => {
      ...
      for (const file of files) {
        ...
        offset +=
          headerHeightByPathRef.current[file.path] ?? defaultHeaderHeight;
  ...
  const handleToggleExpanded = useCallback(
    (path: string) => {
      ...
      const targetOffset = isCurrentlyExpanded
        ? computeHeaderOffset(path)
        : null;
      const headerHeight =
        headerHeightByPathRef.current[path] ?? defaultHeaderHeightRef.current;
      const shouldAnchor =
        isCurrentlyExpanded &&
        targetOffset !== null &&
  ...
  const runMergeBranch = useCheckoutGitActionsStore(
    (state) => state.mergeBranch
  );
  const runMergeFromBase = useCheckoutGitActionsStore(
    (state) => state.mergeFromBase
  );
  const runArchiveWorktree = useCheckoutGitActionsStore(
  ...
  const handleMergeFromBase = useCallback(() => {
    ...
    void runMergeFromBase({ serverId, cwd, baseRef }).catch((err) => {
      const message =
        err instanceof Error ? err.message : "Failed to merge from base";
      setActionError(message);
    });
  ...
  const handleArchiveWorktree = useCallback(() => {
    ...
    void runArchiveWorktree({ serverId, cwd, worktreePath })
      ...
      .catch((err) => {
        const message =
          err instanceof Error ? err.message : "Failed to archive worktree";
        setActionError(message);
      });
  ...
  const prErrorMessage = githubFeaturesEnabled
    ? (prPayloadError?.message ?? null)
    : null;
  const branchLabel =
  ...
  const gitActions: GitActions = useMemo(() => {
    ...
    allActions.set("commit", {
      ...
      status: commitStatus,
      icon: (
        <GitCommitHorizontal color={theme.colors.foregroundMuted} size={16} />
      ),
    ...
    if (hasRemote) {
      allActions.set("push", {
        ...
        status: pushStatus,
        description: hasRemote ? undefined : "No remote configured",
        icon: <Upload color={theme.colors.foregroundMuted} size={16} />,
        handler: handlePush,
      });
    }
    ...
    if (githubFeaturesEnabled && hasPullRequest && prStatus?.url) {
      ...
      allActions.set("view-pr", {
        ...
        disabled: false,
        status: "idle",
        icon: <GitHubIcon color={theme.colors.foregroundMuted} size={16} />,
    ...
    if (aheadCount > 0) {
      allActions.set("merge-branch", {
        ...
        description: hasUncommittedChanges
          ...
          : undefined,
        icon: <GitMerge color={theme.colors.foregroundMuted} size={16} />,
      ...
      allActions.set("merge-from-base", {
        ...
        description: hasUncommittedChanges
          ...
          : // biome-ignore lint/style/noNestedTernary: readable inline conditional
            ...
            : undefined,
        icon: <RefreshCcw color={theme.colors.foregroundMuted} size={16} />,
    ...
    if (isOpenPlaneOwnedWorktree) {
      allActions.set("archive-worktree", {
        ...
        icon: <Archive color={theme.colors.foregroundMuted} size={16} />,
    ...
    // Rule 5: Ahead of base → Ship action based on preference
      ...
      const preferred: GitActionId =
        shipDefault === "merge" ? "merge-branch" : "create-pr";
      const fallback: GitActionId =
        shipDefault === "merge" ? "create-pr" : "merge-branch";
    ...
    const primary = primaryActionId
      ? (allActions.get(primaryActionId) ?? null)
      : null;
    ...
    <View style={styles.container}>
      <View style={styles.header} testID="changes-header">
        <View style={styles.headerLeft}>
          <GitBranch color={theme.colors.foregroundMuted} size={16} />
          <Text
            numberOfLines={1}
            style={styles.branchLabel}
            testID="changes-branch"
          >
            {branchLabel}
          </Text>
          {isStatusFetching && (
          ...
          <View style={styles.headerRight}>
              ...
              <View style={styles.splitButton}>
                ...
                {gitActions.secondary.length > 0 ? (
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent
                      align="end"
                      testID="changes-primary-cta-menu"
                    >
                      {gitActions.secondary.map((action, index) => {
                        const needsSeparator =
                          action.id === "merge-from-base" ||
                          action.id === "push";
                        return (
                          <View key={action.id}>
            ...
            {gitActions.menu.length > 0 ? (
              <DropdownMenu>
                ...
                <DropdownMenuContent
                  align="end"
                  testID="changes-overflow-content"
                  width={220}
                >
