...
export interface MessageInputProps {
  ...
  /**
   * Emits finalized dictation transcript segments for global paste shortcuts.
   * Return true to mark transcript as externally handled and skip local insertion.
   */
  onDictationTranscript?: (text: string) => boolean | undefined;
  /** Desktop only: mutes and restores system audio through native helper while recording dictation. */
  enableNativeHelperSystemAudioMute?: boolean;
  /** Desktop only: primes native helper accessibility context at dictation start. */
  primeNativeHelperAccessibilityContextOnStart?: boolean;
  /** Stable key identifying the owning message input, used for global dictation shortcut coordination. */
  agentKey?: string;
}
...
export const MessageInput = forwardRef<MessageInputRef, MessageInputProps>(
  ...
  function MessageInput(
    ...
    useImperativeHandle(ref, () => ({
      ...
      isDictating: () => isDictatingRef.current,
      getNativeElement: () => {
        if (!IS_WEB) {
          return null;
        }
    ...
    function handleDictationTranscript(
      text: string,
      ...
      const handledExternally = onDictationTranscript?.(text) === true;
    ...
    const isDictationStartEnabled =
      (isReadyForDictation ?? isConnected) && !disabled;
    ...
    useEffect(() => {
      ...
      useKeyboardShortcutsStore.getState().reportDictationActivity({
        agentKey: normalizedAgentKey,
        isRecording: isDictating,
        isProcessing: isDictationProcessing,
      });
    }, [agentKey, isDictating, isDictationProcessing]);
    ...
    useEffect(() => {
      ...
      return () => {
        useKeyboardShortcutsStore
          .getState()
          .clearDictationActivity(normalizedAgentKey);
      };
    }, [agentKey]);
    ...
    const handleToggleRealtimeVoiceShortcut = useCallback(() => {
      if (
        !(voice && voiceServerId && voiceAgentId && isConnected) ||
        disabled
      ) {
        return;
      }
    ...
    useEffect(() => {
      ...
      const handlePaste = (event: ClipboardEvent) => {
        ...
        const imageFiles = collectImageFilesFromClipboardData(
          event.clipboardData
        );
        ...
        void filesToImageAttachments(imageFiles)
          ...
          .catch((error) => {
            console.error(
              "[MessageInput] Failed to process pasted images:",
              error
            );
          });
      };
    ...
    function measureWebInputHeight(_source: string): boolean {
      ...
      const textarea = getWebTextArea();
      if (!textarea || typeof textarea.scrollHeight !== "number") {
        return false;
      }
    ...
    function handleDesktopKeyPress(event: WebTextInputKeyPressEvent) {
      ...
      if ((metaKey || ctrlKey) && isAgentRunning && onQueue) {
        if (isSubmitDisabled || isSubmitLoading || disabled) {
          return;
        }
        event.preventDefault();
        handleQueueMessage();
                      ...
                      {onRemoveImage && (
                        <View
                          style={[
                            ...
                            (hovered || !IS_WEB) &&
                              styles.removeImageButtonVisible,
                          ]}
                        >
                          <X color="white" size={theme.iconSize.md} />
                        </View>
                      )}
                    </>
                  )}
            ...
            <View style={styles.rightButtonGroup}>
              ...
              {hasSendableContent && isAgentRunning && onQueue && (
                ...
                >
                  ...
                  >
                    <Plus color="white" size={theme.iconSize.lg} />
                  </TooltipTrigger>
                  <TooltipContent align="center" offset={8} side="top">
                    <View style={styles.tooltipRow}>
                ...
                >
                  ...
                  >
                    {isSubmitLoading ? (
                      ...
                      <ArrowUp color="white" size={theme.iconSize.lg} />
                    )}
                  </TooltipTrigger>
                  <TooltipContent align="center" offset={8} side="top">
                    <View style={styles.tooltipRow}>
