...
export function WebDesktopScrollbarOverlay({
  ...
  const scrollVisibilityTimeoutRef = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  const scrollActiveTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
    null
  );
  ...
  const handleOpacity = isDragging
    ...
    : // biome-ignore lint/style/noNestedTernary: readable inline conditional
      ...
      : // biome-ignore lint/style/noNestedTernary: readable inline conditional
        ...
        : 0;
  const handleWidth =
    isDragging || isHandleHovered ? HANDLE_WIDTH_ACTIVE : HANDLE_WIDTH_IDLE;
  ...
  const thumbRegionOffset = Math.max(
    0,
    geometry.handleOffset - HANDLE_GRAB_VERTICAL_PADDING
  );
  ...
  const handleInsetTop = Math.max(
    0,
    (thumbRegionHeight - geometry.handleSize) / 2
  );
