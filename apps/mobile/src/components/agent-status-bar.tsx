...
export function AgentStatusBar({ agentId, serverId }: AgentStatusBarProps) {
  ...
  const selectedThinking =
    thinkingOptions?.find((o) => o.id === selectedThinkingId) ?? null;
  const displayThinking =
    selectedThinking?.label ??
    (selectedThinkingId === "default"
      ? "Model default"
      : (selectedThinkingId ?? "auto"));
  ...
  return (
    <View
      style={[styles.container, IS_WEB && { marginBottom: -theme.spacing[1] }]}
    >
                ...
                return (
                  <DropdownMenuItem
                    ...
                    onSelect={() => {
                      ...
                      // biome-ignore lint/complexity/noVoid: fire-and-forget async call
                      void client
                        .setAgentModel(agentId, model.id)
                        .catch((error) => {
                          console.warn(
                            "[AgentStatusBar] setAgentModel failed",
                            error
                          );
                        });
                    }}
                    ...
                    <DropdownMenuItem
                      ...
                      onSelect={() => {
                        ...
                        void client
                          ...
                          .catch((error) => {
                            console.warn(
                              "[AgentStatusBar] setAgentThinkingOption failed",
                              error
                            );
                          });
                      }}
