...
export function AudioDebugNotice({
  ...
  const _pathMissing = !info.debugRecordingPath;
  ...
  return (
    ...
    >
      <View style={styles.headerRow}>
        <Text style={[styles.title, { color: theme.colors.foregroundMuted }]}>
          {title}
        </Text>
        {onDismiss ? (
          <Pressable
            accessibilityLabel="Dismiss audio debug"
            hitSlop={8}
            onPress={onDismiss}
          >
      ...
      {info.debugRecordingPath ? (
        ...
        >
          ...
          <View
            style={[styles.copyPill, { backgroundColor: theme.colors.primary }]}
          >
            {copied ? (
              <Check color={theme.colors.primaryForeground} size={12} />
            ) : (
              <Copy color={theme.colors.primaryForeground} size={12} />
            )}
          </View>
        </Pressable>
      ) : (
        <Text style={[styles.hint, { color: theme.colors.foregroundMuted }]}>
          Raw audio path unavailable. Set STT_DEBUG_AUDIO_DIR on the server to
          persist recordings.
        </Text>
      )}
