...
export async function processGmailWebhook(
  ...
  logger.debug(
    ...
    "Processing Gmail webhook event"
  );
  ...
  const notificationPayload = payload as GmailNotificationPayload;
  ...
  logger.info(
    {
      ...
      historyId: notificationPayload.historyId,
    },
    "Gmail webhook triggered incremental sync"
  );
...
export function shouldProcessGmailRealtime(_eventType: string): boolean {
  return true;
}
