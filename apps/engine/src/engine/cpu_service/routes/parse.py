from __future__ import annotations

from pathlib import Path
from tempfile import NamedTemporaryFile
from typing import Literal

import httpx
from fastapi import APIRouter, HTTPException, Request, UploadFile

from engine.common.config import get_cpu_settings
from engine.common.logging import get_logger
from engine.common.metrics import DOCUMENTS_PARSED_TOTAL
from engine.common.security.utils import is_safe_url
from engine.models.requests import ParseUrlRequest
from engine.models.responses import ParseResponse
from engine.services.parser import ParserService

ParserStrategy = Literal["fast", "hi_res", "ocr_only", "auto"]

router = APIRouter()
logger = get_logger(__name__)
parser_service = ParserService()


@router.post("", response_model=ParseResponse)
async def parse_document(
    request: Request,
    file: UploadFile,
    chunk: bool = True,
    max_chunk_size: int = 1500,
    overlap: int = 150,
    strategy: ParserStrategy | None = None,
) -> ParseResponse:
    settings = get_cpu_settings()

    if not file.filename:
        raise HTTPException(status_code=400, detail="Filename required")

    content = await file.read()
    file_size_mb = len(content) / (1024 * 1024)

    if file_size_mb > settings.max_file_size_mb:
        raise HTTPException(
            status_code=413,
            detail=f"File too large. Max size: {settings.max_file_size_mb}MB",
        )

    suffix = Path(file.filename).suffix
    tmp_path = None
    try:
        with NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
            tmp.write(content)
            tmp_path = Path(tmp.name)

        result = await parser_service.parse(
            tmp_path, mime_type=file.content_type, strategy=strategy
        )

        chunks = None
        if chunk and result.elements:
            chunker = request.app.state.chunker
            chunks = await chunker.chunk_elements(
                result.elements,
                max_characters=max_chunk_size,
                overlap=overlap,
            )

        DOCUMENTS_PARSED_TOTAL.labels(
            file_type=suffix.lstrip(".") or "unknown",
            status="success",
        ).inc()

        return ParseResponse(
            filename=file.filename,
            mime_type=file.content_type,
            elements=result.elements,
            chunks=chunks,
            metadata=result.metadata,
            text_length=result.text_length,
            page_count=result.page_count,
        )
    except Exception as e:
        DOCUMENTS_PARSED_TOTAL.labels(
            file_type=suffix.lstrip(".") if suffix else "unknown",
            status="error",
        ).inc()
        logger.error("parse_failed", filename=file.filename, error=str(e))
        raise HTTPException(status_code=500, detail=str(e)) from e
    finally:
        if tmp_path is not None:
            tmp_path.unlink(missing_ok=True)


@router.post("/url", response_model=ParseResponse)
async def parse_from_url(
    request: Request,
    body: ParseUrlRequest,
    chunk: bool = True,
    max_chunk_size: int = 1500,
    overlap: int = 150,
    strategy: ParserStrategy | None = None,
) -> ParseResponse:
    is_safe, error_msg = is_safe_url(body.url)
    if not is_safe:
        raise HTTPException(status_code=400, detail=f"Invalid URL: {error_msg}")

    settings = get_cpu_settings()
    try:
        async with httpx.AsyncClient(
            timeout=httpx.Timeout(30.0, connect=10.0),
            limits=httpx.Limits(max_connections=10, max_keepalive_connections=5),
        ) as client:
            response = await client.get(body.url, follow_redirects=True)
            response.raise_for_status()

            content_length = response.headers.get("content-length")
            if content_length and int(content_length) > settings.max_file_size_mb * 1024 * 1024:
                raise HTTPException(
                    status_code=413,
                    detail=f"Remote file too large. Max size: {settings.max_file_size_mb}MB",
                )

            content = response.content
            content_type = response.headers.get("content-type")
            if content_type:
                content_type = content_type.split(";")[0].strip()
    except httpx.TimeoutException as e:
        raise HTTPException(status_code=504, detail=f"URL fetch timeout: {e}") from e
    except httpx.HTTPError as e:
        raise HTTPException(status_code=400, detail=f"Failed to fetch URL: {e}") from e

    filename = body.filename or body.url.split("/")[-1].split("?")[0]
    suffix = Path(filename).suffix or ".bin"

    effective_strategy = body.strategy or strategy

    tmp_path = None
    try:
        with NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
            tmp.write(content)
            tmp_path = Path(tmp.name)

        result = await parser_service.parse(
            tmp_path, mime_type=content_type, strategy=effective_strategy
        )

        chunks = None
        if chunk and result.elements:
            chunker = request.app.state.chunker
            chunks = await chunker.chunk_elements(
                result.elements,
                max_characters=max_chunk_size,
                overlap=overlap,
            )

        DOCUMENTS_PARSED_TOTAL.labels(
            file_type=suffix.lstrip(".") or "unknown",
            status="success",
        ).inc()

        return ParseResponse(
            filename=filename,
            mime_type=content_type,
            elements=result.elements,
            chunks=chunks,
            metadata=result.metadata,
            text_length=result.text_length,
            page_count=result.page_count,
        )
    except Exception as e:
        DOCUMENTS_PARSED_TOTAL.labels(
            file_type=suffix.lstrip(".") or "unknown",
            status="error",
        ).inc()
        logger.error("parse_url_failed", url=body.url, error=str(e))
        raise HTTPException(status_code=500, detail=str(e)) from e
    finally:
        if tmp_path is not None:
            tmp_path.unlink(missing_ok=True)
