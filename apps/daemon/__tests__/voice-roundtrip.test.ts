...
describe("voice roundtrip e2e", () => {
  ...
  for (const targetProvider of [
    ...
    "codex",
    "opencode",
  ] as const satisfies readonly VoiceRoundtripProvider[]) {
    speechTest(
      ...
      async () => {
        ...
        const sttOutput = new STTManager(
          "voice-roundtrip-e2e",
          logger,
          sttProvider
        );
        ...
        const voiceCwd = mkdtempSync(
          join(tmpdir(), `voice-roundtrip-agent-${targetProvider}-`)
        );
        ...
        const outputAudio = await (async () => {
          try {
            const transcriptPromise = waitForSignal<{
              text: string;
            ...
            const outputAudioPromise = waitForSignal<{
              ...
              const offAudio = ctx.client.on("audio_output", (message) => {
                ...
                if (!targetGroupId) {
                  targetGroupId = payload.groupId ?? null;
            ...
            for (
              ...
              await withTimeout(
                "sendVoiceAudioChunk",
                5000,
                ctx.client.sendVoiceAudioChunk(
            ...
            await ctx.client.setVoiceMode(false).catch(() => {});
