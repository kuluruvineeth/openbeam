...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!mode.accepted) {
      throw new Error(
        `setVoiceMode rejected: ${mode.error ?? "unknown error"}`
      );
    }
    ...
    const pcm = await streamToBuffer(
      generated.stream as AsyncIterable<unknown>
    );
    ...
    for (let offset = 0; offset < pcm.length; offset += chunkBytes) {
      const chunk = pcm.subarray(
        offset,
        Math.min(pcm.length, offset + chunkBytes)
      );
    ...
    await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error("Timed out waiting for first audio_output")),
          120_000
        )
      ),
    ]);
    await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error("Timed out waiting for final audio_output")),
          120_000
        )
      ),
    ]);
