...
async function runVoiceRoundTrip(params: {
  ...
  if (!mode.accepted) {
    throw new Error(
      `set_voice_mode rejected: ${mode.error ?? "unknown error"}`
    );
  }
  ...
  const offStream = client.on("agent_stream", (msg) => {
    ...
    if (msg.payload.agentId !== activeVoiceAgentId) {
      return;
    }
    ...
    if (
      ...
      item.name.toLowerCase().includes("speak")
    ) {
      speakToolCalls += 1;
    }
  });
  ...
  try {
    ...
    await client.setVoiceMode(false).catch(() => {});
    // biome-ignore lint/suspicious/noEmptyBlockStatements: intentional no-op
