...
const FeatureVoiceModeSchema = z
  .object({
    ...
    llm: z
      .object({
        provider: z
          .enum(AGENT_PROVIDER_IDS as [string, ...string[]])
          .optional(),
        model: z.string().min(1).optional(),
      })
      .strict()
...
const NativeHelperConfigSchema = z
  .object({
    enabled: z.boolean().optional(),
    ...
    daemon: z
      .object({
        ...
        relay: z
          ...
          .strict()
          .optional(),
        nativeHelper: NativeHelperConfigSchema.optional(),
      })
      .strict()
      .optional(),
...
function stripDeprecatedLocalSpeechConfigFields(parsed: unknown): unknown {
  ...
  if ("autoDownload" in localRecord) {
    localRecord.autoDownload = undefined;
  }
...
export function loadPersistedConfig(
  ...
  if (!existsSync(configPath)) {
    try {
      ...
      throw new Error(
        `[Config] Failed to initialize ${configPath}: ${message}`
      );
    }
  }
