...
function buildPersistence(
  ...
  if (provider === "codex") {
    return {
      provider,
      sessionId,
      metadata: { conversationId: sessionId, ...(metadata ?? {}) },
    };
  }
...
function buildToolCallForPrompt(provider: string, prompt: string) {
  ...
  const createFileMatch =
    ...
    /create a file named\s+"([^"]+)"\s+with the content\s+"([^"]*)"/i.exec(
      prompt
    ) ??
    // biome-ignore lint/performance/useTopLevelRegex: scoped regex acceptable here
    /create a file named\s+"([^"]+)"\s+with the content\s+'([^']*)'/i.exec(
  ...
  if (provider === "claude") {
    ...
    if (text.includes("rm -f permission.txt")) {
      return {
        name: "Bash",
        input: { command: "rm -f permission.txt" },
        output: { ok: true },
      };
    }
    if (text.includes("rm -f mcp-smoke.txt")) {
      return {
        name: "Bash",
        input: { command: "rm -f mcp-smoke.txt" },
        output: { ok: true },
      };
    }
...
class FakeAgentSession implements AgentSession {
  ...
  async run(
    ...
    const usage: AgentUsage | undefined = options
  ...
  async *stream(prompt: AgentPromptInput): AsyncGenerator<AgentStreamEvent> {
    ...
    const markerMatch =
      // biome-ignore lint/performance/useTopLevelRegex: scoped regex acceptable here
      /remember (?:this )?(?:marker|string|project name)[^"]*"([^"]+)"/i.exec(
        textPrompt
      );
    if (markerMatch) {
    ...
    if (tool) {
      const needsPermission = this.needsPermissionForTool(
        tool.name,
        tool.input ?? {}
      );
  ...
  private buildAssistantText(prompt: string): string {
    ...
    if (
      lower.includes("run `pwd`") &&
      lower.includes("respond with exactly: cwd:")
    ) {
    ...
    if (lower.includes("state saved")) {
      return "state saved";
    }
    if (lower.includes("timeline test")) {
    ...
    if (lower.includes("say 'timeline test'")) {
      return "timeline test";
    }
    if (lower.includes("say 'state saved'")) {
      return "state saved";
    }
    if (
      lower.includes("return schema-valid json") ||
      lower.includes("schema-valid json")
    ) {
      return JSON.stringify({ ok: true });
    }
    if (
      lower.includes("what was the marker") ||
      lower.includes("what was the project name")
    ) {
  ...
  private async applyToolSideEffects(
    ...
    if (toolName === "Read" || toolName === "read_file") {
      const p =
        typeof toolInput.path === "string" ? toolInput.path : "/etc/hosts";
      try {
        readFileSync(p, "utf8");
      } catch {
    ...
    if (toolName === "Bash" || toolName === "shell") {
      ...
      if (
        lower.includes("rm -f permission.txt") ||
        command.includes("rm -f permission.txt")
      ) {
      ...
      if (
        lower.includes("rm -f mcp-smoke.txt") ||
        command.includes("rm -f mcp-smoke.txt")
      ) {
        const dest = path.join(
          this.config.cwd ?? process.cwd(),
          "mcp-smoke.txt"
      ...
      if (
        lower.includes("printf") &&
        lower.includes(">") &&
        lower.includes(".txt")
      ) {
        const destMatch =
          // biome-ignore lint/performance/useTopLevelRegex: scoped regex acceptable here
          />\s*([^\s`]+)\s*$/i.exec(command) ?? />\s*([^\s`]+)/i.exec(lower);
    ...
    if (toolName === "Edit" || toolName === "apply_patch") {
      ...
      const filePath =
        match?.[1] ??
        (lowerPrompt.includes("tool-create.txt") ? "tool-create.txt" : null);
      if (filePath) {
        try {
  ...
  private needsPermissionForTool(
    toolName: string,
    toolInput: Record<string, unknown>
  ): boolean {
...
class FakeAgentClient implements AgentClient {
  ...
  async resumeSession(
    ...
    const marker =
      ...
      (handle.metadata as Record<string, unknown> | undefined)
        ?.conversationId ??
      null;
