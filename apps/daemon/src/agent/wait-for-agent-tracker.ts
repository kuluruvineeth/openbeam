...
export type WaitForAgentCanceler = (
  agentId: string,
  reason?: string
) => boolean;
...
export class WaitForAgentTracker {
  private readonly waiters = new Map<string, Set<(reason?: string) => void>>();
  private readonly logger: Logger;

  constructor(logger: Logger) {
    this.logger = logger.child({
      module: "agent",
      component: "wait-for-agent-tracker",
    });
  }
  ...
  cancel(agentId: string, _reason?: string): boolean {
    const waitersForAgent = this.waiters.get(agentId);
