...
function getStructuredContent(result: McpToolResult): StructuredContent | null {
  ...
  if (
    ...
    "structuredContent" in content
  ) {
    const structured = (content as { structuredContent?: StructuredContent })
      .structuredContent;
    if (structured) {
      return structured;
    }
...
async function waitForAgentCompletion(options: {
  ...
  if (payload.permission) {
    throw new Error(
      `Unexpected permission while waiting: ${JSON.stringify(payload.permission)}`
    );
  }
  const status = payload.status;
  if (status === "running" || status === "initializing") {
    throw new Error(
      `Agent still running after wait_for_agent (status=${String(status)})`
    );
  }
}
...
describe("agent MCP end-to-end (offline)", () => {
  ...
  test("create_agent with worktree is async and boots terminals only after setup success", async () => {
    ...
    try {
      ...
      expect(existsSync(path.join(worktreePath, "dev-terminal.txt"))).toBe(
        false
      );
