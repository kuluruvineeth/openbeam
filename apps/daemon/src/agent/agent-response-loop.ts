...
export class StructuredAgentResponseError extends Error {
  ...
  constructor(
    message: string,
    options: { lastResponse: string; validationErrors: string[] }
  ) {
    super(message);
...
export interface StructuredAgentGenerationWithFallbackOptions<T> {
  ...
  schemaName?: string;
  runner?: <TResult>(
    options: StructuredAgentGenerationOptions<TResult>
  ) => Promise<TResult>;
}
...
export async function generateStructuredAgentResponse<T>(
  ...
  const modeId =
    ...
    undefined;
  const validator = buildValidator(schema, schemaName ?? "Response");
  ...
  try {
    const caller: AgentCaller = async (nextPrompt) => {
      const result = await manager.runAgent(agent.id, nextPrompt, {
        outputSchema: validator.jsonSchema,
      });
