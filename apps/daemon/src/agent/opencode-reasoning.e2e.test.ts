...
describe("OpenCode reasoning events (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    agentCwd = await mkdtemp(
      path.join(os.tmpdir(), "opencode-reasoning-test-")
    );
  });
  ...
  test("gpt-5 nano emits reasoning events through daemon", async () => {
    ...
    for (const [_type, _count] of byType) {
    }
    ...
    for (const { event } of timelineEvents) {
      if (event.type === "timeline") {
        itemTypes.set(
          event.item.type,
          (itemTypes.get(event.item.type) ?? 0) + 1
        );
      }
    }

    // biome-ignore lint/suspicious/noEmptyBlockStatements: intentional no-op
    for (const [_type, _count] of itemTypes) {
    }
