...
class CodexAppServerClient {
  ...
  private async handleLine(line: string): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        { error, line },
        "Failed to parse Codex app-server JSON"
      );
      return;
    }
    ...
    if (typeof (msg as JsonRpcResponse).id === "number") {
      ...
      if (
        (msg as JsonRpcResponse).result !== undefined ||
        (msg as JsonRpcResponse).error
      ) {
        ...
        if ((msg as JsonRpcResponse).error) {
          pending.reject(
            new Error(
              (msg as JsonRpcResponse).error?.message ?? "Unknown error"
            )
          );
        } else {
...
function parsePlanTextToTodoItems(
  text: string
...
function parseCodexPatchChanges(changes: unknown): CodexPatchFileChange[] {
  ...
  if (
    typeof recordChanges.path === "string" &&
    recordChanges.path.trim().length > 0
  ) {
    return [
      {
...
export async function codexAppServerTurnInputFromPrompt(
  ...
  const blocks = prompt as unknown[];
...
class CodexAppServerAgentSession implements AgentSession {
  ...
  >();
  private readonly resolvedPermissionRequests = new Set<string>();
  private readonly pendingAgentMessages = new Map<string, string>();
  private readonly pendingReasoning = new Map<string, string[]>();
  private readonly pendingCommandOutputDeltas = new Map<string, string[]>();
  private readonly pendingFileChangeOutputDeltas = new Map<string, string[]>();
  ...
  private readonly warnedInvalidNotificationPayloads = new Set<string>();
    ...
    let match =
      normalized === "read-only"
        ...
        : // biome-ignore lint/style/noNestedTernary: readable inline conditional
          ...
          : findByName(
              (name) => name.includes("auto") || name.includes("code")
            );
    if (!match) {
  ...
  private async ensureThreadLoaded(): Promise<void> {
    ...
    try {
      const loaded = (await this.client.request("thread/loaded/list", {})) as {
        data?: string[];
      };
      ...
      this.logger.warn(
        { error },
        "Failed to resume Codex thread, starting new thread"
      );
  ...
  private async *streamInternal(
    ...
    this.logger.debug(
      { model: this.config.model, mode: this.currentMode },
      "Codex stream connecting"
    );
    await this.connect();
    if (!this.client) {
      this.logger.error("Codex connect() returned but client is null");
      return;
    }
    ...
    try {
      ...
      const approvalPolicy =
        this.config.approvalPolicy ?? preset.approvalPolicy;
    ...
    for (const question of questions) {
      ...
      if (options.length > 0) {
        const byLabel = options.find((opt) =>
          (opt.label ?? "").toLowerCase().includes(decision)
        );
        const byValue = options.find((opt) =>
          (opt.value ?? "").toLowerCase().includes(decision)
        );
        // biome-ignore lint/style/noNonNullAssertion: value guaranteed to be set
  ...
  async listCommands(): Promise<AgentSlashCommand[]> {
    ...
    const fallbackSkills =
      appServerSkills.length === 0
        ? await listCodexSkills(this.config.cwd)
        : [];
  ...
  private async ensureThread(): Promise<void> {
    ...
    if (!model) {
      const configuredDefaults = await readCodexConfiguredDefaults(
        this.client,
        this.logger
      );
    ...
    if (!model) {
      const modelResponse = (await this.client.request(
        "model/list",
        {}
  ...
  private handleNotification(method: string, params: unknown): void {
    ...
    if (parsed.kind === "turn_completed") {
      if (parsed.status === "failed") {
        this.logger.warn(
          { status: parsed.status, errorMessage: parsed.errorMessage },
          "Codex turn failed"
        );
        this.emitEvent({
          type: "turn_failed",
...
export class CodexAppServerAgentClient implements AgentClient {
  ...
  private spawnAppServer(): ChildProcessWithoutNullStreams {
    ...
    return spawn(launchPrefix.command, [...launchPrefix.args, "app-server"], {
      ...
      env: buildChildProcessEnv("codex", this.runtimeSettings),
    });
  }
    ...
    try {
      ...
      for (const thread of threads.slice(0, limit)) {
        ...
        try {
          ...
          timeline =
            rolloutTimeline.length > 0 ? rolloutTimeline : itemsFromThreadRead;
        } catch {
          timeline = [];
        }
        ...
        descriptors.push({
          ...
          lastActivityAt: new Date(
            (thread.updatedAt ?? thread.createdAt ?? 0) * 1000
          ),
          persistence: {
            provider: CODEX_PROVIDER,
            sessionId: threadId,
  ...
  async listModels(
    ...
    try {
      ...
      const response = (await client.request("model/list", {})) as {
        // biome-ignore lint/suspicious/noExplicitAny: daemon type interop
        data?: any[];
      };
      ...
      return models.map((model) => {
        ...
        const thinkingOptions = Array.from(thinkingById.values()).map(
          (option) => ({
            ...option,
        ...
        return {
          ...
          thinkingOptions:
            thinkingOptions.length > 0 ? thinkingOptions : undefined,
