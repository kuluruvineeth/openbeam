...
async function autoApprove(
  session: Awaited<ReturnType<ClaudeAgentClient["createSession"]>>,
...
function _isFileWriteResult(
  value: unknown
): value is { type: "file_write"; filePath: string } {
  return (
    isKeyValueObject(value) &&
    value.type === "file_write" &&
...
async function startAgentMcpServer(): Promise<AgentMcpServerHandle> {
  ...
  const registryDir = mkdtempSync(
    path.join(os.tmpdir(), "agent-mcp-registry-")
  );
  ...
  () => {
    ...
    beforeAll(async () => {
      const stream = await import(
        "../../../../../apps/mobile/src/types/stream.js"
      );
      hydrateStreamState =
        stream.hydrateStreamState as typeof hydrateStreamState;
    });
    beforeAll(async () => {
    ...
    test("streams reasoning chunks", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
    ...
    test("emits a single assistant message in the hydrated stream", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      const config = buildConfig(cwd, { maxThinkingTokens: 2048 });
      const session = await client.createSession(config);
      ...
      try {
        const events = session.stream(
          "Reply with the exact words HELLO WORLD."
        );
        for await (const event of events) {
          await autoApprove(session, event);
    ...
    test("shows the command inside permission requests", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      const config = buildConfig(cwd, { maxThinkingTokens: 2048 });
      const session = await client.createSession(config);
    ...
    test("tracks permission + tool lifecycle when editing a file", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      const config = buildConfig(cwd, { maxThinkingTokens: 1024 });
      const session = await client.createSession(config);
      ...
      const sawPwdCommand = commandEvents.some(
        (item) =>
          (extractToolCommand(item.detail) ?? "")
            .toLowerCase()
            .includes("pwd") && item.status === "completed"
      );
    ...
    test("permission flow parity - allows command after approval", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      ...
      for await (const event of session.stream(prompt)) {
        if (event.type === "permission_requested" && !captured) {
          ...
          expect((requestedCommand ?? "").toLowerCase()).toContain(
            "permission.txt"
          );
    ...
    test("permission flow parity - denies command execution", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      const config = buildConfig(cwd, {
        maxThinkingTokens: 1024,
    ...
    test("permission flow parity - aborts on interrupt response", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      const config = buildConfig(cwd, {
        maxThinkingTokens: 1024,
    ...
    test("interrupts a long-running bash command before it finishes", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      const config = buildConfig(cwd, { maxThinkingTokens: 2048 });
    ...
    test("supports /rewind by reverting the latest file changes", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      const config = buildConfig(cwd, { maxThinkingTokens: 2048 });
      const session = await client.createSession(config);
    ...
    test("resumes a persisted session with context preserved", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      const config = buildConfig(cwd, { maxThinkingTokens: 1024 });
      const session = await client.createSession(config);
    ...
    test("updates session modes", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      const config = buildConfig(cwd, { maxThinkingTokens: 1024 });
      const session = await client.createSession(config);
    ...
    test("handles plan mode approval flow", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      const config = buildConfig(cwd, { maxThinkingTokens: 2048 });
      const session = await client.createSession(config);
    ...
    test("handles AskUserQuestion approval flow", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      const config = buildConfig(cwd, { maxThinkingTokens: 2048 });
      const session = await client.createSession(config);
      ...
      for await (const event of session.stream(prompt)) {
        if (
          ...
          const baseInput =
            typeof capturedQuestion.input === "object" &&
            capturedQuestion.input !== null
    ...
    test("hydrates persisted tool call results into the UI stream", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      ...
      try {
        ...
        const commandTool = liveSnapshots.find(
          (snapshot) =>
            snapshot.data.name.toLowerCase().includes("bash") &&
            (extractToolCommand(snapshot.data.detail) ?? "")
              .toLowerCase()
              .includes("pwd")
        );
        ...
        expect(await waitForHistoryFile(historyPaths)).toBe(true);
        expect(
          await waitForHistoryContains(historyPaths, "HYDRATION_PROOF_LINE_TWO")
        ).toBe(true);
        ...
        assertHydratedReplica(
          ...
          commandTool!,
          hydratedMap,
          (data) => rawContainsText(data.detail, cwd),
          ({ live, hydrated }) => {
            expect(rawContainsText(live.detail, cwd)).toBe(true);
            expect(rawContainsText(hydrated.detail, cwd)).toBe(true);
            expect(
              (extractToolCommand(live.detail) ?? "").toLowerCase()
            ).toContain("pwd");
            expect(
              (extractToolCommand(hydrated.detail) ?? "").toLowerCase()
            ).toContain("pwd");
          }
        );
        assertHydratedReplica(
          ...
          editTool!,
          hydratedMap,
          (data) => rawContainsText(data.detail, "hydrate-proof.txt"),
          ({ live, hydrated }) => {
    ...
    test("hydrates user messages from persisted history", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      const config = buildConfig(cwd, { maxThinkingTokens: 1024 });
      ...
      try {
        ...
        const historyPaths = getClaudeHistoryPaths(cwd, sessionId!);
        expect(await waitForHistoryFile(historyPaths)).toBe(true);
        expect(await waitForHistoryContains(historyPaths, promptMarker)).toBe(
          true
        );
        ...
        expect(stateIncludesUserMessage(hydratedState, promptMarker)).toBe(
          true
        );
      } finally {
        cleanupClaudeHistory(cwd);
    ...
    test("collapses sub-agent tool calls into Task metadata updates", async () => {
      const cwd = tmpCwd();
      const client = new ClaudeAgentClient({ logger });
      const config = buildConfig(cwd, { maxThinkingTokens: 2048 });
      const session = await client.createSession(config);
      ...
      const _subAgentLeaks = toolCalls.filter(
        (item) =>
          item.name !== "Task" && item.metadata?.subAgentActivity === undefined
      );
...
describe("convertClaudeHistoryEntry", () => {
  ...
  test("converts compact_boundary entry to compaction timeline item", () => {
    const entry = {
      ...
      compactMetadata: { trigger: "auto", preTokens: 168_428 },
    };
...
function recordTimelineUpdate(
  target: StreamHydrationUpdate[],
  event: AgentStreamEvent
) {
  if (event.type !== "timeline") {
    return;
...
function stateIncludesUserMessage(
  state: StreamItem[],
  marker: string
): boolean {
  return state.some(
    (item) =>
      ...
      item.text.toLowerCase().includes(marker.toLowerCase())
  );
}
...
function assertHydratedReplica(
  ...
  extraAssertions?: (ctx: {
    live: AgentToolCallData;
    hydrated: AgentToolCallData;
  }) => void
) {
...
async function waitForHistoryFile(
