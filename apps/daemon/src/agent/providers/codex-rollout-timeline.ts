...
function extractReasoningText(
  payload: RolloutResponseReasoningPayload
): string {
  ...
  const contentText = extractContentTextByType(
    payload.content,
    "reasoning_text"
  );
  if (contentText) {
...
const FunctionCallInputNormalizationSchema = z.union([
  ...
  z.object({ command: z.array(z.string()) }).transform((input) => ({
    name: "Bash",
    input: { command: input.command[2] ?? "" },
  })),
  z.unknown().transform((input) => ({ name: "unknown", input })),
]);

const RolloutResponseRecordSchema = z.union([
  RolloutResponseMessagePayloadSchema.transform(
    (payload): ParsedRolloutRecord => {
      const text = extractMessageText(payload.content);
      const itemType =
        payload.role === "assistant" ? "assistant_message" : "user_message";
      const shouldEmit =
        text.length > 0 &&
        (itemType !== "user_message" || !isSyntheticRolloutUserMessage(text));
      return shouldEmit
        ? { kind: "timeline", item: { type: itemType, text } }
        : { kind: "ignore" };
    }
  ),
  RolloutResponseReasoningPayloadSchema.transform(
    (payload): ParsedRolloutRecord => {
  ...
  z
    ...
    .transform((payload): ParsedRolloutRecord => {
      ...
      const rawInput =
        payload.arguments ?? ("input" in payload ? payload.input : undefined);
...
const RolloutEventRecordSchema = z.union([
  ...
  RolloutEventAgentMessagePayloadSchema.transform(
    (payload): ParsedRolloutRecord => {
  ...
  RolloutEventUserMessagePayloadSchema.transform(
    (payload): ParsedRolloutRecord => {
      const text = extractEventMessageText(payload.message);
      const shouldEmit =
        text.length > 0 && !isSyntheticRolloutUserMessage(text);
      return shouldEmit
  ...
  for (const entry of items) {
    ...
    if (messagePayloadResult.success) {
      ...
      if (payload.role === "assistant") {
        ...
        payload.role === "user" &&
        !isSyntheticRolloutUserMessage(text)
      ) {
...
export async function parseRolloutFile(
  ...
  const parsedRecords = lines
    ...
    .filter(
      (result): result is { success: true; data: ParsedRolloutRecord } =>
        result.success
    )
  ...
  const outputsByCallId = parsedRecords
    ...
    .reduce(
      (map, record) => map.set(record.callId, record.output),
      ...
      : // biome-ignore lint/style/noNestedTernary: readable inline conditional
        ...
        ? (() => {
            const mapped = mapCodexRolloutToolCall({
              ...
              output: record.callId
                ? (outputsByCallId.get(record.callId) ?? null)
                : null,
            });
