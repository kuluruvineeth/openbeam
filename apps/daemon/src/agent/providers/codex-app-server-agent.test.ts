...
function useTempCodexSessionDir(): () => void {
  ...
  return () => {
    if (prevSessionDir === undefined) {
      process.env.CODEX_SESSION_DIR = undefined;
    } else {
...
function hasShellCommand(
  ...
  const unknownInput =
    ...
    typeof item.detail.input === "object" &&
    item.detail.input
      ? (item.detail.input as {
...
function hasApplyPatchFile(item: AgentTimelineItem, fileName: string): boolean {
  ...
  if (item.detail.type === "edit") {
    return (
      item.detail.filePath === fileName ||
      (item.detail.unifiedDiff?.includes(fileName) ?? false)
    );
  }
  const unknownInput =
    item.detail.type === "unknown" &&
  ...
  const unknownOutput =
    item.detail.type === "unknown" &&
    typeof item.detail.output === "object" &&
    item.detail.output
      ? (item.detail.output as {
          path?: string;
          file_path?: string;
          filePath?: string;
      ...
      : undefined;
  const inputPath =
    unknownInput?.path ?? unknownInput?.file_path ?? unknownInput?.filePath;
  const outputPath =
    unknownOutput?.path ?? unknownOutput?.file_path ?? unknownOutput?.filePath;
  const inInput = (unknownInput?.files ?? []).some(
    (file) => file?.path === fileName
  );
  const inOutput = (unknownOutput?.files ?? []).some(
    (file) => file?.path === fileName
  );
  const inDiff =
    typeof unknownOutput?.diff === "string" &&
    unknownOutput.diff.includes(fileName);
  return (
    inInput ||
    inOutput ||
    inDiff ||
...
function buildStrictApplyPatchPrompt(
  ...
  lines.push(patch);
  lines.push(
    `After successful apply_patch completion, reply exactly ${completionToken}.`
  );
  return lines.join("\n");
}
...
describe("Codex app-server provider (integration)", () => {
  ...
  test("maps image prompt blocks to Codex localImage input", async () => {
    ...
    const localImage = input.find(
      (item) => (item as any)?.type === "localImage"
  ...
  test.runIf(isCodexInstalled())(
    "listModels returns live Codex models",
    async () => {
      const client = new CodexAppServerAgentClient(logger);
      const models = await client.listModels();
      expect(models.some((model) => model.id.includes("gpt-5.1-codex"))).toBe(
        true
      );
    },
    30_000
    ...
    async () => {
      ...
      for (const model of models) {
        ...
        if (options.length > 0) {
          ...
          expect(
            options.some((option) => option.id === defaultThinkingId)
          ).toBe(true);
        }
      }
    },
  ...
  test.runIf(isCodexInstalled())(
    ...
    async () => {
      ...
      try {
        ...
        expect(
          configuredModel?.thinkingOptions?.some(
  ...
  test.runIf(isCodexInstalled())(
    "accepts image prompt blocks without request validation errors",
    async () => {
  ...
  test.runIf(isCodexInstalled())(
    "getRuntimeInfo reflects model + mode",
    async () => {
      const cleanup = useTempCodexSessionDir();
  ...
  test.runIf(isCodexInstalled())(
    ...
    async () => {
      ...
      let session: Awaited<
        ReturnType<CodexAppServerAgentClient["createSession"]>
      > | null = null;

      try {
        const client = new CodexAppServerAgentClient(logger);
        const models = await client.listModels();
        const modelWithThinking = models.find(
          (m) => (m.thinkingOptions?.length ?? 0) > 1
        );
        if (!modelWithThinking) {
          throw new Error(
            "No Codex model with at least two non-default thinking options"
          );
        }

        const defaultThinkingId =
          modelWithThinking.defaultThinkingOptionId ?? null;
        const thinkingIds = (modelWithThinking.thinkingOptions ?? []).map(
          (opt) => opt.id
        );
        ...
        if (!(threadId && codexClient)) {
          throw new Error(
            "Codex session did not initialize app-server client/thread"
          );
        }
  ...
  test.runIf(isCodexInstalled())(
    "round-trips a stdio MCP tool call",
    async () => {
      const cleanup = useTempCodexSessionDir();
      ...
      try {
        ...
        const distinctMcpCalls = new Map<
          string,
        ...
        for (const call of toolCalls) {
          ...
          const key = String(
            call.callId ?? `${call.name}:${JSON.stringify(call.detail)}`
          );
        ...
        expect(
          toolNames.some((name) => name.toLowerCase().includes("shell"))
        ).toBe(false);

        // Hard assertion: roundtrip token must be present in the MCP tool I/O.
        const mcpDetail =
          mcpToolCall.detail.type === "unknown" ? mcpToolCall.detail : null;
        expect(JSON.stringify(mcpDetail?.input ?? {})).toContain(token);
        expect(JSON.stringify(mcpDetail?.output ?? {})).toContain(
          `ECHO:${token}`
        );
    ...
    async () => {
      ...
      try {
        ...
        try {
          ...
          expect(commands?.some((cmd) => cmd.name === "prompts:test")).toBe(
            true
          );

          const _executeArgs = "NAME=world extra_value";
  ...
  test.runIf(isCodexInstalled())(
    "command approval flow requests permission and runs command",
    async () => {
      const cleanup = useTempCodexSessionDir();
  ...
  test.runIf(isCodexInstalled())(
    "command approval deny emits failed tool call and skips execution",
    async () => {
      const cleanup = useTempCodexSessionDir();
      ...
      try {
        ...
        expect(
          timelineItems.some(
            (item) =>
              item.status === "failed" &&
              hasShellCommand(item, "permission-deny.txt")
          )
        ).toBe(true);
        expect(existsSync(filePath)).toBe(true);
      } finally {
  ...
  test.runIf(isCodexInstalled())(
    ...
    async () => {
      ...
      try {
        ...
        const shellItem = timelineItems.find((item) =>
          hasShellCommand(item, "printf")
        );
        const patchItem = timelineItems.find((item) =>
          hasApplyPatchFile(item, "patch.txt")
        );
  ...
  test.runIf(isCodexInstalled())(
    ...
    async () => {
      ...
      try {
        ...
        const completedPatchCall = patchCalls.find(
        ...
        if (completedPatchCall.detail.type === "edit") {
          const renderablePayload =
            completedPatchCall.detail.unifiedDiff ??
            completedPatchCall.detail.newString;
  ...
  test.runIf(isCodexInstalled())(
    ...
    async () => {
      ...
      try {
        ...
        const lifecycleChannelsByItemId = new Map<
          string,
          { item: boolean; codexEvent: boolean }
        >();
        ...
        rawClient?.setNotificationHandler?.(
            ...
            if (
              ...
              const record =
                ...
                "msg" in (params as Record<string, unknown>)
                  ? ((
                      params as {
                        msg?: { item?: { id?: unknown; type?: unknown } };
                      }
                    ).msg?.item ?? null)
                  : ((params as { item?: { id?: unknown; type?: unknown } })
                      ?.item ?? null);
        ...
        for await (const event of session.stream(
          "Reply with exactly: DUPLICATE_CHECK_DONE"
        )) {
          if (
            event.type === "timeline" &&
            event.item.type === "assistant_message"
        ...
        const normalizedMessages = assistantMessages
          .map((text) => text.trim())
          .filter(Boolean);
        expect(
          normalizedMessages.some((text) =>
            text.toLowerCase().includes("duplicate_check_done")
          )
        ).toBe(true);
  ...
  test.runIf(isCodexInstalled())(
    ...
    async () => {
      ...
      try {
        ...
        const commandLifecycleByCallId = new Map<
          string,
          { execEnd: boolean; itemCompleted: boolean }
        >();
        const rawClient = (session as any).client as {
          notificationHandler?: (method: string, params: unknown) => void;
          setNotificationHandler?: (
          ...
          (method: string, params: unknown) => {
            if (method === "codex/event/exec_command_end") {
              const callId =
                ...
                typeof params === "object" &&
                "msg" in (params as Record<string, unknown>) &&
                typeof (params as { msg?: { call_id?: unknown } }).msg
                  ?.call_id === "string"
                  ? ((params as { msg?: { call_id?: string } }).msg?.call_id ??
                    null)
                  : null;
              if (callId) {
        ...
        const targetCall = shellCalls.find(
          (entry) =>
            entry.status === "completed" && entry.output.includes(marker)
        );
        ...
        expect(
          statusesForCall.filter((status) => status === "completed").length
        ).toBe(1);
      } finally {
        cleanup();
        rmSync(cwd, { recursive: true, force: true });
  ...
  test.runIf(isCodexInstalled())(
    ...
    async () => {
      ...
      > | null = null;
      let resumed: Awaited<
        ReturnType<CodexAppServerAgentClient["resumeSession"]>
      > | null = null;
      ...
      try {
        ...
        const first = await session.run(
          `Remember the word ${token} and reply ACK.`
        );
      ...
      try {
        ...
        const result = await Promise.race([
          ...
          new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error("Plan run timed out")), 110_000)
          ),
        ]);
  ...
  test.runIf(isCodexInstalled())(
    "file change approval flow requests permission and applies change",
    async () => {
      const cleanup = useTempCodexSessionDir();
      ...
      try {
          ...
          await Promise.race([
            (async () => {
              for await (const event of events) {
                if (
                  event.type === "permission_requested" &&
                  event.request.name === "CodexFileChange"
                ) {
                  _sawPermission = true;
                  captured = event.request;
                  await session.respondToPermission(event.request.id, {
        ...
        const sawPatch = timelineItems.some((item) =>
          hasApplyPatchFile(item, "approval-test.txt")
        );
        if (!sawPatch) {
          const toolCalls = timelineItems
            .filter(
        ...
        const text = await waitForFileToContainText(targetPath, "ok", {
          timeoutMs: 10_000,
        });
        if (!text) {
  ...
  test.runIf(isCodexInstalled())(
    "tool approval flow requests user input for app tools",
    async () => {
      const cleanup = useTempCodexSessionDir();
      ...
      try {
        ...
        const appsResult = await rawClient.request("app/list", {
          cursor: null,
          limit: 10,
        });
        ...
        const input = [
          {
            type: "text",
            text: `$${app.id} Perform a minimal action and wait for approval if required. Reply with TOOL_DONE.`,
          },
          {
            type: "mention",
