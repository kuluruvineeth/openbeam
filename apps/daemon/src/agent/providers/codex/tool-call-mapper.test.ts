...
describe("codex tool-call mapper", () => {
  ...
  it("truncates large diff payloads deterministically in canonical detail", () => {
    ...
    const item = mapCodexToolCallFromThreadItem(
      {
        ...
        status: "completed",
        changes: [
          { path: "/tmp/repo/src/index.ts", kind: "modify", diff: hugeDiff },
        ],
      },
      { cwd: "/tmp/repo" }
    );
    ...
    if (item?.detail?.type === "edit") {
      ...
      expect((item.detail.unifiedDiff?.length ?? 0) < hugeDiff.length).toBe(
        true
      );
    }
  });
    ...
    const editItem = mapCodexToolCallFromThreadItem(
      {
        ...
        tool: "apply_patch",
        arguments: {
          file_path: "/tmp/repo/src/index.ts",
          patch: "@@\\n-a\\n+b\\n",
        },
        result: null,
      },
    ...
    const item = mapCodexToolCallFromThreadItem(
      {
        ...
        changes: [
          {
            path: "/tmp/repo/src/from-file-change.ts",
            kind: "modify",
            content: patch,
          },
        ],
  ...
  it("maps path-only apply_patch rollout payloads to unknown detail instead of empty edit detail", () => {
    const item = expectMapped(
      mapCodexRolloutToolCall({
        ...
        output: {
          files: [
            { path: "/tmp/repo/src/path-only-rollout.ts", kind: "modify" },
          ],
        },
        cwd: "/tmp/repo",
      })
    ...
    if (item.detail.type === "unknown") {
      expect(item.detail.input).toEqual({
        path: "/tmp/repo/src/path-only-rollout.ts",
      });
    }
  });
