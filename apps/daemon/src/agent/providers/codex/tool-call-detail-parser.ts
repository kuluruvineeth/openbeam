import { z } from "zod";
import { stripCwdPrefix } from "../../../shared/path-utils";
...
const CodexToolDetailPass2Schema = z.union([
  ...
  toolDetailBranchByNameWithCwd(
    ...
    (input, output, cwd) => {
      ...
      return toReadToolDetail(
        input,
        parsedOutput.success ? parsedOutput.data : null,
        {
  ...
  toolDetailBranchByNameWithCwd(
    ...
    ToolWriteOutputSchema,
    (input, output, cwd) =>
      toWriteToolDetail(input, output, {
        normalizePath: normalizePathForCwd(cwd),
      })
  ),
  toolDetailBranchByNameWithCwd(
  ...
  toolDetailBranchByNameWithCwd(
    ...
    ToolSearchInputSchema,
    z.unknown(),
    (input) => toSearchToolDetail(input)
  ),
  toolDetailBranchByNameWithCwd(
    "web_search",
    ToolSearchInputSchema,
