...
type CodexNormalizedToolCallPass2 = z.infer<
  typeof CodexNormalizedToolCallPass2Schema
>;

function toToolCallTimelineItem(
  envelope: CodexNormalizedToolCallPass2
): ToolCallTimelineItem {
  const name =
    envelope.toolKind === "speak" ? ("speak" as const) : envelope.name;
...
function asEditTextFields(text: string | undefined): {
  unifiedDiff?: string;
  newString?: string;
} {
...
function asEditFileOutputFields(text: string | undefined): {
  patch?: string;
  content?: string;
} {
  if (typeof text !== "string" || text.length === 0) {
...
function hasRenderableEditDetail(
  ...
  return (
    (typeof detail.unifiedDiff === "string" &&
      detail.unifiedDiff.trim().length > 0) ||
    (typeof detail.newString === "string" &&
      detail.newString.trim().length > 0) ||
...
function toToolCallFromNormalizedEnvelope(
  ...
  const parsed = CodexNormalizedToolCallPass2Schema.safeParse(
    pass2Envelope.data
  );
  if (!parsed.success) {
    return null;
...
function mapThreadItemToNormalizedEnvelope(
  ...
  switch (item.type) {
    ...
    default: {
      ...
      throw new Error(
        `Unhandled Codex thread item type: ${String(exhaustiveCheck)}`
      );
    }
  }
}
