...
function looksLikeCodexShellEnvelope(lines: string[]): boolean {
  ...
  const hasWallTime = headerWindow.some((line) =>
    line.startsWith("wall time:")
  );
  const hasExitCode = headerWindow.some((line) =>
    line.startsWith("process exited with code")
  );
  return hasWallTime && hasExitCode;
}

export function extractCodexShellOutput(
...
export function coerceToolCallId(params: {
  ...
  if (
    typeof params.rawCallId === "string" &&
    params.rawCallId.trim().length > 0
  ) {
