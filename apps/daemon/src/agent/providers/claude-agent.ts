...
function normalizeModelIdCandidate(
...
function pickSupportedModelId(
  ...
  return supportedModelIds.has(normalizedCandidate)
    ? normalizedCandidate
    : null;
}
...
const CLAUDE_MODEL_LIST_TIMEOUT_MS = 6000;
const CLAUDE_MODEL_PRIME_TIMEOUT_MS = 4000;

async function fetchSupportedModelsWithTimeout(
  claudeQuery: Query,
  timeoutMs: number,
  contextLabel: string
): Promise<ModelInfo[]> {
  ...
  try {
    ...
    return await Promise.race([claudeQuery.supportedModels(), timeoutPromise]);
  } finally {
    if (timeoutHandle) {
...
function isPermissionUpdate(
  value: AgentPermissionUpdate
): value is PermissionUpdate {
  if (!isMetadata(value)) {
    return false;
  }
  const type = value.type;
...
export class ClaudeAgentClient implements AgentClient {
  ...
  constructor(options: ClaudeAgentClientOptions) {
    ...
    try {
      this.claudePath =
        execSync("which claude", { encoding: "utf8" }).trim() || null;
    } catch {
  ...
  private applyRuntimeSettings(options: ClaudeOptions): ClaudeOptions {
    const hasEnvOverrides =
      Object.keys(this.runtimeSettings?.env ?? {}).length > 0;
  ...
  async listModels(
    ...
    try {
      const models: ModelInfo[] = await fetchSupportedModelsWithTimeout(
        claudeQuery,
      ...
      if (
        ...
        error.message.startsWith("Claude model discovery timed out after")
      ) {
        this.logger.warn(
          { err: error },
          "Claude model discovery timed out; falling back to static model aliases"
        );
  ...
  async listPersistedAgents(
    ...
    for (const candidate of candidates) {
      const descriptor = await parseClaudeSessionDescriptor(
        candidate.path,
        candidate.mtime
      );
      if (descriptor) {
  ...
  private assertConfig(config: AgentSessionConfig): ClaudeAgentConfig {
    if (config.provider !== "claude") {
      throw new Error(
        `ClaudeAgentClient received config for provider '${config.provider}'`
      );
    }
...
class ClaudeAgentSession implements AgentSession {
  ...
  private readonly toolUseCache = new Map<string, ToolUseCacheEntry>();
  ...
  private readonly activeSidechains = new Map<string, string>();
  private compacting = false;
  private queryRestartNeeded = false;
  private readonly userMessageIds: string[] = [];
  ...
  async *stream(
    ...
    const requestCancel = () => {
      ...
      this.pendingInterruptPromise = this.interruptActiveTurn().catch(
        (error) => {
    ...
    forwardPromise.catch((error) => {
      this.logger.error(
        { err: error },
        "Unexpected error in forwardPromptEvents"
      );
    });
  ...
  async setThinkingOption(thinkingOptionId: string | null): Promise<void> {
    ...
    if (
      ...
      normalizedThinkingOptionId === "default"
    ) {
  ...
  getPendingPermissions(): AgentPermissionRequest[] {
    return Array.from(this.pendingPermissions.values()).map(
      (entry) => entry.request
    );
  }
  ...
  async respondToPermission(
    ...
    if (response.behavior === "allow") {
      ...
      const result: PermissionResult = {
        ...
        updatedPermissions: this.normalizePermissionUpdates(
  ...
  private rememberUserMessageId(messageId: string | null | undefined): void {
    ...
    const last = this.userMessageIds.at(-1);
    if (last === messageId) {
      return;
    }
  ...
  private async primeSelectableModelIds(query: Query): Promise<void> {
    try {
      const models = await fetchSupportedModelsWithTimeout(
        query,
        CLAUDE_MODEL_PRIME_TIMEOUT_MS,
        "Claude selectable model priming"
      );
      const ids = models
      ...
      this.logger.warn(
        { err: error },
        "Failed to prime Claude selectable model IDs"
      );
    }
  }
  ...
  private handleSidechainMessage(
    ...
    if (message.type === "assistant") {
      ...
      if (event.type === "content_block_start") {
        const cb = isClaudeContentChunk(event.content_block)
          ? event.content_block
          : null;
  ...
  private readonly handlePermissionRequest: CanUseTool = async (
    toolName,
    input,
    options
  ...
  private buildToolOutput(
    ...
    const structured = this.buildStructuredToolResult(
      server,
      tool,
      content,
      input
  ...
  private buildStructuredToolResult(
    ...
    if (
      ...
      (input &&
        (typeof input.command === "string" || Array.isArray(input.command)))
    ) {
    ...
    switch (event.type) {
      ...
      case "content_block_delta":
        return isClaudeContentChunk(event.delta)
          ? this.mapBlocksToTimeline([event.delta], { turnContext })
          : [];
      default:
        return [];
    }
  ...
  private upsertToolUseEntry(
    block: ClaudeContentChunk
  ): ToolUseCacheEntry | null {
...
export function convertClaudeHistoryEntry(
    ...
    return [
      {
        ...
        status: "completed",
        trigger:
          entry.compactMetadata?.trigger === "manual" ? "manual" : "auto",
  ...
  const hasToolBlock =
    normalizedBlocks?.some((block) => hasToolLikeBlock(block)) ?? false;
  const timeline: AgentTimelineItem[] = [];
...
function extractClaudeUserText(message: ClaudeHistoryMessage): string | null {
  if (!message) {
    return null;
  }
