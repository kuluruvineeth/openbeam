...
import { parseRolloutFile } from "./codex-rollout-timeline";
      ...
      const lines = [
        JSON.stringify({
          ...
          payload: {
            ...
            name: "shell",
            arguments:
              '{"command":["bash","-lc","ls -la"],"workdir":"/Users/test/project"}',
  ...
  describe("real rollout file structure", () => {
    ...
    test("parses custom_tool_call apply_patch with input/output into editable tool detail", async () => {
      ...
      if (patchCall.detail.type === "edit") {
        ...
        expect(patchCall.detail.unifiedDiff).toContain(
          "+export const value = 1;"
        );
      ...
      const lines = [
        ...
        JSON.stringify({
          ...
          payload: {
            type: "reasoning",
            content: [
              { type: "reasoning_text", text: "I need to run the tests." },
            ],
          },
        }),
        // Command
