...
const MCP_ALREADY_PRESENT_ERROR_TOKENS = [
  "already",
  "exists",
  "connected",
] as const;
...
const OpencodeToolPartTimelineEnvelopeSchema = OpencodeToolPartSchema.transform(
  (part) => ({
...
const OpencodeToolPartToTimelineItemSchema =
...
export class OpenCodeServerManager {
  ...
  static getInstance(
    ...
    if (!OpenCodeServerManager.instance) {
      ...
      OpenCodeServerManager.instance.runtimeSettingsKey !== nextSettingsKey
    ) {
      logger.warn(
        {
          existingRuntimeSettings:
            OpenCodeServerManager.instance.runtimeSettingsKey,
    ...
    return new Promise((resolve, reject) => {
      this.server = spawn(
        ...
        [...launchPrefix.args, "serve", "--port", String(this.port)],
        {
          ...
          env: buildChildProcessEnv("opencode", this.runtimeSettings),
        }
      );
...
export class OpenCodeAgentClient implements AgentClient {
  ...
  async createSession(config: AgentSessionConfig): Promise<AgentSession> {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(
        () => reject(new Error("OpenCode session.create timed out after 10s")),
        10_000
      );
    });
    ...
    if (response.error) {
      throw new Error(
        `Failed to create OpenCode session: ${JSON.stringify(response.error)}`
      );
    }
  ...
  async resumeSession(
    ...
    if (!cwd) {
      throw new Error(
        "OpenCode resume requires the original working directory"
      );
    }
    ...
    for (const provider of providers.all) {
      ...
      for (const [modelId, model] of Object.entries(provider.models)) {
        ...
        models.push({
          ...
          thinkingOptions:
            thinkingOptions.length > 1 ? thinkingOptions : undefined,
  ...
  async listPersistedAgents(
    ...
    let url: string;
    try {
      ({ url } = await this.serverManager.ensureRunning());
    } catch {
      return [];
    }
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(
        () => reject(new Error("OpenCode session.list timed out after 10s")),
        10_000
      );
    });
    ...
    try {
      ...
      return response.data
        ...
        .map((session) => ({
          ...
          title: session.title || `OpenCode session ${session.id.slice(0, 8)}`,
  ...
  private assertConfig(config: AgentSessionConfig): OpenCodeAgentConfig {
    if (config.provider !== "opencode") {
      throw new Error(
        `OpenCodeAgentClient received config for provider '${config.provider}'`
      );
    }
      ...
      if (role === "user") {
        ...
        for (const part of parts) {
          ...
          if (partType === "text") {
            ...
            const parsedToolPart =
              OpencodeToolPartToTimelineItemSchema.safeParse(part);
  ...
  private translateEvent(event: unknown): AgentStreamEvent[] {
    ...
    switch (type) {
      ...
      case "message.part.updated": {
        ...
        const messageRole = messageId
          ? this.messageRoles.get(messageId)
          : undefined;
        ...
        if (partType === "text") {
          ...
          if (tokens) {
            this.accumulatedUsage.inputTokens =
              (this.accumulatedUsage.inputTokens ?? 0) + (tokens.input ?? 0);
            this.accumulatedUsage.outputTokens =
              (this.accumulatedUsage.outputTokens ?? 0) + (tokens.output ?? 0);
          }
          if (cost !== undefined) {
            this.accumulatedUsage.totalCostUsd =
              (this.accumulatedUsage.totalCostUsd ?? 0) + cost;
          }
        }
        break;
      }
