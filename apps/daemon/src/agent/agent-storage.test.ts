...
function createManagedAgent(
  ...
  const session =
    lifecycle === "closed" ? null : (overrides.session ?? ({} as AgentSession));
  const pendingRun =
  ...
  const agent: ManagedAgent = {
    ...
    pendingPermissions:
      overrides.pendingPermissions ?? new Map<string, AgentPermissionRequest>(),
    pendingRun,
...
describe("AgentStorage", () => {
  ...
  test("applySnapshot persists configs and snapshot metadata", async () => {
    ...
    expect(persisted.config?.extra?.claude).toMatchObject({
  ...
  test("setTitle throws when the agent record does not exist", async () => {
    await expect(
      storage.setTitle("missing-agent", "Impossible")
  ...
  test("remove deletes all duplicate record files across project directories", async () => {
    ...
    // biome-ignore lint/style/noDoneCallback: necessary for this context
