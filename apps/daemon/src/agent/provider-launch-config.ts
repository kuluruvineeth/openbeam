...
const ANTHROPIC_ENV_KEYS = ["ANTHROPIC_API_KEY", "ANTHROPIC_BASE_URL"] as const;

const PROVIDER_ENV_BLOCKLIST: Record<AgentProvider, readonly string[]> = {
  codex: [...OPENAI_ENV_KEYS, ...ANTHROPIC_ENV_KEYS],
  claude: [...OPENAI_ENV_KEYS],
  opencode: [],
};
...
export function buildChildProcessEnv(
  ...
  const blocked = PROVIDER_ENV_BLOCKLIST[provider] ?? [];
  for (const key of blocked) {
    delete env[key];
  }
  return applyProviderEnv(env, runtimeSettings);
}
...
export function isCommandAvailable(command: string): boolean {
  ...
  try {
    const output = execFileSync("which", [normalized], {
      encoding: "utf8",
    }).trim();
...
export function isProviderCommandAvailable(
  ...
  try {
    const prefix = resolveProviderCommandPrefix(
      commandConfig,
