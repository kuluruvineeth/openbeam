...
const AGENT_WAIT_TIMEOUT_MS = 50_000;
...
function sanitizePermissionRequest(
  ...
  if (sanitized.input === undefined) {
    sanitized.input = undefined;
  }
  if (sanitized.suggestions === undefined) {
    sanitized.suggestions = undefined;
  }
...
export async function createAgentMcpServer(
  ...
  const shouldRegisterSpeak =
    options.voiceOnly ||
    options.enableVoiceTools ||
    callerContext?.enableVoiceTools;
  if (shouldRegisterSpeak) {
    childLogger.info({ callerAgentId }, "Registering speak tool on MCP server");
    server.registerTool(
      "speak",
      {
        title: "Speak",
      ...
      async (args, context?: McpToolContext) => {
        if (!callerAgentId) {
          throw new Error(
            "speak is only available to agent-scoped MCP sessions"
          );
        }
        const handler = resolveSpeakHandler?.(callerAgentId) ?? null;
        if (!handler) {
          throw new Error(
            `No speak handler registered for caller agent '${callerAgentId}'`
          );
        }
        childLogger.info(
          { callerAgentId, textLength: args.text.length },
          "Speak tool invoked"
        );
        await handler({
          text: args.text,
    ...
    async (args: unknown) => {
      ...
      const snapshot = await agentManager.createAgent(
        ...
        childAgentDefaultLabels
          ? { labels: childAgentDefaultLabels }
          : undefined
      );
