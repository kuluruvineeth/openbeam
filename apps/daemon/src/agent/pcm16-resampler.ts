export class Pcm16MonoResampler {
  ...
  public processChunk(pcm16le: Buffer): Buffer {
    ...
    if (srcLen < 2) {
      this.carrySample = srcChunk.length ? srcChunk.at(-1) : this.carrySample;
      return Buffer.alloc(0);
    }
    ...
    let offset = 0;
    if (hasCarry) {
      src[0] = (this.carrySample as number) / 32_768;
      offset = 1;
    }
    for (let i = 0; i < srcChunk.length; i += 1) {
      src[offset + i] = srcChunk[i] / 32_768;
    }
