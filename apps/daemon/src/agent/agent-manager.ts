...
export class AgentManager {
  ...
  fetchTimeline(
    ...
    const agent = this.requireAgent(id);
    const { rows, epoch, nextSeq, minSeq, maxSeq } =
    ...
    const cloneRows = (items: AgentTimelineRow[]) =>
      items.map((row) => ({ ...row }));
    ...
    if (direction === "after") {
      ...
      const selected = selectAll
        ? rows.slice(startIdx)
        : rows.slice(startIdx, startIdx + limit);
  ...
  async runAgent(
    ...
    const agent = this.requireAgent(agentId);
    const sessionId = agent.persistence?.sessionId ?? null;
    return {
      sessionId,
      finalText,
      usage,
        ...
        (event) => {
          if (event.type === "agent_state") {
            ...
            if (event.agent.lifecycle === "error") {
              finishErr(
                new Error(
                  event.agent.lastError ?? `Agent ${agentId} failed to start`
                )
              );
              return;
            }
            if ("pendingRun" in event.agent && !event.agent.pendingRun) {
              finishErr(
                new Error(`Agent ${agentId} run finished before starting`)
              );
              return;
            }
            return;
  ...
  private async registerSession(
    ...
    const derivedNextSeq =
      ...
      (initialTimelineRows.length ? initialTimelineRows.at(-1)?.seq + 1 : 1);
    ...
    if (normalized.cwd) {
      ...
      try {
        ...
        if (!cwdStats.isDirectory()) {
          throw new Error(
            `Working directory is not a directory: ${normalized.cwd}`
          );
        }
      } catch (error) {
        if (
          ...
          throw new Error(
            `Working directory does not exist: ${normalized.cwd}`
          );
        }
        ...
        throw new Error(
          `Failed to access working directory: ${normalized.cwd}`
        );
      }
    }
