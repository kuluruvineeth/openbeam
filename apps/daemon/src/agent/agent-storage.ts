...
const SERIALIZABLE_CONFIG_SCHEMA = z
  .object({
    ...
    mcpServers: z.record(z.unknown()).nullable().optional(),
  })
  .nullable()
  .optional();
...
export class AgentStorage {
  private readonly cache: Map<string, StoredAgentRecord> = new Map();
  private readonly pathById: Map<string, string> = new Map();
  private readonly pendingWrites: Map<string, Promise<void>> = new Map();
  private readonly deleting: Set<string> = new Set();
  ...
  async applySnapshot(
    ...
    const hasTitleOverride =
      options !== undefined && Object.hasOwn(options, "title");
    const hasInternalOverride =
      options !== undefined && Object.hasOwn(options, "internal");
