...
function encodePrintableKey(input: TerminalKeyInput): string {
  ...
  if (input.ctrl) {
    ...
    if (upper.length === 1 && upper >= "A" && upper <= "Z") {
      return applyAltLikePrefix(
        String.fromCharCode(upper.charCodeAt(0) - 64),
        input
      );
    }
    switch (char) {
...
export function encodeTerminalKeyInput(input: TerminalKeyInput): string {
  ...
  switch (key) {
    ...
    case "F1":
      return modifierParam(input) === 1
        ? "\x1bOP"
        : csiWithModifier("P", input);
    case "F2":
      return modifierParam(input) === 1
        ? "\x1bOQ"
        : csiWithModifier("Q", input);
    case "F3":
      return modifierParam(input) === 1
        ? "\x1bOR"
        : csiWithModifier("R", input);
    case "F4":
      return modifierParam(input) === 1
        ? "\x1bOS"
        : csiWithModifier("S", input);
    case "F5":
