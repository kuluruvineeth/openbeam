...
function hasActiveClientOnAgent(
  ...
  return allClientStates.some(
    (state) =>
      state.focusedAgentId === agentId && state.appVisible && !state.isStale
  );
}
...
function hasOtherCompetingClient(
  ...
  return allClientStates.some(
    (state) =>
      state !== clientState &&
      (state.deviceType === "mobile" || state.deviceType === null)
  );
}

function hasActiveForegroundMobileClient(
  allClientStates: ClientAttentionState[]
): boolean {
  return allClientStates.some(
    (state) =>
      state.deviceType === "mobile" && state.appVisible && !state.isStale
  );
}
...
export function computeShouldNotifyClient({
  ...
  if (
    !clientState.isStale &&
    clientState.appVisible &&
    clientState.focusedAgentId !== null
  ) {
    return true;
  }
...
export function computeShouldSendPush({
  ...
  return !(
    hasActiveWebClient(allClientStates) ||
    hasActiveForegroundMobileClient(allClientStates)
  );
}
