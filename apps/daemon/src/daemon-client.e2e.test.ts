...
function hasSherpaKittenModels(modelsDir: string): boolean {
  return (
    existsSync(
      path.join(modelsDir, "kitten-nano-en-v0_1-fp16", "model.fp16.onnx")
    ) &&
    existsSync(
      path.join(modelsDir, "kitten-nano-en-v0_1-fp16", "voices.bin")
    ) &&
...
const hasLocalSpeech =
  hasSherpaZipformerModels(localModelsDir) &&
  hasSherpaKittenModels(localModelsDir);
...
describe("daemon client E2E", () => {
  ...
  test("handles session actions", async () => {
    ...
    await expect(
      ctx.client.setVoiceMode(true, created.id)
  ...
  test("archives agents and excludes them from default listings", async () => {
    ...
    try {
      ...
      expect(
        active.entries.some((entry) => entry.agent.id === created.id)
      ).toBe(false);
      ...
      expect(
        withArchived.entries.some((entry) => entry.agent.id === created.id)
      ).toBe(true);
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  ...
  test("emits disabled voice capability reasons on fresh daemon startup", async () => {
    ...
    try {
      ...
      expect(info.dictationReason).toBe(
        "Dictation is disabled in daemon config."
      );
      expect(info.voiceEnabled).toBe(false);
      expect(info.voiceReason).toBe(
        "Realtime voice is disabled in daemon config."
      );
    } finally {
  ...
  test("creates agent and exercises lifecycle", async () => {
    ...
    const agentUpdatePromise = waitForSignal(15_000, (resolve) => {
    ...
    const createdStatusPromise = waitForSignal(15_000, (resolve) => {
    ...
    expect((createdStatus.payload as { agentId?: string }).agentId).toBe(
      agent.id
    );

    const failRequestId = `fail-${Date.now()}`;
    const failedStatusPromise = waitForSignal(15_000, (resolve) => {
      const unsubscribe = ctx.client.on("status", (message) => {
        if (message.type !== "status") {
          return;
        }
    ...
    const statusPromise = waitForSignal(15_000, (resolve) => {
    ...
    const modelsPromise = waitForSignal(30_000, (resolve) => {
    ...
    const commandsResponsePromise = waitForSignal(15_000, (resolve) => {
    ...
    const agentDeletedPromise = waitForSignal(15_000, (resolve) => {
  ...
  test("handles permission flow", async () => {
    ...
    const permissionRequestPromise = waitForSignal(60_000, (resolve) => {
    ...
    const permissionResolvedPromise = waitForSignal(60_000, (resolve) => {
      const unsubscribe = ctx.client.on(
  ...
  speechTest(
    ...
    async () => {
      ...
      const transcriptSeen = waitForSignal(60_000, (resolve) => {
      ...
      const wav = await import("node:fs/promises").then((fs) =>
  ...
  speechTest(
    ...
    async () => {
      ...
      try {
        ...
        const firstChunk = pcm16.subarray(
          0,
          Math.min(chunkBytes, pcm16.length)
        );
        await ctx.client.sendVoiceAudioChunk(
          firstChunk.toString("base64"),
          format,
          false
        );
        await earlyTranscription
          .then(() => {
            throw new Error(
              "Expected no transcription_result before isLast=true"
            );
          })
          // biome-ignore lint/suspicious/noEmptyBlockStatements: intentional no-op
          .catch(() => {});
        ...
        for (
          ...
          const chunk = pcm16.subarray(
            offset,
            Math.min(pcm16.length, offset + chunkBytes)
          );
  ...
  speechTest(
    ...
    async () => {
      ...
      const result = await ctx.client.finishDictationStream(
        dictationId,
        finalSeq
      );
  ...
  speechTest(
    ...
    async () => {
      ...
      // biome-ignore lint/suspicious/noMisplacedAssertion: assertion in test helper
      await expect(
        ctx.client.finishDictationStream(dictationId, 0)
        // biome-ignore lint/performance/useTopLevelRegex: scoped regex acceptable here
      ).rejects.toThrow(/no audio chunks were received/i);
    },
    15_000
  );
  ...
  test("supports git and file operations", async () => {
    ...
    const diffResult = await ctx.client.getCheckoutDiff(cwd, {
      mode: "uncommitted",
    });
    ...
    expect(diffResult.files.some((file) => file.path === "test.txt")).toBe(
      true
    );

    const listRequestId = `list-${Date.now()}`;
    const listMessagePromise = waitForSignal(15_000, (resolve) => {
    ...
    const fileMessagePromise = waitForSignal(15_000, (resolve) => {
    ...
    const tokenMessagePromise = waitForSignal(15_000, (resolve) => {
