...
const DEFAULT_DICTATION_FINAL_TIMEOUT_MS = 10_000;
...
export type DictationStreamOutboundMessage =
  | {
      ...
      payload: { dictationId: string; ackSeq: number };
    }
  | {
...
export class DictationStreamManager {
  ...
  constructor(params: {
    ...
    this.logger = params.logger.child({
      component: "dictation-stream-manager",
    });
    this.emit = params.emit;
    this.sessionId = params.sessionId;
    this.resolveStt = toResolver(params.stt);
    this.finalTimeoutMs =
      params.finalTimeoutMs ?? DEFAULT_DICTATION_FINAL_TIMEOUT_MS;
  ...
  public async handleStart(dictationId: string, format: string): Promise<void> {
    ...
    const inputRate = parsePcmRateFromFormat(format, 16_000) ?? 16_000;
  ...
  public async handleChunk(params: {
    ...
    if (!state.receivedChunks.has(params.seq)) {
      state.receivedChunks.set(
        params.seq,
        Buffer.from(params.audioBase64, "base64")
      );
    }
    ...
    while (state.receivedChunks.has(state.nextSeqToForward)) {
      ...
      const resampled = state.resampler
        ? state.resampler.processChunk(pcm16)
        : pcm16;
      if (resampled.length > 0) {
        ...
        if (state.debugChunkWriter) {
          // biome-ignore lint/complexity/noVoid: fire-and-forget async call
          void state.debugChunkWriter
            .writeChunk(seq, resampled)
            .catch((err) => {
              this.logger.warn(
  ...
  private async failAndCleanupDictationStream(
    ...
    const debugRecordingPath =
      await this.maybePersistDictationStreamAudio(dictationId);
    this.emit({
      type: "dictation_stream_error",
      payload: {
        dictationId,
  ...
  private estimateFinalizationTimeout(state: DictationStreamState): {
    ...
    const pendingCommittedSegments = state.committedSegmentIds.reduce(
      (count, segmentId) =>
        state.finalTranscriptSegmentIds.has(segmentId) ? count : count + 1,
      0
    );
    ...
    const extraMs =
      ...
      pendingAudioSeconds *
        DICTATION_FINAL_TIMEOUT_PER_PENDING_AUDIO_SECOND_MS +
