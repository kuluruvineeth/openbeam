...
program
  ...
  .action(async (title, opts) => {
    ...
    const task = await store.create(title, {
      ...
      priority: opts.priority ? Number.parseInt(opts.priority, 10) : undefined,
    });
  ...
  .action(async (id) => {
    ...
    if (ancestors.length > 0) {
      ...
      for (const ancestor of ancestors.reverse()) {
        process.stdout.write(
          `## ${ancestor.title} (${ancestor.id}) [${ancestor.status}]\n`
        );
        if (ancestor.body) {
          process.stdout.write(`\n${ancestor.body}\n`);
        }
...
program
  ...
  .option(
    "-s, --status <status>",
    "Set status (draft, open, in_progress, done, failed)"
  )
  .option(
    "--clear-acceptance",
    "Clear all acceptance criteria (combine with -a to replace)"
  )
  .option(
    "-a, --accept <criterion>",
  ...
  .action(async (id, opts) => {
    ...
    if (opts.priority !== undefined) {
      changes.priority = Number.parseInt(opts.priority, 10);
    }
...
program
  ...
  .action(async (id) => {
    ...
    for (const child of children) {
      ...
      const priority =
        child.priority !== undefined ? ` !${child.priority}` : "";
      process.stdout.write(
...
program
  ...
  .description(
    "Add a timestamped note (timestamp is automatic, don't include one)"
  )
...
function parseJudgeVerdict(output: string): "DONE" | "NOT_DONE" | null {
  ...
  return matches.at(-1)[1] as "DONE" | "NOT_DONE";
}
...
program
  ...
  .option(
    "--max-iterations <n>",
    "Max worker/judge iterations per task (0 = no limit)",
    "0"
  )
  ...
  .action(async (scopeId: string | undefined, opts) => {
    ...
    process.stdout.write(
      `Planner: ${enablePlanner ? plannerModel : "disabled"}\n`
    );
    ...
    const runPlanner = async (task: Task, reason: string): Promise<boolean> => {
      ...
      if (children.some((c) => c.status !== "done")) {
        log(
          logFile,
          "[PLANNER] Task has pending children, will process those first"
        );
    ...
    const runTaskLoop = async (): Promise<void> => {
      ...
      const candidates = scopeId
        ? ([
            await store.get(scopeId),
            ...(await store.getDescendants(scopeId)),
      ...
      while (true) {
        ...
        if (!workerModel && task.assignee) {
          workerModel =
            task.assignee === "codex" ? "gpt-5.2-codex" : task.assignee;
        }
        ...
        let lastSeenSteerCount =
          scopeTaskForSteer?.notes.filter((n) => n.content.startsWith("STEER:"))
            .length ?? 0;
        ...
        while (
          ...
          log(
            logFile,
            `[WORKER] Iteration ${iterLabel} with ${workerModel}...`
          );
          ...
          if (enablePlanner && scopeId) {
            ...
            if (freshScope) {
              ...
              if (currentSteerCount > lastSeenSteerCount) {
                ...
                log(
                  logFile,
                  "[STEER] New steering note detected - triggering planner"
                );
          ...
          const judgeResult = runAgentWithModel(
            judgePrompt,
            judgeModel,
            logFile
          );
        ...
        if (!taskDone && maxIterations > 0) {
          ...
          if (finalTask && finalTask.status === "in_progress") {
            log(
              logFile,
              `⚠️  Task ${task.id} not completed after ${maxIterations} iterations`
            );
