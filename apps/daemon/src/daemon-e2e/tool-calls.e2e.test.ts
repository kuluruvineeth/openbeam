...
function commandFromRawInput(value: unknown): string | null {
  ...
  if (Array.isArray(commandValue)) {
    const parts = commandValue.filter(
...
function commandFromToolCallDetail(
  detail: ToolCallItem["detail"]
): string | null {
...
function pathFromToolCallDetail(detail: ToolCallItem["detail"]): string | null {
  ...
  if (detail.type === "unknown") {
    return (
      extractPathFromPayload(detail.input) ??
      extractPathFromPayload(detail.output)
    );
  }
  return null;
}
...
describe("daemon E2E", () => {
  ...
  describe("tool call structure", () => {
    ...
    test("Claude agent: Read tool", async () => {
      ...
      const readPath = readCall
        ? pathFromToolCallDetail(readCall.detail)
        : null;
    ...
    test("Claude agent: Bash tool", async () => {
      ...
      const command = bashCall
        ? commandFromToolCallDetail(bashCall.detail)
        : null;
    ...
    test("Claude agent: Edit tool", async () => {
      ...
      const editPath = editCall
        ? pathFromToolCallDetail(editCall.detail)
        : null;
      if (editPath) {
