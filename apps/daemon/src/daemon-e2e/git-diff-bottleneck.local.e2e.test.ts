...
function seedLargeDirtyRepo(cwd: string, fileCount: number): void {
  ...
  writeFileSync(
    ...
    Buffer.from([0x00, 0xff, 0x10, 0x80, 0x00, 0x7f])
  );
}
...
runDescribe("daemon E2E git diff bottleneck profiling", () => {
  test("shows per-file git diff subprocess fanout and timeout pressure", async () => {
    ...
    try {
      ...
      const cliDiff = execSync("git diff HEAD", {
        cwd,
        stdio: "pipe",
      }).toString();
      ...
      try {
        ...
        expect(checkoutPayload.files.length).toBeGreaterThanOrEqual(
          LARGE_CHANGESET_SIZE
        );

        const binaryEntry = checkoutPayload.files.find(
