...
describe("daemon restart resume", () => {
  ...
  test("Codex agent survives daemon restart with persistence handle", async () => {
    ...
    try {
      ...
      expect(afterRemember.final?.persistence?.metadata).toMatchObject({
        marker,
      });
      ...
      const afterRecall = await ctx.client.waitForFinish(resumed.id, 5000);
      expect(afterRecall.status).toBe("idle");
      expect(afterRecall.final?.persistence).toBeTruthy();
      expect(afterRecall.final?.persistence?.metadata).toMatchObject({
        marker,
      });
