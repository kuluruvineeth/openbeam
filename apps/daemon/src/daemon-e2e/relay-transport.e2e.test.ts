...
function decodeOfferFromFragmentUrl(url: string): {
  ...
  const offer = JSON.parse(json) as {
    v?: unknown;
    serverId?: string;
    daemonPublicKeyB64?: string;
  };
  if (offer.v !== 2) {
    throw new Error("expected offer.v=2");
  }
  if (!offer.serverId) {
    throw new Error("offer.serverId missing");
  }
  if (!offer.daemonPublicKeyB64) {
    throw new Error("offer.daemonPublicKeyB64 missing");
  }
  return {
...
(shouldRunRelayE2e ? describe : describe.skip)(
  "Relay transport (E2EE) - daemon E2E",
  () => {
    ...
    test("daemon connects to relay and client ping/pong works through relay", async () => {
      ...
      try {
        ...
        const { serverId, daemonPublicKeyB64 } =
          decodeOfferFromFragmentUrl(offerUrl);
