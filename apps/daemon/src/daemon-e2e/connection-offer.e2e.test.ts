...
describe("ConnectionOfferV2 (daemon E2E)", () => {
  ...
  test("emits relay-only offer URL with stable serverId", async () => {
    ...
    try {
      ...
      expect(offerUrl.startsWith("https://app.openplane.sh/#offer=")).toBe(
        true
      );
  ...
  test("persists serverId and daemon keypair across daemon restarts", async () => {
    ...
    const tempHomeRoot = await mkdtemp(
      path.join(os.tmpdir(), "openplane-offer-home-")
    );

    const { logger: logger1, lines: lines1 } = createCapturingLogger();
    const daemon1 = await createTestOpenPlaneDaemon({
      listen: "0.0.0.0",
      logger: logger1,
      relayEnabled: true,
      openplaneHomeRoot: tempHomeRoot,
      cleanup: false,
    });
    ...
    try {
      ...
      const daemon2 = await createTestOpenPlaneDaemon({
        listen: "0.0.0.0",
        logger: logger2,
        relayEnabled: true,
        openplaneHomeRoot: tempHomeRoot,
  ...
  test("respects --no-relay (CLI) by not emitting a pairing offer", async () => {
    ...
    const tempHome = await mkdtemp(
      path.join(os.tmpdir(), "openplane-offer-e2e-")
    );
    ...
    const env = {
      ...process.env,
      OPENPLANE_HOME: tempHome,
      OPENPLANE_LISTEN: `0.0.0.0:${port}`,
      OPENAI_API_KEY: "",
      OPENPLANE_DICTATION_ENABLED: "0",
      OPENPLANE_VOICE_MODE_ENABLED: "0",
      OPENPLANE_LOG_FORMAT: "json",
    };
        ...
        const onData = (data: Buffer) => {
          ...
          for (const line of text.split("\n")) {
            ...
            if (line.includes("pairing_offer")) {
              ...
              reject(
                new Error("unexpected pairing_offer log when --no-relay is set")
              );
              return;
            }
            ...
            try {
              ...
              if (parsed.msg !== `Server listening on http://0.0.0.0:${port}`) {
                continue;
              }
              clearTimeout(timeout);
              resolve(true);
