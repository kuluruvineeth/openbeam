...
describe("ConnectionOfferV2 (daemon E2E)", () => {
  ...
  test("emits relay-only offer URL with stable serverId", async () => {
    ...
    try {
      ...
      expect(offerUrl.startsWith("https://app.openbeam.sh/#offer=")).toBe(true);
      ...
      expect(offer.relay.endpoint).toBe("relay.openbeam.sh:443");
  ...
  test("persists serverId and daemon keypair across daemon restarts", async () => {
    ...
    const tempHomeRoot = await mkdtemp(
      path.join(os.tmpdir(), "openbeam-offer-home-")
    );

    const { logger: logger1, lines: lines1 } = createCapturingLogger();
    const daemon1 = await createTestOpenBeamDaemon({
      listen: "0.0.0.0",
      logger: logger1,
      relayEnabled: true,
      openbeamHomeRoot: tempHomeRoot,
      cleanup: false,
    });
    ...
    try {
      ...
      const daemon2 = await createTestOpenBeamDaemon({
        listen: "0.0.0.0",
        logger: logger2,
        relayEnabled: true,
        openbeamHomeRoot: tempHomeRoot,
  ...
  test("respects --no-relay (CLI) by not emitting a pairing offer", async () => {
    ...
    const tempHome = await mkdtemp(
      path.join(os.tmpdir(), "openbeam-offer-e2e-")
    );
    ...
    const env = {
      ...process.env,
      OPENBEAM_HOME: tempHome,
      OPENBEAM_LISTEN: `0.0.0.0:${port}`,
      OPENAI_API_KEY: "",
      OPENBEAM_DICTATION_ENABLED: "0",
      OPENBEAM_VOICE_MODE_ENABLED: "0",
      OPENBEAM_LOG_FORMAT: "json",
    };
