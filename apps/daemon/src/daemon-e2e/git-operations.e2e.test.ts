...
import { deriveWorktreeProjectHash } from "../utils/worktree";
...
describe("daemon E2E", () => {
  ...
  describe("worktree setup", () => {
    ...
    test("bootstraps configured worktree terminals after setup succeeds", async () => {
      await withShell("/bin/sh", async () => {
        ...
        expect(existsSync(path.join(agent.cwd, "lint-terminal.txt"))).toBe(
          false
        );
        ...
        expect(
          list.terminals.some((terminal) => terminal.name === "Dev Server")
        ).toBe(true);
        ...
        const setupPort = readFileSync(
          path.join(agent.cwd, "setup-port.txt"),
          "utf8"
        ).trim();
    ...
    test("reports failures via timeline tool_call without deleting the created worktree", async () => {
      ...
      expect(existsSync(path.join(agent.cwd, "should-not-run.txt"))).toBe(
        false
      );
  ...
  describe("archiveOpenBeamWorktree", () => {
    test("archives worktree by running destroy commands and shutting down worktree terminals", async () => {
            ...
            terminals: [
              {
                name: "Dev Server",
                command:
                  'echo "dev-server" > dev-terminal.txt; tail -f /dev/null',
              },
            ],
            destroy: [
      ...
      const beforeArchiveDirectories =
    ...
    test("archives the worktree when the last agent in it is archived", async () => {
      const repoRoot = tmpCwd();
