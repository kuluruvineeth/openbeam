...
import type { AgentSnapshotPayload, SessionOutboundMessage } from "../messages";
import {
...
function pickTwoDistinctModels(
  models: Array<{ id: string }>
): [string, string] {
  const ids = Array.from(new Set(models.map((m) => m.id))).filter(Boolean);
  if (ids.length < 2) {
    throw new Error(
      `Need at least 2 models to test switching; got ${ids.length}`
    );
  }
...
describe("daemon E2E", () => {
  ...
  beforeEach(async () => {
    ...
    await ctx.client.fetchAgents({
      subscribe: { subscriptionId: "live-preferences" },
    });
  });
      ...
      test.runIf(shouldRun)(
        ...
        async () => {
          ...
          try {
            ...
            const final = await ctx.client.waitForFinish(agent.id, 120_000);
    ...
    async () => {
      ...
      try {
        ...
        const defaultThinkingId =
          modelWithOptions.defaultThinkingOptionId ?? "default";
        const nonDefault =
          modelWithOptions.thinkingOptions?.find(
            (o) => o.id !== defaultThinkingId
          )?.id ??
  ...
  test.runIf(hasOpenCode)(
    ...
    async () => {
      ...
      try {
        ...
        const defaultThinkingId =
          modelWithThinkingOptions.defaultThinkingOptionId ?? "default";
        const thinkingId =
          modelWithThinkingOptions.thinkingOptions?.find(
            (o) => o.id !== defaultThinkingId
          )?.id ??
