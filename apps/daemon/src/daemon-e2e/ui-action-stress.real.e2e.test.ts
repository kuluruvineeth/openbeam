...
function buildNormalUsageScenario(
  ...
  return {
    ...
    actions: [
      ...
      {
        type: "assert_queue_size",
        label: "queue remains empty after idle send",
        size: 0,
      },
      {
        type: "ui_enter_submit",
...
function buildOverlapScenario(
  ...
  return {
    ...
    actions: [
      ...
      {
        type: "wait_for_finish",
        label: "wait after first queued-now",
        timeoutMs: 240_000,
      },
      {
        type: "assert_last_message",
      ...
      {
        type: "wait_for_finish",
        label: "wait after second queued-now",
        timeoutMs: 180_000,
      },
      {
        type: "assert_last_message",
      ...
      {
        type: "assert_queue_size",
        label: "queue remains empty at end",
        size: 0,
      },
    ],
  };
...
async function runUiScenario(params: {
  ...
  for (const action of scenario.actions) {
    ...
    if (action.type === "ui_send_queued_now") {
      ...
      if (!selected) {
        throw new Error(
          `[${scenario.name}] ${action.label}: missing queued prompt`
        );
      }
    ...
    if (action.type === "wait_for_finish") {
      ...
      while (
        ...
        const remaining = Math.max(1000, action.timeoutMs - elapsed);
...
function createRealAgentClient(
  provider: AgentProvider,
  logger: pino.Logger
): AgentClient {
  if (provider === "claude") {
