...
import type { PersistenceHandle } from "../shared/messages";
import {
...
describe("two-cycle Codex agent resume", () => {
  ...
  test("Codex agent remembers original marker after two resume cycles", async () => {
    ...
    try {
      ...
      const afterRemember = await ctx.client.waitForFinish(agent.id, 5000);
      expect(afterRemember.status).toBe("idle");
      expect(afterRemember.final?.persistence).toBeTruthy();

      const persistence0 = afterRemember.final
        ?.persistence as PersistenceHandle;
      ...
      const afterRecall1 = await ctx.client.waitForFinish(resumed1.id, 5000);
      expect(afterRecall1.status).toBe("idle");
      expect(afterRecall1.final?.persistence).toBeTruthy();
      expect(afterRecall1.final?.persistence?.metadata).toMatchObject({
        marker,
      });
      ...
      const afterRecall2 = await ctx.client.waitForFinish(resumed2.id, 5000);
      expect(afterRecall2.status).toBe("idle");
      expect(afterRecall2.final?.persistence).toBeTruthy();
      expect(afterRecall2.final?.persistence?.metadata).toMatchObject({
        marker,
      });
