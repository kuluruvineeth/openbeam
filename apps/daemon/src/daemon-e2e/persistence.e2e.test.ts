...
describe("daemon E2E - persistence", () => {
  ...
  test("persists and resumes Codex agent with conversation context", async () => {
    ...
    try {
      ...
      const afterMessage = await ctx.client.waitForFinish(agent.id, 5000);
      ...
      expect(
        (
          afterMessage.final?.persistence?.metadata as
      ...
      const afterResume = await ctx.client.waitForFinish(resumed.id, 5000);
  ...
  test("timeline survives daemon restart", async () => {
    ...
    try {
      ...
      const afterMessage = await ctx.client.waitForFinish(agentId, 5000);
      expect(afterMessage.status).toBe("idle");
      expect(afterMessage.final?.persistence).toBeTruthy();
      ...
      expect(
        timelineItems.some((item) => item.type === "assistant_message")
      ).toBe(true);
    } finally {
      await ctx.cleanup();
      cleaned = true;
