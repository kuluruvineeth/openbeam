...
class LoggingWebSocket extends OriginalWebSocket {
  ...
  constructor(url: string, ...args: any[]) {
    ...
    this.on("message", (data) => {
      ...
      console.log(
        `[WS] Message received (${data.toString().length} bytes): ${str}...`
      );
    });
  }
}
...
async function testMultiAgentSequence() {
  ...
  try {
    ...
    console.log(
      `Connection state: ${JSON.stringify(client.getConnectionState())}`
    );
    ...
    const agent1 =
      (arg1 ? agents.find((a) => a.id === arg1) : null) ?? agents[0] ?? null;
    const agent2 =
