...
describe("daemon E2E (real claude) - send while running recovery", () => {
  test.runIf(isCommandAvailable("claude"))(
    ...
    async () => {
      ...
      const primary = new DaemonClient({
        url: `ws://127.0.0.1:${daemon.port}/ws`,
      });
      const secondary = new DaemonClient({
        url: `ws://127.0.0.1:${daemon.port}/ws`,
      });
      ...
      try {
        ...
        await secondary.fetchAgents({
          subscribe: { subscriptionId: "secondary" },
        });
        ...
        await secondary.sendMessage(
          agent.id,
          "Reply with exactly: state saved"
        );
