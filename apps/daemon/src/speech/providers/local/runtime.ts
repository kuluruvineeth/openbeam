...
export async function initializeLocalSpeechServices(params: {
  ...
  if (
    ...
    if (localConfig) {
      const voiceEngine = await getLocalSttEngine(
        localModels.voiceLocalSttModel
      );
      if (voiceEngine?.kind === "offline") {
        sttService = new SherpaOnnxParakeetSTT(
  ...
  if (
    ...
    if (localConfig) {
      const dictationEngine = await getLocalSttEngine(
        localModels.dictationLocalSttModel
      );
      if (dictationEngine?.kind === "offline") {
        dictationSttService = {
          id: "local",
          createSession: () =>
            new SherpaParakeetRealtimeTranscriptionSession({
              engine: dictationEngine.engine,
            }),
        };
        ...
        dictationSttService = {
          id: "local",
          createSession: () =>
            new SherpaRealtimeTranscriptionSession({
              engine: dictationEngine.engine,
            }),
        };
      }
  ...
  if (
    ...
    providers.voiceTts.provider === "local"
  ) {
    if (localConfig) {
      try {
  ...
  const cleanup = () => {
    const maybeFreeable = localVoiceTtsProvider as unknown as {
      free?: () => void;
    } | null;
