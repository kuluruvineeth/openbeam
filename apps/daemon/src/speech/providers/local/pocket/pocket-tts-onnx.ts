...
async function loadSentencePiece(
  ...
  if (!Processor) {
    throw new Error(
      "Failed to load SentencePiece processor from @sctg/sentencepiece-js"
    );
  }
...
class PocketTtsOnnxEngine {
  ...
  static async create(
    ...
    const referenceAudioFile =
      config.referenceAudioFile ?? `${modelDir}/reference_sample.wav`;

    const flowMainFile =
      precision === "int8" ? "flow_lm_main_int8.onnx" : "flow_lm_main.onnx";
    const flowFlowFile =
      precision === "int8" ? "flow_lm_flow_int8.onnx" : "flow_lm_flow.onnx";
    const decoderFile =
      precision === "int8" ? "mimi_decoder_int8.onnx" : "mimi_decoder.onnx";

    assertFileExists(`${onnxDir}/mimi_encoder.onnx`, "PocketTTS mimi_encoder");
    assertFileExists(
      `${onnxDir}/text_conditioner.onnx`,
      "PocketTTS text_conditioner"
    );
    ...
    const [
      ...
      ort.InferenceSession.create(`${onnxDir}/${decoderFile}`, {
        executionProviders: providers,
      }),
    ]);
    ...
    const audioTensor = new ort.Tensor("float32", floatAudio, [
      1,
      1,
      floatAudio.length,
    ]);
    ...
    // biome-ignore lint/suspicious/noExplicitAny: daemon type interop
    const firstOutName = (mimiEncoder as any).outputNames?.[0] as
      | string
      | undefined;
  ...
  private async runTextConditioner(tokenIds: OrtTensor): Promise<OrtTensor> {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: daemon type interop
    const firstOutName = (this.textConditioner as any).outputNames?.[0] as
      | string
      | undefined;
    const t = firstOutName
      ? // biome-ignore lint/suspicious/noExplicitAny: daemon type interop
        (out as any)[firstOutName]
      : // biome-ignore lint/suspicious/noExplicitAny: daemon type interop
        (Object.values(out)[0] as any);
    if (!t) {
      throw new Error("PocketTTS text_conditioner: missing output");
    }
    return t;
  }

  private async *runFlowLm(
    textEmbeddings: OrtTensor
  ): AsyncGenerator<Float32Array> {
    const ort = this.ort;
    const state = initState(this.flowLmMain, ort);

    const emptySeq = new ort.Tensor("float32", new Float32Array(0), [1, 0, 32]);
    const emptyText = new ort.Tensor(
      "float32",
      new Float32Array(0),
      [1, 0, 1024]
    );
    ...
    for (let step = 0; step < this.maxFrames; step += 1) {
      ...
      // biome-ignore lint/suspicious/noExplicitAny: daemon type interop
      const outputNames = (this.flowLmMain as any).outputNames as
        | string[]
        | undefined;
      // biome-ignore lint/style/noNonNullAssertion: value guaranteed to be set
      ...
      if (!(conditioning && eos)) {
        throw new Error(
          "PocketTTS flow_lm_main: missing conditioning/EOS outputs"
        );
      }
      // biome-ignore lint/suspicious/noExplicitAny: daemon type interop
      ...
      for (const st of this.stBuffers) {
        ...
        // biome-ignore lint/suspicious/noExplicitAny: daemon type interop
        const first = (this.flowLmFlow as any).outputNames?.[0] as
          | string
          | undefined;
        const flowTensor = first
          ? // biome-ignore lint/suspicious/noExplicitAny: daemon type interop
            (flowOut as any)[first]
          : // biome-ignore lint/suspicious/noExplicitAny: daemon type interop
            (Object.values(flowOut)[0] as any);
        if (!flowTensor) {
          throw new Error("PocketTTS flow_lm_flow: missing output");
        }
  ...
  private async decodeLatentsChunk(
    ...
    // biome-ignore lint/suspicious/noExplicitAny: daemon type interop
    const firstOutName = (this.mimiDecoder as any).outputNames?.[0] as
      | string
      | undefined;
...
export class PocketTtsOnnxTTS implements TextToSpeechProvider {
  ...
  async synthesizeSpeech(text: string): Promise<SpeechStreamResult> {
    ...
    const pcmChunkBytes = Math.max(
      2,
      Math.round((sampleRate * this.chunkMs) / 1000) * 2
    );
