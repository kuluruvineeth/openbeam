  ...
  downloadTest(
    ...
    async () => {
      ...
      try {
        ...
        const dictationFinal = await ctx.client.finishDictationStream(
          dictationId,
          finalSeq
        );
        unsubscribe();
        ...
        // biome-ignore lint/suspicious/noMisplacedAssertion: assertion in test helper
        expect(wordSimilarity(dictationFinal.text, baseline)).toBeGreaterThan(
          0.45
        );
        // biome-ignore lint/suspicious/noMisplacedAssertion: assertion in test helper
        ...
        const transcriptionPromise = waitForSignal<string>(
          30_000,
          (resolve, reject) => {
            const offResult = ctx.client.on(
        ...
        const voiceCwd = mkdtempSync(
          path.join(tmpdir(), "speech-download-voice-agent-")
        );
        ...
        if (set === "parakeet-pocket") {
          const modelDir = getSherpaOnnxModelDir(
            modelsDir,
            "pocket-tts-onnx-int8"
          );
          ...
          const { stream, format: ttsFormat } =
            await tts.synthesizeSpeech(ttsText);
          ...
          const sttModelDir = getSherpaOnnxModelDir(
            modelsDir,
            "parakeet-tdt-0.6b-v3-int8"
          );
          ...
          const rt = await stt.transcribeAudio(
            combined,
            toAudioPcmFormat(ttsFormat)
          );
          engine.free();
          // biome-ignore lint/suspicious/noMisplacedAssertion: assertion in test helper
