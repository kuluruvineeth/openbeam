...
export const SHERPA_ONNX_MODEL_CATALOG = {
  ...
  "parakeet-tdt-0.6b-v3-int8": {
    ...
    description:
      "NVIDIA Parakeet TDT v3 (offline NeMo transducer, multilingual).",
  ...
  "pocket-tts-onnx-int8": {
    ...
    description:
      "Pocket TTS ONNX (INT8) with streaming decode support (via onnxruntime).",
...
export type LocalSttModelId =
  | ModelIdByKind<"stt-online">
  | ModelIdByKind<"stt-offline">;
export type LocalTtsModelId = ModelIdByKind<"tts">;

const ALL_MODEL_IDS = Object.keys(
  SHERPA_ONNX_MODEL_CATALOG
) as SherpaOnnxModelId[];
...
function buildAliasMap<T extends string>(
  ...
  for (const modelId of modelIds) {
    const aliases =
      SHERPA_ONNX_MODEL_CATALOG[modelId as SherpaOnnxModelId].aliases ?? [];
...
function createAliasedModelIdSchema<T extends string>(params: {
  ...
  return z
    ...
    .refine(
      (value): value is T =>
        validIds.has(value as T) || Object.hasOwn(params.aliases, value),
      {
...
export function getSherpaOnnxModelSpec(
  id: SherpaOnnxModelId
): SherpaOnnxModelSpec {
