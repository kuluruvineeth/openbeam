...
export class SherpaOnnxSTT implements SpeechToTextProvider {
  ...
  public async transcribeAudio(
    ...
    if (format.toLowerCase().includes("audio/wav")) {
      ...
      inputRate =
        parsePcmRateFromFormat(format, this.engine.sampleRate) ??
    ...
    if (inputRate !== this.engine.sampleRate) {
      const resampler = new Pcm16MonoResampler({
        inputRate,
        outputRate: this.engine.sampleRate,
      });
    ...
    const peakFloat = peakForModel / 32_768.0;
    ...
    try {
      ...
      const text = String(
        this.engine.recognizer.getResult(stream)?.text ?? ""
      ).trim();
      const duration = Date.now() - start;
      this.logger.debug(
        { duration, textLength: text.length },
        "Sherpa transcription complete"
      );
      return {
        text,
        duration,
