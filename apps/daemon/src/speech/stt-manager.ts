...
import { parsePcm16MonoWav, parsePcmRateFromFormat } from "./audio";
  ...
  public async transcribe(
    ...
    try {
      ...
      const resolveIfComplete = () => {
        ...
        if (
          expectedFinals > 0 &&
          finalTranscriptSegmentIds.size >= expectedFinals
        ) {
          settled = true;
          settle?.();
          return;
      ...
      const commitEveryBytes =
        commitEverySeconds > 0
          ? Math.max(
              1,
              Math.round(session.requiredSampleRate * 2 * commitEverySeconds)
            )
          : 0;
      ...
      const singleSegmentMeta =
        orderedFinalMeta.length === 1 ? orderedFinalMeta[0] : null;
