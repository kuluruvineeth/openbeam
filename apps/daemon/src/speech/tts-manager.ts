...
export class TTSManager {
  private readonly pendingPlaybacks: Map<string, PendingPlayback> = new Map();
  ...
  private async generateSegmentAndWaitForPlayback(
    ...
    this.logger.info(
      { hasTts: !!tts, textLength: text.length },
    ...
    if (abortSignal.aborted) {
      this.logger.info("Aborted before generating audio");
      return;
    }

    this.logger.info(
      { textPreview: text.substring(0, 80) },
      "Calling tts.synthesizeSpeech"
    );
    const { stream, format } = await tts.synthesizeSpeech(text);
    this.logger.info({ format }, "TTS synthesis complete, streaming audio");
  ...
  public confirmAudioPlayed(chunkId: string): void {
    ...
    if (!pending) {
      this.logger.warn(
        { chunkId },
        "Received confirmation for unknown audio ID"
      );
      return;
    }
