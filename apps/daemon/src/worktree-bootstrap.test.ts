...
describe("runAsyncWorktreeBootstrap", () => {
  ...
  it("streams running setup updates live and persists only a final setup timeline row", async () => {
    ...
    if (persistedSetupItems[0]?.type === "tool_call") {
      ...
      if (persistedSetupItems[0].detail.type === "worktree_setup") {
        ...
        expect(persistedSetupItems[0].detail.log).toMatch(
          // biome-ignore lint/performance/useTopLevelRegex: scoped regex acceptable here
          /<== \[1\/2\] Exit 0 in \d+\.\d{2}s/
        );
        expect(persistedSetupItems[0].detail.log).toMatch(
          // biome-ignore lint/performance/useTopLevelRegex: scoped regex acceptable here
          /<== \[2\/2\] Exit 0 in \d+\.\d{2}s/
        );
        ...
        expect(
          typeof persistedSetupItems[0].detail.commands[0]?.durationMs ===
            "number"
        ).toBe(true);
        expect(
  ...
  it("truncates each command output to 64kb in the middle", async () => {
    ...
    if (
      !persistedSetupItem ||
      persistedSetupItem.detail.type !== "worktree_setup"
    ) {
