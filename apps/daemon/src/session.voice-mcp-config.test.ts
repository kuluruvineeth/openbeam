...
describe("voice MCP stdio config", () => {
  test("builds stdio MCP config for voice agent", () => {
    const config = buildVoiceAgentMcpServerConfig({
      ...
      socketPath: "/tmp/openbeam-voice.sock",
      env: {
        OPENBEAM_HOME: "/tmp/openbeam-home",
      },
    });
...
describe("voice mode prompt instructions", () => {
  test("builds enabled voice instructions and preserves base prompt", () => {
    ...
    expect(prompt).toContain("<openbeam_voice_mode>");
    expect(prompt).toContain("OpenBeam voice mode is now on.");
  ...
  test("builds disabled voice instructions and supersedes previous voice block", () => {
    ...
    expect(prompt).toContain("Base system prompt");
    expect(prompt).toContain("OpenBeam voice mode is now off.");
    expect(prompt).toContain(
      "Ignore any earlier OpenBeam voice mode instructions in this thread."
    );
    expect(prompt.match(/<openbeam_voice_mode>/g)?.length ?? 0).toBe(1);
