...
expectTypeOf<
  "getGitDiff" extends keyof DaemonClient ? true : false
>().toEqualTypeOf<false>();
expectTypeOf<
...
describe("DaemonClient", () => {
  ...
  test("reconnects after relay close with replaced-by-new-connection reason", async () => {
    ...
    try {
      ...
      const client = new DaemonClient({
        ...
        transportFactory: () => {
          const next =
            transports[Math.min(transportIndex, transports.length - 1)];
          transportIndex += 1;
  ...
  test("getCheckoutDiff uses one-shot subscription protocol", async () => {
    ...
    expect(subscribeRequest.message.type).toBe(
    ...
    expect(unsubscribeRequest.message.type).toBe(
  ...
  test("handles terminal binary frames delivered as UTF-8 strings", async () => {
    ...
    const frameAsString = new TextDecoder("utf-8", { fatal: true }).decode(
      frame
    );
  ...
  test("calls native helper getAccessibilityStatus via correlated request", async () => {
    const logger = createMockLogger();
    const mock = createMockTransport();
    ...
    const promise = client.getNativeHelperAccessibilityStatus({
      requestId: "req-native-status",
    });
    ...
    expect(request.message.type).toBe("native_helper_call_request");
    expect(request.message.requestId).toBe("req-native-status");
    expect(request.message.method).toBe("getAccessibilityStatus");
  ...
  test("throws when native helper call response reports failure", async () => {
    const logger = createMockLogger();
    const mock = createMockTransport();
    ...
    const promise = client.pasteTextWithNativeHelper("hello", {
      requestId: "req-native-paste",
    });
    ...
    expect(request.message.type).toBe("native_helper_call_request");
    expect(request.message.requestId).toBe("req-native-paste");
    expect(request.message.method).toBe("pasteText");
    expect(request.message.params).toEqual({ transcript: "hello" });
    ...
    await expect(promise).rejects.toThrow("Native helper is unavailable");
  });
});
