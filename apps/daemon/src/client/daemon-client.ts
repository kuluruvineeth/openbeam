...
type AgentConfigOverrides = Partial<
  Omit<AgentSessionConfig, "provider" | "cwd">
>;
...
type NativeHelperCallPayload = Extract<
  SessionOutboundMessage,
...
export type FetchAgentTimelinePayload =
  FetchAgentTimelineResponseMessage["payload"];

export type FetchAgentTimelineDirection =
  FetchAgentTimelinePayload["direction"];
export type FetchAgentTimelineProjection =
  FetchAgentTimelinePayload["projection"];
...
const DEFAULT_NATIVE_HELPER_TIMEOUT_MS = 10_000;

function isWaiterTimeoutError(error: unknown): boolean {
  return (
    error instanceof Error &&
    error.message.startsWith("Timeout waiting for message")
  );
}
...
export class DaemonClient {
  ...
  private readonly rawMessageListeners: Set<
  ...
  > = new Map();
  private readonly eventListeners: Set<DaemonEventHandler> = new Set();
  // biome-ignore lint/suspicious/noExplicitAny: daemon type interop
  private readonly waiters: Set<Waiter<any>> = new Set();
  private readonly checkoutStatusInFlight: Map<
    string,
    Promise<CheckoutStatusPayload>
  > = new Map();
  private readonly connectionListeners: Set<(status: ConnectionState) => void> =
    new Set();
  private reconnectTimeout: ReturnType<typeof setTimeout> | null = null;
  private connectTimeout: ReturnType<typeof setTimeout> | null = null;
  private pendingGenericTransportErrorTimeout: ReturnType<
    typeof setTimeout
  > | null = null;
  ...
  >();
  private readonly terminalDirectorySubscriptions = new Set<string>();
  private readonly agentStreamReceiveCounts: Map<string, number> = new Map();
  private readonly logger: Logger;
  private pendingSendQueue: PendingSend[] = [];
  private readonly terminalStreams: TerminalStreamManager;
  ...
  constructor(private readonly config: DaemonClientConfig) {
    ...
    this.logConnectionPath = isRelayClientWebSocketUrl(this.config.url)
      ? "relay"
      : "direct";
    ...
    const configClientSessionKey = normalizeClientSessionKey(
      this.config.clientSessionKey
    );
    ...
    if (isRelayClientWebSocketUrl(this.config.url)) {
      const urlClientId = normalizeClientSessionKey(
        parsed?.searchParams.get("clientId")
      );
      if (
        urlClientId &&
        configClientSessionKey &&
        configClientSessionKey !== urlClientId
      ) {
        throw new Error(
          "Relay clientId and clientSessionKey must match when both are provided"
        );
      }
      const resolvedClientSessionKey = configClientSessionKey ?? urlClientId;
      if (!resolvedClientSessionKey) {
        throw new Error(
          "Relay client requires clientSessionKey or URL clientId"
        );
      }
    ...
    const urlClientSessionKey = normalizeClientSessionKey(
      parsed?.searchParams.get("clientSessionKey")
    );
    if (
      urlClientSessionKey &&
      configClientSessionKey &&
      configClientSessionKey !== urlClientSessionKey
    ) {
      throw new Error(
        "Direct URL clientSessionKey and config clientSessionKey must match when both are provided"
      );
    }
    const resolvedClientSessionKey =
      configClientSessionKey ?? urlClientSessionKey;
    if (!resolvedClientSessionKey) {
      throw new Error(
        "Direct client requires clientSessionKey or URL clientSessionKey"
      );
    }
    this.config.clientSessionKey = resolvedClientSessionKey;
    this.logClientSessionKeyHash = hashForLog(resolvedClientSessionKey);
  ...
  private sendSessionMessage(message: SessionInboundMessage): void {
    ...
    try {
      this.transport.send(
        JSON.stringify({ type: "session", message: payload })
      );
    } catch (error) {
      if (this.config.suppressSendErrors) {
        return;
  ...
  private sendSessionMessageOrThrow(
    ...
    if (status === "connecting") {
      return new Promise((resolve, reject) => {
        const timeoutHandle = setTimeout(() => {
          // Remove from queue
          const idx = this.pendingSendQueue.findIndex(
            (p) => p.resolve === resolve
          );
          if (idx !== -1) {
    ...
    return Promise.reject(
      new Error(`Transport not connected (status: ${status})`)
    );
  }
    ...
    for (const pending of queue) {
      ...
      try {
        if (this.transport && this.connectionState.status === "connected") {
          ...
          pending.reject(
            new Error("Connection lost before message could be sent")
          );
        }
      } catch (error) {
        pending.reject(
  ...
  private async sendRequest<T>(params: {
    ...
    const { promise, cancel } = this.waitForWithCancel<RpcWaitResult<T>>(
      (msg) => {
        if (
          msg.type === "rpc_error" &&
          msg.payload.requestId === params.requestId
        ) {
          return {
            kind: "error",
  ...
  >(params: {
    ...
    options?: { skipQueue?: boolean };
    selectPayload?: (
      payload: CorrelatedResponsePayload<TResponseType>
    ) => TResult | null;
  }): Promise<TResult> {
  ...
  >(params: {
    ...
    return this.sendCorrelatedRequest({
      ...
      ...(params.selectPayload
        ? {
            selectPayload: params.selectPayload as (
              payload: CorrelatedResponsePayload<TResponseType>
            ) => TResult | null,
          }
        : {}),
  ...
  private resubscribeCheckoutDiffSubscriptions(): void {
    ...
    for (const [subscriptionId, subscription] of this
      .checkoutDiffSubscriptions) {
      const message = SessionInboundMessageSchema.parse({
  ...
  async createAgent(
    ...
    const status = await this.sendRequest({
      ...
      select: (msg) => {
        ...
        const failed = AgentCreateFailedStatusPayloadSchema.safeParse(
          msg.payload
        );
  ...
  async refreshAgent(
    ...
    return this.sendRequest({
      ...
      select: (msg) => {
        ...
        const refreshed = AgentRefreshedStatusPayloadSchema.safeParse(
          msg.payload
        );
        if (
          refreshed.success &&
          refreshed.data.requestId === resolvedRequestId
        ) {
  ...
  async setVoiceMode(
    ...
    if (!response.accepted) {
      const codeSuffix =
        ...
        response.reasonCode.trim().length > 0
          ? ` (${response.reasonCode})`
          : "";
      throw new Error(
        (response.error ?? "Failed to set voice mode") + codeSuffix
      );
    }
    return response;
  }
  ...
  async startDictationStream(
    ...
    // biome-ignore lint/suspicious/noEmptyBlockStatements: intentional no-op
    const ackPromise = ack.promise.then(() => {});
  ...
  async finishDictationStream(
    ...
    const finishAccepted =
      this.waitForWithCancel<DictationFinishAcceptedPayload>(
        (msg) => {
    ...
    const waitForFinalResult = async (
      ...
      if (outcome.kind === "timeout") {
        throw new Error(
          `Timeout waiting for dictation finalization (${timeoutMs}ms)`
        );
      }
      if (outcome.kind === "error") {
        throw outcome.error;
    ...
    try {
      ...
      if (firstOutcome.kind === "accepted") {
        return await waitForFinalResult(
          firstOutcome.payload.timeoutMs +
            DEFAULT_DICTATION_FINISH_TIMEOUT_GRACE_MS
        );
      }
      ...
      // biome-ignore lint/suspicious/noEmptyBlockStatements: intentional no-op
      // biome-ignore lint/complexity/noVoid: fire-and-forget async call
      void finishAcceptedPromise.catch(() => {});
    }
  }
  ...
  async subscribeCheckoutDiff(
    ...
    const previousSubscription =
      this.checkoutDiffSubscriptions.get(subscriptionId) ?? null;
    ...
    try {
      ...
      if (previousSubscription) {
        this.checkoutDiffSubscriptions.set(
          subscriptionId,
  ...
  async callNativeHelper<M extends NativeHelperRpcMethod>(params: {
    method: M;
    args: NativeHelperMethodParams<M>;
    requestId?: string;
    timeoutMs?: number;
    ...
    const payload = await this.sendCorrelatedSessionRequest({
      ...
      responseType: "native_helper_call_response",
      timeout: params.timeoutMs ?? DEFAULT_NATIVE_HELPER_TIMEOUT_MS,
      selectPayload: (candidate): NativeHelperCallPayload | null => {
        if (candidate.method !== params.method) {
          return null;
        }
        return candidate;
      },
    ...
    if (!payload.ok) {
      throw new DaemonRpcError({
        requestId: payload.requestId,
        requestType: "native_helper_call_request",
        error: payload.error ?? "Native helper request failed",
        ...(payload.code ? { code: payload.code } : {}),
      });
    }

    const parseResult = NativeHelperMethodSchemas[
      params.method
    ].result.safeParse(payload.result);
    if (!parseResult.success) {
      throw new DaemonRpcError({
        requestId: payload.requestId,
        requestType: "native_helper_call_request",
        error: `Native helper returned invalid payload for ${params.method}`,
        code: "native_helper_response_invalid",
      });
    }
  ...
  // biome-ignore lint/suspicious/useAwait: async signature required by interface
  async getNativeHelperAccessibilityStatus(options?: {
    requestId?: string;
    timeoutMs?: number;
  }) {
    return this.callNativeHelper({
  ...
  // biome-ignore lint/suspicious/useAwait: async signature required by interface
  async requestNativeHelperAccessibilityPermission(options?: {
    requestId?: string;
    timeoutMs?: number;
  }) {
    return this.callNativeHelper({
  ...
  // biome-ignore lint/suspicious/useAwait: async signature required by interface
  async getNativeHelperAccessibilityContext(options?: {
    ...
    return this.callNativeHelper({
      ...
      args: {
        ...(typeof options?.editableOnly === "boolean"
  ...
  // biome-ignore lint/suspicious/useAwait: async signature required by interface
  async getNativeHelperAccessibilityTreeDetails(options?: {
    ...
    return this.callNativeHelper({
      ...
      args: {
        ...(typeof options?.includeChildren === "boolean"
  ...
  // biome-ignore lint/suspicious/useAwait: async signature required by interface
  async pasteTextWithNativeHelper(
    text: string,
    options?: {
      requestId?: string;
      timeoutMs?: number;
  ...
  // biome-ignore lint/suspicious/useAwait: async signature required by interface
  async muteSystemAudioWithNativeHelper(options?: {
    requestId?: string;
    timeoutMs?: number;
  }) {
    return this.callNativeHelper({
      method: "muteSystemAudio",
      args: {},
      requestId: options?.requestId,
      timeoutMs: options?.timeoutMs,
    });
  ...
  // biome-ignore lint/suspicious/useAwait: async signature required by interface
  async restoreSystemAudioWithNativeHelper(options?: {
    requestId?: string;
    timeoutMs?: number;
  }) {
    return this.callNativeHelper({
      method: "restoreSystemAudio",
      args: {},
      requestId: options?.requestId,
      timeoutMs: options?.timeoutMs,
    });
  ...
  // biome-ignore lint/suspicious/useAwait: async signature required by interface
  async setNativeHelperShortcuts(
    shortcuts: {
      pushToTalk: number[];
  ...
  // biome-ignore lint/suspicious/useAwait: async signature required by interface
  async recheckNativeHelperPressedKeys(
  ...
  async listCommands(
    agentId: string,
    options?: ListCommandsOptions | string
  ...
  async attachTerminalStream(
    ...
    const message = SessionInboundMessageSchema.parse({
      ...
      terminalId,
      requestId: resolvedRequestId,
      ...(options?.resumeOffset !== undefined
  ...
  async waitForTerminalStreamData(
    ...
    return new Promise<TerminalStreamChunk>((resolve, reject) => {
      const timeoutHandle = setTimeout(() => {
        unsubscribe();
        reject(
          new Error(`Timeout waiting for terminal stream data (${timeout}ms)`)
        );
      }, timeout);
  ...
  private handleTransportMessage(data: unknown): void {
    ...
    if (!parsed.success) {
      const msgType =
        (parsedJson as { message?: { type?: string } })?.message?.type ??
        "unknown";
      this.logger.warn(
  ...
  private handleSessionMessage(msg: SessionOutboundMessage): void {
    ...
    if (msg.type === "agent_stream") {
      const agentId =
        (msg.payload as { agentId?: string }).agentId ?? "unknown";
      const count = (this.agentStreamReceiveCounts.get(agentId) ?? 0) + 1;
      this.agentStreamReceiveCounts.set(agentId, count);
      const handlers = this.messageHandlers.get("agent_stream");
      if (count === 1 || count === 10 || count === 50 || count % 100 === 0) {
        this.logger.info(
          { agentId, count, handlerCount: handlers?.size ?? 0 },
          `[DaemonClient] agent_stream received (event #${count}, ${handlers?.size ?? 0} handlers)`
        );
      }
    }
    ...
    switch (msg.type) {
      ...
      case "agent_stream":
        return {
          ...
          ...(typeof msg.payload.seq === "number"
            ? { seq: msg.payload.seq }
            : {}),
          ...(typeof msg.payload.epoch === "string"
...
function resolveAgentConfig(
  ...
  if (!merged.modeId) {
    merged.modeId =
      getAgentProviderDefinition(merged.provider).defaultModeId ?? undefined;
  }
