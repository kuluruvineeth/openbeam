...
export class VoiceAssistantWebSocketServer {
  ...
  private readonly externalSessionsByKey: Map<string, SessionConnection> =
    new Map();
  ...
  constructor(
    ...
    this.nativeHelper?.bridge?.setEventHandler((event) => {
      this.broadcast(
        wrapSessionMessage({
  ...
  public async close(): Promise<void> {
    this.nativeHelper?.bridge?.setEventHandler(null);
  ...
  private async attachSocket(
    ...
    const relayExternalSessionKey =
      ...
      metadata.externalSessionKey.trim().length > 0
    ...
    const externalSessionKey =
      relayExternalSessionKey ?? directExternalSessionKey;
    ...
    const session = new Session({
      ...
      nativeHelper: this.nativeHelper ?? undefined,
    ...
    if (connection.externalSessionKey && connection.sockets.size === 0) {
      ...
      const timeout = setTimeout(() => {
        ...
        void this.cleanupConnection(
          connection,
          "Client disconnected (grace timeout)"
        );
  ...
  private async cleanupConnection(
    ...
    if (connection.externalSessionKey) {
      const existing = this.externalSessionsByKey.get(
...
function extractSocketRequestMetadata(request: unknown): SocketRequestMetadata {
  ...
  const host =
    typeof record.headers?.host === "string" ? record.headers.host : undefined;
  const origin =
    typeof record.headers?.origin === "string"
      ? record.headers.origin
      : undefined;
  const userAgent =
