...
export async function listBranchSuggestions(
  ...
  for (const localRef of localRefs) {
    ...
    const origins =
      merged.get(normalized) ?? new Set<BranchSuggestionRefOrigin>();
    origins.add("local");
...
async function listCheckoutFileChanges(
  ...
  const { stdout: nameStatusOut } = await execGit(
...
export class MergeConflictError extends Error {
  ...
  constructor(options: {
    ...
    super(
      `Merge conflict while merging ${options.currentBranch} into ${options.baseRef}`
    );
...
function isGitError(error: unknown): boolean {
  ...
  return (
    ...
    // biome-ignore lint/performance/useTopLevelRegex: scoped regex acceptable here
    /git repository/i.test(error.message)
  );
}
...
async function getMainRepoRoot(cwd: string): Promise<string> {
  ...
  const { stdout: worktreeOut } = await execAsync(
...
async function resolveBestBaseRefForMerge(
  cwd: string,
  ...
  if (!(hasLocal || hasOrigin)) {
    throw new Error(
      `Base branch not found locally or on origin: ${normalizedBaseRef}`
    );
  }
  ...
  try {
    ...
    if (
      !(Number.isNaN(localOnly) || Number.isNaN(originOnly)) &&
      originOnly > localOnly
    ) {
      return `origin/${normalizedBaseRef}`;
    }
  } catch {
...
export async function getCheckoutStatus(
  ...
  const [aheadBehind, aheadOfOrigin, behindOfOrigin] = await Promise.all([
    baseRef && currentBranch
      ? getAheadBehind(cwd, baseRef, currentBranch)
      : Promise.resolve(null),
    hasRemote && currentBranch
      ? getAheadOfOrigin(cwd, currentBranch)
      : Promise.resolve(null),
    hasRemote && currentBranch
      ? getBehindOfOrigin(cwd, currentBranch)
      : Promise.resolve(null),
  ]);
...
export async function getCheckoutDiff(
  ...
  if (compare.mode === "uncommitted") {
    ...
    const baseRef =
      configured.baseRef ?? compare.baseRef ?? (await resolveBaseRef(cwd));
    if (!baseRef) {
      return { diff: "" };
    }
    if (
      ...
      throw new Error(
        `Base ref mismatch: expected ${baseRef}, got ${compare.baseRef}`
      );
    }

    const normalizedBaseRef = normalizeLocalBranchRefName(baseRef);
    const bestBaseRef = await resolveBestBaseRefForMerge(
      cwd,
      normalizedBaseRef
    );
  ...
  const untrackedChanges = changes.filter(
    (change) => change.isUntracked === true
  );
  ...
  if (compare.includeStructured) {
    const parsedTrackedFiles =
      trackedDiffText.length > 0
        ...
        : [];
    const parsedTrackedByPath = new Map(
      parsedTrackedFiles.map((file) => [file.path, file])
    );
...
export async function commitChanges(
  ...
  await execFileAsync(
    "git",
    ["-c", "commit.gpgsign=false", "commit", "-m", options.message],
    {
      cwd,
    }
  );
...
export async function mergeToBase(
  ...
  const configured = await getConfiguredBaseRefForCwd(cwd, context);
  const baseRef =
    configured.baseRef ?? options.baseRef ?? (await resolveBaseRef(cwd));
  if (!baseRef) {
  ...
  if (
    ...
    throw new Error(
      `Base ref mismatch: expected ${baseRef}, got ${options.baseRef}`
    );
  }
  if (!currentBranch) {
  ...
  try {
    ...
    if (mode === "squash") {
      await execAsync(`git merge --squash ${currentBranch}`, {
        cwd: operationCwd,
      });
      const message =
        options.commitMessage ??
        `Squash merge ${currentBranch} into ${normalizedBaseRef}`;
      await execFileAsync(
        "git",
    ...
    try {
      const [unmergedOutput, lsFilesOutput, statusOutput] = await Promise.all([
        execAsync("git diff --name-only --diff-filter=U", {
          cwd: operationCwd,
        }),
      ...
      const conflictDetected =
        conflicts.length > 0 ||
        // biome-ignore lint/performance/useTopLevelRegex: scoped regex acceptable here
        /CONFLICT|Automatic merge failed/i.test(errorDetails);
      if (conflictDetected) {
        try {
    ...
    if (
      isSameCheckout &&
      originalBranch &&
      originalBranch !== normalizedBaseRef
    ) {
      try {
...
function buildPullRequestStatus(
  // biome-ignore lint/suspicious/noExplicitAny: daemon type interop
  current: any,
  fallbackHead: string
...
export async function createPullRequest(
  ...
  const base =
    configured.baseRef ?? options.base ?? (await resolveBaseRef(cwd));
  if (!head) {
