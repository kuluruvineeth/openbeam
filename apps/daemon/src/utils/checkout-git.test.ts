...
function initRepo(): { tempDir: string; repoDir: string } {
  const tempDir = realpathSync(
    mkdtempSync(join(tmpdir(), "checkout-git-test-"))
  );
...
describe("checkout git utilities", () => {
  ...
  it("reports ahead/behind relative to origin on the base branch", async () => {
    ...
    execSync("git -c commit.gpgsign=false commit -m 'remote update'", {
      cwd: cloneDir,
    });
    ...
    execSync("git -c commit.gpgsign=false commit -m 'local update'", {
      cwd: repoDir,
    });
  ...
  it("diffs base mode against merge-base (no base-only deletions)", async () => {
    ...
    execSync("git -c commit.gpgsign=false commit -m 'base only'", {
      cwd: repoDir,
    });
  ...
  it("does not throw on large diffs (marks file as too_large)", async () => {
    ...
    expect(
      diff.structured?.some(
        (f) => f.path === "file.txt" && f.status === "too_large"
      )
    ).toBe(true);
  });
  ...
  it("short-circuits tracked binary files", async () => {
    ...
    writeFileSync(
      trackedBinaryPath,
      Buffer.from([0x00, 0xff, 0x11, 0x81, 0x00])
    );
    ...
    const entry = diff.structured?.find(
      (file) => file.path === "tracked-blob.bin"
    );
  ...
  it("short-circuits untracked binary files", async () => {
    ...
    writeFileSync(
      binaryPath,
      Buffer.from([0x00, 0xff, 0x10, 0x80, 0x00, 0x7f, 0x00])
    );
  ...
  it("marks untracked oversized files as too_large", async () => {
    ...
    const entry = diff.structured?.find(
      (file) => file.path === "untracked-large.txt"
    );
    expect(entry).toBeTruthy();
    expect(entry?.status).toBe("too_large");
    expect(diff.diff).toContain(
      "# untracked-large.txt: diff too large omitted"
    );
  });
  ...
  it("merges from the most-ahead base ref (origin/main when it is ahead)", async () => {
    ...
    execSync("git -c commit.gpgsign=false commit -m 'remote only'", {
      cwd: otherClone,
    });
    ...
    execSync(`git merge-base --is-ancestor ${remoteOnlyCommit} feature`, {
      cwd: repoDir,
    });
  });
  ...
  it("merges from the most-ahead base ref (local main when it is ahead)", async () => {
    ...
    execSync("git -c commit.gpgsign=false commit -m 'local only'", {
      cwd: repoDir,
    });
    ...
    execSync(`git merge-base --is-ancestor ${localOnlyCommit} feature`, {
      cwd: repoDir,
    });
  });
  ...
  it("aborts merge-from-base on conflicts and leaves no merge in progress", async () => {
    ...
    const porcelain = execSync("git status --porcelain", { cwd: repoDir })
      .toString()
      .trim();
    expect(porcelain).toBe("");
    expect(() =>
      execSync("git rev-parse -q --verify MERGE_HEAD", { cwd: repoDir })
    ).toThrow();
  });
  ...
  it("pushes the current branch to origin", async () => {
    ...
    execSync("git -c commit.gpgsign=false commit -m 'push commit'", {
      cwd: repoDir,
    });
  ...
  it("lists merged local and remote branch suggestions without duplicates", async () => {
    ...
    execSync("git add remote-only.txt", { cwd: otherClone });
    execSync("git -c commit.gpgsign=false commit -m 'remote only branch'", {
      cwd: otherClone,
    });
  ...
  it("disables GitHub features when gh is unavailable", async () => {
    ...
    const gitPath = execSync("command -v git", { stdio: "pipe" })
      .toString()
      .trim();
  ...
  it("returns merged PR status when no open PR exists for the current branch", async () => {
    ...
    writeFileSync(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        // biome-ignore lint/suspicious/noTemplateCurlyInString: intentional string literal
        'if [[ "${1-}" == "--version" ]]; then',
  ...
  it("uses stored baseRefName for OpenPlane worktrees (no heuristics)", async () => {
    ...
    execSync("git -c commit.gpgsign=false commit -m 'develop change'", {
      cwd: repoDir,
    });
    execSync("git checkout main", { cwd: repoDir });
