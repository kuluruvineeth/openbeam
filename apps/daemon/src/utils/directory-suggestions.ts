...
export async function searchWorkspaceEntries(
  ...
  return searchWorkspaceAcrossTree({
    ...
    maxDepth: options.maxDepth ?? DEFAULT_MAX_DEPTH,
    maxEntriesScanned:
      options.maxEntriesScanned ?? DEFAULT_MAX_DIRECTORIES_SCANNED,
  });
}
...
function setDirectoryListCache(
  cacheKey: string,
  entry: DirectoryListCacheEntry
): void {
  directoryListCache.set(cacheKey, entry);
  pruneDirectoryListCache();
}

function setWorkspaceEntryListCache(
  cacheKey: string,
  entry: WorkspaceEntryListCacheEntry
): void {
...
function pruneDirectoryListCache(): void {
  ...
  while (directoryListCache.size > DIRECTORY_LIST_CACHE_MAX_ENTRIES) {
    const oldestKey = directoryListCache.keys().next().value as
      | string
      | undefined;
    if (!oldestKey) {
      return;
...
function pruneWorkspaceEntryListCache(): void {
  ...
  while (workspaceEntryListCache.size > DIRECTORY_LIST_CACHE_MAX_ENTRIES) {
    const oldestKey = workspaceEntryListCache.keys().next().value as
      | string
      | undefined;
    if (!oldestKey) {
      return;
