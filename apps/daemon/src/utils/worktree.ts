  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, () => {
      ...
      if (!address || typeof address === "string") {
        server.close(() =>
          reject(new Error("Failed to acquire available port"))
        );
        return;
      }
      server.close((error) => {
...
async function inferRepoRootPathFromWorktreePath(
  worktreePath: string
): Promise<string> {
  try {
  ...
  for (const [index, cmd] of setupCommands.entries()) {
    ...
    if (result.exitCode !== 0) {
      if (options.cleanupOnFailure) {
        try {
          await execAsync(
            `git worktree remove "${options.worktreePath}" --force`,
            {
              cwd: options.worktreePath,
            }
          );
...
async function resolveBranchNameForWorktreePath(
  worktreePath: string
): Promise<string> {
  try {
...
export async function resolveWorktreeRuntimeEnv(options: {
  ...
  const repoRootPath =
    ...
    (await inferRepoRootPathFromWorktreePath(options.worktreePath));
  const branchName =
    options.branchName ??
...
export function validateBranchSlug(slug: string): {
  ...
  if (slug.includes("--")) {
    return {
      valid: false,
      error: "Branch name cannot have consecutive hyphens",
    };
  }
...
export async function deriveWorktreeProjectHash(cwd: string): Promise<string> {
  try {
    ...
    const normalizedCommonDir = normalizePathForOwnership(commonDir);
    const repoRoot =
      basename(normalizedCommonDir) === ".git"
...
export async function isOpenPlaneOwnedWorktreeCwd(
  ...
  const allowed = worktrees.some((entry) => {
    ...
    return (
      resolvedCwd === worktreePath || resolvedCwd.startsWith(worktreePath + sep)
    );
  });
  return {
    allowed,
...
export async function createWorktree({
  ...
  const normalizedBaseBranch = baseBranch
    ? normalizeBaseRefName(baseBranch)
    : "";
  ...
  try {
    ...
    try {
      await execAsync(`git rev-parse --verify origin/${normalizedBaseBranch}`, {
        cwd,
      });
