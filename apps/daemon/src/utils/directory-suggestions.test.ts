...
describe("searchHomeDirectories", () => {
  ...
  beforeEach(() => {
    tempRoot = realpathSync(
      mkdtempSync(path.join(tmpdir(), "directory-suggestions-"))
    );
    ...
    writeFileSync(
      path.join(homeDir, "projects", "README.md"),
      "not a directory\n"
    );

    mkdirSync(path.join(outsideDir, "outside-match"), { recursive: true });
    symlinkSync(
      path.join(outsideDir, "outside-match"),
      path.join(homeDir, "outside-link")
    );
  });
...
describe("searchWorkspaceEntries", () => {
  ...
  beforeEach(() => {
    tempRoot = realpathSync(
      mkdtempSync(path.join(tmpdir(), "workspace-suggestions-"))
    );
    ...
    symlinkSync(
      path.join(outsideDir, "escaped"),
      path.join(workspaceDir, "escaped-link")
    );
  });
  ...
  it("returns relative file and directory suggestions for workspace queries", async () => {
    ...
    expect(
      results.some((entry) => entry.path === path.join(workspaceDir, "src"))
    ).toBe(false);
  });
  ...
  it("supports path-style queries and does not escape cwd through symlinks", async () => {
    ...
    expect(
      escapedResults.some((entry) => entry.path.includes("escaped-link"))
    ).toBe(false);
  });

  it("ignores node_modules entries so deep workspace files still resolve under scan limits", async () => {
    mkdirSync(path.join(workspaceDir, "packages", "app", "src", "app"), {
      recursive: true,
    });
    writeFileSync(
      path.join(workspaceDir, "packages", "app", "src", "app", "_layout.tsx"),
      ""
    );

    for (let index = 0; index < 120; index += 1) {
      mkdirSync(path.join(workspaceDir, "node_modules", `pkg-${index}`), {
        recursive: true,
      });
      writeFileSync(
    ...
    expect(
      results.some((entry) => entry.path.startsWith("node_modules/"))
    ).toBe(false);
  });
});
