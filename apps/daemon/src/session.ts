...
import {
  ...
  type NativeHelperCallRequestMessage,
  NativeHelperCallRequestSchema,
...
const PROVIDER_MODELS_LIST_TIMEOUT_MS = 12_000;
...
>;
type FetchAgentsRequestFilter = NonNullable<
  FetchAgentsRequestMessage["filter"]
>;
type FetchAgentsRequestSort = NonNullable<
  FetchAgentsRequestMessage["sort"]
>[number];
...
export type SessionOptions = {
  ...
  nativeHelper?: {
    bridge?: NativeHelperBridge | null;
  };
...
export class Session {
  ...
  private readonly agentStreamEmitCounts: Map<string, number> = new Map();
  private readonly terminalSubscriptions: Map<string, () => void> = new Map();
  private readonly terminalExitSubscriptions: Map<string, () => void> =
    new Map();
  ...
  private readonly checkoutDiffSubscriptions = new Map<
    string,
    { targetKey: string }
  >();
  private readonly checkoutDiffTargets = new Map<
    string,
  ...
  private readonly ensureVoiceMcpSocketForAgent?: (
    agentId: string
  ) => Promise<string>;
  private readonly removeVoiceMcpSocketForAgent?: (
    agentId: string
  ) => Promise<void>;
  private readonly getSpeechReadiness?: () => SpeechReadinessSnapshot;
  private readonly nativeHelperBridge: NativeHelperBridge | null;
  ...
  constructor(options: SessionOptions) {
    ...
    if (this.terminalManager) {
      this.unsubscribeTerminalsChanged =
        this.terminalManager.subscribeTerminalsChanged((event) =>
    ...
    this.unregisterVoiceCallerContext =
      voiceBridge?.unregisterVoiceCallerContext;
    this.ensureVoiceMcpSocketForAgent =
      voiceBridge?.ensureVoiceMcpSocketForAgent;
    this.removeVoiceMcpSocketForAgent =
      voiceBridge?.removeVoiceMcpSocketForAgent;
    this.getSpeechReadiness = dictation?.getSpeechReadiness;
    this.nativeHelperBridge = nativeHelper?.bridge ?? null;
      ...
      this.sessionLogger.debug(
        {
          ...
          pendingRun: Boolean(snapshot.pendingRun),
        },
      ...
      try {
        for await (const event of iterator) {
          if (
            ...
            if (text.length > 0) {
              // biome-ignore lint/complexity/noVoid: fire-and-forget async call
              void this.pipeVoiceAgentTextToTts(agentId, text);
            }
          }
        }
      } catch (error) {
    ...
    this.unsubscribeAgentEvents = this.agentManager.subscribe(
      (event) => {
          ...
          this.sessionLogger.info(
            {
              ...
              permissionName: (event.event.request as any)?.name ?? "unknown",
              isVoiceMode: this.isVoiceMode,
              voiceModeAgentId: this.voiceModeAgentId ?? null,
              isVoiceAllowed: isVoicePermissionAllowed(event.event.request),
            },
            "Permission requested by agent in voice mode"
          );
        }
        ...
        if (
          ...
          this.sessionLogger.info(
            { agentId: event.agentId, requestId },
            "Auto-allowing voice tool permission"
          );
          // biome-ignore lint/complexity/noVoid: fire-and-forget async call
        ...
        if (activity?.deviceType === "mobile") {
          ...
          if (!activity.appVisible) {
            const hiddenForMs =
              Date.now() - activity.appVisibilityChangedAt.getTime();
        ...
        const emitCount =
          (this.agentStreamEmitCounts.get(event.agentId) ?? 0) + 1;
        this.agentStreamEmitCounts.set(event.agentId, emitCount);
        if (
          emitCount === 1 ||
          emitCount === 10 ||
          emitCount === 50 ||
          ...
          this.sessionLogger.info(
            {
              agentId: event.agentId,
              eventType: serializedEvent.type,
              emitCount,
              clientDeviceType: this.clientActivity?.deviceType ?? "unknown",
            },
            `Agent stream emitted (event #${emitCount})`
          );
        }
  ...
  private buildFallbackProjectCheckout(
    cwd: string
  ...
  private async buildProjectPlacement(
    cwd: string
  ...
  public async handleMessage(msg: SessionInboundMessage): Promise<void> {
    try {
      // biome-ignore lint/style/useDefaultSwitchClause: all cases handled
      switch (msg.type) {
        ...
        case "wait_for_finish_request":
          await this.handleWaitForFinish(
            msg.agentId,
            msg.requestId,
            msg.timeoutMs
          );
        ...
        case "dictation_stream_start":
          {
            const unavailable =
              this.resolveVoiceFeatureUnavailableContext("dictation");
            if (unavailable) {
              this.emit({
          ...
          await this.dictationStreamManager.handleStart(
            msg.dictationId,
            msg.format
          );
          break;
        ...
        case "dictation_stream_finish":
          await this.dictationStreamManager.handleFinish(
            msg.dictationId,
            msg.finalSeq
          );
          break;
        ...
        case "set_agent_mode_request":
          await this.handleSetAgentModeRequest(
            msg.agentId,
            msg.modeId,
            msg.requestId
          );
        ...
        case "set_agent_model_request":
          await this.handleSetAgentModelRequest(
            msg.agentId,
            msg.modelId,
            msg.requestId
          );
        ...
        case "native_helper_call_request":
          await this.handleNativeHelperCallRequest(
            NativeHelperCallRequestSchema.parse(msg)
          );
          break;
  ...
  private toNativeHelperErrorCode(error: unknown): string {
    if (error instanceof NativeHelperTimeoutError) {
      return "native_helper_timeout";
    }
    if (error instanceof NativeHelperRpcError) {
      return "native_helper_rpc_error";
    }
    if (error instanceof NativeHelperResponseValidationError) {
      return "native_helper_response_invalid";
    }
    if (error instanceof NativeHelperUnavailableError) {
      return "native_helper_unavailable";
    }
    return "native_helper_error";
  }

  private toNativeHelperErrorMessage(error: unknown): string {
  ...
  // biome-ignore lint/suspicious/useAwait: async signature required by interface
  private async callNativeHelperFromRequest(
    msg: NativeHelperCallRequestMessage
  ): Promise<unknown> {
    const bridge = this.nativeHelperBridge;
    if (!bridge) {
      throw new NativeHelperUnavailableError(
        "Native helper bridge is not configured"
      );
    }
    ...
    switch (msg.method) {
      case "getAccessibilityStatus":
        return bridge.call("getAccessibilityStatus", msg.params);
      case "requestAccessibilityPermission":
        return bridge.call("requestAccessibilityPermission", msg.params);
      case "getAccessibilityContext":
        return bridge.call("getAccessibilityContext", msg.params);
      case "getAccessibilityTreeDetails":
        return bridge.call("getAccessibilityTreeDetails", msg.params);
      case "pasteText":
        return bridge.call("pasteText", msg.params);
      case "muteSystemAudio":
        return bridge.call("muteSystemAudio", msg.params);
      case "restoreSystemAudio":
        return bridge.call("restoreSystemAudio", msg.params);
      case "setShortcuts":
        return bridge.call("setShortcuts", msg.params);
      case "recheckPressedKeys":
        return bridge.call("recheckPressedKeys", msg.params);
    }
  }

  private async handleNativeHelperCallRequest(
    msg: NativeHelperCallRequestMessage
  ): Promise<void> {
    const bridge = this.nativeHelperBridge;
    if (!bridge?.isRunning()) {
      this.emit({
        type: "native_helper_call_response",
        payload: {
          requestId: msg.requestId,
          method: msg.method,
          ok: false,
    ...
    try {
      const result = await this.callNativeHelperFromRequest(msg);
      this.emit({
        type: "native_helper_call_response",
        payload: {
          requestId: msg.requestId,
      ...
      const errorMessage = this.toNativeHelperErrorMessage(error);
      const code = this.toNativeHelperErrorCode(error);
      this.sessionLogger.warn(
        {
          err: error,
          method: msg.method,
        ...
        "native_helper_call_request failed"
      );
      this.emit({
        type: "native_helper_call_response",
        payload: {
  ...
  private async handleArchiveAgentRequest(
    ...
    if (!archivedRecord) {
      ...
      if (!archivedRecord) {
        throw new Error(
          `Agent not found in storage after snapshot: ${agentId}`
        );
      }
    }
  ...
  private async handleSetVoiceMode(
    ...
    try {
      if (enabled) {
        ...
        if (!this.isVoiceMode || this.voiceModeAgentId !== normalizedAgentId) {
          const refreshedAgentId =
            await this.enableVoiceModeForAgent(normalizedAgentId);
      ...
      const unavailable =
        this.getVoiceFeatureUnavailableResponseMetadata(error);
      this.sessionLogger.error(
        {
          err: error,
          enabled,
  ...
  private async enableVoiceModeForAgent(agentId: string): Promise<string> {
    ...
    const refreshOverrides: Partial<AgentSessionConfig> = {
      ...
      mcpServers: this.buildVoiceModeMcpServers(
    ...
    try {
      ...
      await this.removeVoiceMcpSocketForAgent?.(agentId).catch(() => {});
  ...
  private handleDictationManagerMessage(
    ...
    if (msg.type === "activity_log") {
      const metadata = msg.payload.metadata as
        | { dictationId?: unknown }
        | undefined;
      const dictationId =
        metadata && typeof metadata.dictationId === "string"
          ? metadata.dictationId
          : null;
    ...
    const dictationId =
      payloadWithDictationId &&
      typeof payloadWithDictationId.dictationId === "string"
  ...
  private cancelActiveVoiceDictationStream(reason: string): void {
    ...
    if (this.activeVoiceDictationReject) {
      this.activeVoiceDictationReject(
        new Error(`Voice dictation cancelled: ${reason}`)
      );
    }
  ...
  private async ensureActiveVoiceDictationStream(
    format: string
  ): Promise<void> {
    if (
      this.activeVoiceDictationId &&
    ...
    const startPromise = this.voiceStreamManager.handleStart(
      dictationId,
      format
    );
  ...
  private async appendToActiveVoiceDictationStream(
    ...
    if (this.activeVoiceDictationFinalizePromise) {
      // biome-ignore lint/suspicious/noEmptyBlockStatements: intentional no-op
      await this.activeVoiceDictationFinalizePromise.catch(() => {});
    }
  ...
  private async finalizeActiveVoiceDictationStream(
    reason: string
  ): Promise<void> {
    const dictationId = this.activeVoiceDictationId;
    if (!dictationId) {
  ...
  private async handleSendAgentMessage(
    ...
    try {
      ...
      this.sessionLogger.info({ agentId }, "Agent loaded for voice message");
    } catch (error) {
  ...
  private async handleCreateAgentRequest(
    ...
    this.sessionLogger.info(
      ...
      `Creating agent in ${config.cwd} (provider=${config.provider}, model=${config.model ?? "default"})${
        worktreeName ? ` with worktree ${worktreeName}` : ""
      }`
    );

    try {
      const { sessionConfig, worktreeConfig } =
        await this.buildAgentSessionConfig(config, git, worktreeName, labels);
      ...
      if (trimmedPrompt) {
        scheduleAgentMetadataGeneration({
          ...
          agentId: snapshot.id,
          cwd: snapshot.cwd,
          preferredProvider: snapshot.provider,
  ...
  private async handleListProviderModelsRequest(
    ...
    const modelsPromise = this.providerRegistry[msg.provider].fetchModels({
    ...
    // biome-ignore lint/complexity/noVoid: fire-and-forget async call
    void modelsPromise.catch((error) => {
      if (!didTimeout) {
        return;
      }
      this.sessionLogger.warn(
        { err: error, provider: msg.provider },
        `Provider model listing completed after timeout for ${msg.provider}`
      );
    });
    ...
    try {
      const timeoutPromise = new Promise<never>((_, reject) => {
        timeoutHandle = setTimeout(() => {
          ...
          reject(
            new Error(
              `Timed out listing models for ${msg.provider} after ${PROVIDER_MODELS_LIST_TIMEOUT_MS}ms`
            )
          );
        }, PROVIDER_MODELS_LIST_TIMEOUT_MS);
      });
  ...
  private async handleSpeechModelsDownloadRequest(
    ...
    const invalid = modelIdsRaw.filter(
      (id) => !allModelIds.has(id as LocalSpeechModelId)
    );
    if (invalid.length > 0) {
      this.emit({
  ...
  private async generateCommitMessage(cwd: string): Promise<string> {
    ...
    const schema = z.object({
      message: z
        ...
        .max(72)
        .describe(
          "Concise git commit message, imperative mood, no trailing period."
        ),
    });
    const fileList =
      diff.structured && diff.structured.length > 0
        ? [
            ...
            ...diff.structured.map((file) => {
              ...
              const status =
                file.status && file.status !== "ok" ? ` [${file.status}]` : "";
  ...
  private async handleListCommandsRequest(
    ...
    try {
      ...
      if (!agent && draftConfig) {
        ...
        const commands =
          await this.agentManager.listDraftCommands(sessionConfig);
        this.emit({
          type: "list_commands_response",
          payload: {
            agentId,
  ...
  private async handleDirectorySuggestionsRequest(
    ...
    const { query, limit, requestId, cwd, includeFiles, includeDirectories } =
      msg;
  ...
  private buildCheckoutDiffTargetKey(
    cwd: string,
    compare: CheckoutDiffCompareInput
  ): string {
  ...
  private checkoutDiffSnapshotFingerprint(
    snapshot: CheckoutDiffSnapshotPayload
  ): string {
  ...
  private async refreshCheckoutDiffTarget(
    target: CheckoutDiffWatchTarget
  ): Promise<void> {
  ...
  private async ensureCheckoutDiffWatchTarget(
    ...
    for (const watchPath of watchPaths) {
      const shouldTryRecursive =
        watchPath === repoWatchPath && allowRecursiveRepoWatch;
  ...
  private async maybeArchiveWorktreeAfterLastAgentArchived(options: {
    ...
    try {
      ...
      const hasUnknownLiveAgent = this.agentManager
        .listAgents()
        .some((agent) => {
      ...
      this.sessionLogger.warn(
        {
          err: error,
          agentId: options.archivedAgentId,
          cwd: options.archivedAgentCwd,
        },
  ...
  private async resolveAgentIdentifier(
    ...
    const prefixMatches = Array.from(knownIds).filter((id) =>
  ...
  private async getAgentPayloadById(
    agentId: string
  ...
  private compareEntryWithCursor(
    ...
    for (const spec of sort) {
      ...
      const rightValue =
        cursor.values[spec.key] !== undefined
          ? (cursor.values[spec.key] ?? null)
          : null;
      const base = this.compareSortValues(leftValue, rightValue);
      if (base === 0) {
        continue;
  ...
  private async listFetchAgentsEntries(
    ...
    const nextCursor =
      hasMore && pagedEntries.length > 0
        ? this.encodeFetchAgentsCursor(pagedEntries.at(-1), sort)
        : null;
  ...
  private async handleSendAgentMessageRequest(
    ...
    try {
      ...
      const startTimeout = setTimeout(
        () => startAbort.abort("timeout"),
        startTimeoutMs
      );
      try {
        await this.agentManager.waitForAgentRunStart(agentId, {
  ...
  private async handleWaitForFinish(
    ...
    try {
      ...
      if (!isAbort) {
        ...
        this.sessionLogger.error(
          { err: error, agentId },
          "wait_for_finish_request failed"
        );
        const final = await this.getAgentPayloadById(agentId);
        this.emit({
          type: "wait_for_finish_response",
  ...
  private async handleAudioChunk(
    ...
    if (this.isVoiceMode) {
      ...
      if (!msg.isLast) {
        ...
        this.sessionLogger.debug(
          "Voice mode: streaming chunk, waiting for speech end"
        );
        return;
      }
      ...
      this.clearVoiceModeInactivityTimeout();
      this.sessionLogger.debug(
        "Voice mode: speech ended, finalizing streaming transcription"
      );
  ...
  private async processCompletedAudio(
    ...
    if (this.pendingAudioSegments.length > 0) {
      ...
      const combinedFormat = pendingSegments.at(-1).format;
  ...
  private async handleTranscriptionResultPayload(
    ...
    if (!transcriptText) {
      this.sessionLogger.debug(
        "Empty transcription (false positive), not aborting"
      );
      this.setPhase("idle");
    ...
    this.sessionLogger.info(
      ...
      "Post-transcription routing check"
    );

    if (!this.isVoiceMode) {
      this.sessionLogger.info(
        { requestId: result.requestId },
        "Skipping voice agent processing: voice mode disabled"
      );
      return;
    }
    ...
    if (!agentId) {
      this.sessionLogger.info(
        { requestId: result.requestId },
        "Skipping voice agent processing: no voice-enabled agent"
      );
      return;
    }

    this.sessionLogger.info(
      { agentId, textPreview: result.text.substring(0, 80) },
      "Routing voice transcription to agent"
    );
  ...
  private async pipeVoiceAgentTextToTts(
    agentId: string,
    text: string
  ): Promise<void> {
    try {
      this.sessionLogger.info(
        { agentId, textLength: text.length, preview: text.slice(0, 160) },
        "Voice TTS: piping agent text to speech"
      );
      await this.ttsManager.generateAndWaitForPlayback(
        text,
        (msg) => this.emit(msg),
      ...
      this.sessionLogger.error(
        { err: error, agentId },
        "Voice TTS failed for agent text"
      );
    }
  }
  ...
  private async handleAbort(): Promise<void> {
    ...
    if (this.processingPhase === "transcribing") {
      ...
      this.sessionLogger.debug(
        "Will buffer next audio (currently transcribing)"
      );
  ...
  private async handleVoiceSpeechStart(): Promise<void> {
    ...
    this.sessionLogger.debug(
      "Voice speech detected – aborting playback and active agent run"
    );
  ...
  private setBufferTimeout(): void {
    ...
    this.bufferTimeout = setTimeout(async () => {
      this.sessionLogger.debug(
        "Buffer timeout reached, processing pending segments"
      );
  ...
  private setVoiceModeInactivityTimeout(): void {
    ...
    this.voiceModeInactivityTimeout = setTimeout(() => {
      ...
      // biome-ignore lint/complexity/noVoid: fire-and-forget async call
      void this.finalizeActiveVoiceDictationStream("inactivity timeout").catch(
        (error) => {
          this.sessionLogger.error(
            { err: error },
  ...
  private async emitInitialTerminalsChangedSnapshot(
    cwd: string
  ): Promise<void> {
    if (
      !(this.terminalManager && this.subscribedTerminalDirectories.has(cwd))
    ) {
      return;
    }
  ...
  private handleUnsubscribeTerminalRequest(
    msg: UnsubscribeTerminalRequest
  ): void {
  ...
  private killTrackedTerminal(
    ...
    const streamId = this.terminalStreamByTerminalId.get(terminalId);
    if (typeof streamId === "number") {
      this.detachTerminalStream(streamId, {
        emitExit: options?.emitExit ?? true,
      });
    }
  ...
  private async killTerminalsUnderPath(rootPath: string): Promise<void> {
    ...
    if (cleanupErrors.length > 0) {
      const details = cleanupErrors
        ...
        .join("; ");
      throw new Error(
        `Failed to clean up worktree terminals during archive (${details})`
      );
    }
  }
  ...
  private async handleAttachTerminalStreamRequest(
    ...
    const existingStreamId = this.terminalStreamByTerminalId.get(
      msg.terminalId
    );
  ...
  private getTerminalStreamChunkByteLength(
  ...
  private canEmitTerminalStreamChunk(
    ...
    return (
      chunk.startOffset < binding.lastAckOffset + TERMINAL_STREAM_WINDOW_BYTES
    );
  }
  ...
  private enqueueOrEmitTerminalStreamChunk(
    ...
    if (
      binding.pendingChunks.length > 0 ||
      !this.canEmitTerminalStreamChunk(binding, chunk)
    ) {
      if (
  ...
  private handleDetachTerminalStreamRequest(
    msg: DetachTerminalStreamRequest
  ): void {
    const success = this.detachTerminalStream(msg.streamId, {
      emitExit: false,
    });
    this.emit({
  ...
  private detachTerminalStream(
    ...
    try {
      ...
      this.sessionLogger.warn(
        { err: error, streamId },
        "Failed to unsubscribe terminal stream"
      );
    }
    this.terminalStreams.delete(streamId);
