...
function tryParseControlMessage(raw: unknown): ControlMessage | null {
  try {
    ...
    if (
      parsed.type === "client_disconnected" &&
      typeof parsed.clientId === "string" &&
      parsed.clientId.trim()
    ) {
...
export function startRelayTransport({
  ...
  const connectControl = (): void => {
    ...
    socket.on("open", () => {
          ...
          relayLogger.warn(
            {
              ...
              staleForMs,
              connectionId,
              staleTimeoutMs: CONTROL_STALE_TIMEOUT_MS,
            },
      ...
      relayLogger.debug(
        { url, connectionId },
        "relay_control_open_waiting_for_ready"
      );
    });
  ...
  const scheduleReconnect = (): void => {
    ...
    reconnectAttempt += 1;
    const delayMs = Math.min(30_000, 1000 * reconnectAttempt);
  ...
  const ensureClientDataSocket = (clientId: string): void => {
    ...
    const openTimeout = setTimeout(() => {
      ...
      relayLogger.warn(
        { url, clientId },
        "relay_data_open_timeout_terminating"
      );
      try {
        socket.terminate();
      } catch {
