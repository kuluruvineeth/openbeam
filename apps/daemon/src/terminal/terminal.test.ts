...
(hasNodePty ? describe : describe.skip)("Terminal", () => {
  ...
  describe("createTerminal", () => {
    it("ensures darwin prebuild spawn-helper is executable", () => {
      const packageRoot = mkdtempSync(
        join(tmpdir(), "terminal-node-pty-helper-")
      );
  ...
  describe("colors", () => {
    ...
    it("captures 256 color codes (mode 2)", async () => {
      ...
      await waitForLines(session, [
        "$ printf '\\033[38;5;208mORG\\033[0m'",
        "ORG$",
      ]);
    ...
    it("captures true color RGB (mode 3)", async () => {
      ...
      await waitForLines(session, [
        "$ printf '\\033[38;2;255;128;64mRGB\\033[0m'",
        "RGB$",
      ]);
  ...
  describe("raw output stream", () => {
    it("streams raw output chunks with byte offsets", async () => {
      ...
      expect(chunks.at(-1).endOffset).toBe(session.getOutputOffset());
    ...
    it("replays buffered output from offset", async () => {
      ...
      const resumeOffset = seen.at(-1)?.endOffset ?? 0;
