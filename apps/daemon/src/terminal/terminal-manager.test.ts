...
describe("TerminalManager", () => {
  ...
  describe("getTerminals", () => {
    ...
    it("throws for relative paths", async () => {
      manager = createTerminalManager();
      await expect(manager.getTerminals("tmp")).rejects.toThrow(
  ...
  describe("createTerminal", () => {
    ...
    it("inherits registered env for the worktree root cwd", async () => {
      await withShell("/bin/sh", async () => {
        ...
        for (
          let attempt = 0;
          attempt < 10 && !existsSync(markerPath);
          attempt++
        ) {
          session.send({
            type: "input",
        ...
        await waitForCondition(() => existsSync(markerPath), 10_000);
    ...
    it("inherits registered env for subdirectories within the worktree", async () => {
      await withShell("/bin/sh", async () => {
        manager = createTerminalManager();
        const rootCwd = mkdtempSync(
          join(tmpdir(), "terminal-manager-env-subdir-")
        );
  ...
  describe("killTerminal", () => {
    ...
    it("auto-removes terminal when shell exits", async () => {
      ...
      await waitForCondition(
        () => manager.getTerminal(exitedId) === undefined,
        10_000
      );
