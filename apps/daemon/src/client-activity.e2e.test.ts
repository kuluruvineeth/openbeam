...
describe("client activity tracking", () => {
  ...
  afterEach(async () => {
    if (client1) {
      await client1.close().catch(Function.prototype);
    }
    if (client2) {
      await client2.close().catch(Function.prototype);
    }
    await daemon.close();
  }, 30_000);
  ...
  function waitForAttentionRequired(
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        cleanup();
        reject(
          new Error(`Timeout waiting for attention_required (${timeout}ms)`)
        );
      }, timeout);
      ...
      const cleanup = client.on("agent_stream", (msg) => {
        ...
        if (msg.payload.event.type !== "attention_required") {
          return;
        }
