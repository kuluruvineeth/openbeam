...
function getHeadTailBudgets(maxBytes: number): {
  headBytes: number;
  tailBytes: number;
} {
...
function appendToMiddleTruncationAccumulator(
  ...
  const { tailBytes } = getHeadTailBudgets(
    MAX_WORKTREE_SETUP_COMMAND_OUTPUT_BYTES
  );
...
function buildTerminalTimelineItem(input: {
  ...
  return {
    ...
    error: {
      message: input.errorMessage ?? "Worktree terminal bootstrap failed",
    },
  };
}
