...
export type SceneState = {
  ...
  loadScene: () => void;
  clearScene: () => void;
  setScene: (nodes: Record<string, AnyNode>, rootNodeIds: AnyNodeId[]) => void;
    ...
    temporal(
      (set, get) => ({
        ...
        setScene: (nodes, rootNodeIds) => {
          ...
          for (const node of Object.values(patchedNodes)) {
            ...
            for (const nodeId of nodeIds) {
              ...
              const existing =
                ("collectionIds" in node
                  ? (node.collectionIds as CollectionId[])
                  : undefined) ?? [];
              nextNodes[nodeId] = {
                ...node,
                collectionIds: [...existing, id],
              } as AnyNode;
            }
        ...
        deleteCollection: (id) => {
          set((state) => {
            ...
            for (const nodeId of col?.nodeIds ?? []) {
              ...
              nextNodes[nodeId] = {
                ...node,
                collectionIds: (node.collectionIds as CollectionId[]).filter(
                  (cid) => cid !== id
                ),
              } as AnyNode;
            }
        ...
        updateCollection: (id, data) => {
          set((state) => {
            ...
            return {
              collections: { ...state.collections, [id]: { ...col, ...data } },
            };
          });
        },
        ...
        removeFromCollection: (id, nodeId) => {
          set((state) => {
            ...
            const nextCollections = {
              ...
              [id]: {
                ...col,
                nodeIds: col.nodeIds.filter((n) => n !== nodeId),
              },
            };
            const node = state.nodes[nodeId];
            if (!(node && "collectionIds" in node)) {
        ...
        return (state, error) => {
          ...
          if (rootNode && rootNode.type !== "site") {
            ...
            const existingRoots = (state.rootNodeIds || [])
              ...
              .filter(
                (node) => node?.type === "building" || node?.type === "item"
              );
