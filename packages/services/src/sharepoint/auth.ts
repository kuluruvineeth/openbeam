...
export class SharePointAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      sharePointApp.auth.type === AuthType.OAUTH2
        ? sharePointApp.auth.config.redirectPath
        : "/connectors/setup/sharepoint/oauth/callback";
  ...
  private async getCredentials(connectorId: string) {
    ...
    const config = connector.config as SharePointConfig;

    if (!(config.client_id && config.client_secret)) {
      throw new Error("SharePoint OAuth credentials not configured");
    }
  ...
  async start(ctx: AuthStartContext): Promise<string> {
    ...
    return generateSharePointAuthUrl({
      clientId,
  ...
  async complete(ctx: AuthCompleteContext): Promise<ConnectorResult> {
    ...
    try {
      ...
      const tokens = await exchangeSharePointCode({
        clientId,
        clientSecret,
        code: ctx.code,
        redirectUri: this.getRedirectUri(),
