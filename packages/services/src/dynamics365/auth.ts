import prisma, {
  AppType,
  activateConnector,
  createDefaultSyncJobs,
  getConnectorById,
  getConnectorWithCredentials,
  setConnectorError,
  upsertOAuthProvider,
} from "@openbeam/db";
import {
  AuthType,
  dynamics365App,
  exchangeDynamics365Code,
  generateDynamics365AuthUrl,
} from "@openbeam/integrations";
import type {
  AuthCompleteContext,
  AuthStartContext,
  ConnectorResult,
  IntegrationAuth,
} from "@openbeam/types/services";
import { createOAuthState, verifyOAuthState } from "../lib/oauth-state";

type Dynamics365ConnectorConfig = {
  client_id?: string;
  client_secret?: string;
  org_url?: string;
  tenant_id?: string;
  [key: string]: unknown;
};

export class Dynamics365Auth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      dynamics365App.auth.type === AuthType.OAUTH2
        ? dynamics365App.auth.config.redirectPath
        : "/connectors/setup/dynamics365/oauth/callback";
    return `${webUrl}${redirectPath}`;
  }

  private async getCredentials(connectorId: string) {
    const connector = await getConnectorWithCredentials(prisma, connectorId);
    if (!connector?.config) {
      throw new Error("Connector configuration not found");
    }

    const config = connector.config as Dynamics365ConnectorConfig;

    if (!(config.client_id && config.client_secret)) {
      throw new Error("Dynamics 365 OAuth credentials not configured");
    }

    if (!config.org_url) {
      throw new Error("Dynamics 365 organization URL not configured");
    }

    return {
      clientId: config.client_id,
      clientSecret: config.client_secret,
      orgUrl: config.org_url,
    };
  }

  async start(ctx: AuthStartContext): Promise<string> {
    if (!ctx.connectorId) {
      throw new Error("Connector ID required");
    }

    const { clientId, orgUrl } = await this.getCredentials(ctx.connectorId);

    return generateDynamics365AuthUrl({
      clientId,
      redirectUri: this.getRedirectUri(),
      state: createOAuthState({
        userId: ctx.user.id,
        workspaceId: ctx.workspaceId,
        connectorId: ctx.connectorId,
        redirectUrl: ctx.redirectUrl,
      }),
      orgUrl,
    });
  }

  async complete(ctx: AuthCompleteContext): Promise<ConnectorResult> {
    const { connectorId, redirectUrl } = verifyOAuthState(ctx.state);

    if (!connectorId) {
      throw new Error("Invalid state: missing connector ID");
    }

    try {
      const { clientId, clientSecret, orgUrl } =
        await this.getCredentials(connectorId);

      const tokens = await exchangeDynamics365Code({
        clientId,
        clientSecret,
        code: ctx.code,
        redirectUri: this.getRedirectUri(),
      });

      const connector = await prisma.$transaction(async (tx) => {
        const current = await getConnectorById(tx, connectorId);

        const updated = await activateConnector(tx, connectorId, {
          workspaceExternalId: tokens.userId,
          name: tokens.userEmail
            ? `Dynamics 365 (${tokens.userEmail})`
            : `Dynamics 365 (${tokens.userId})`,
          config: {
            ...(current.config as object),
            orgUrl,
            userEmail: tokens.userEmail,
          },
        });

        await upsertOAuthProvider(tx, {
          connectorId: updated.id,
          app: AppType.DYNAMICS_365,
          accessToken: tokens.accessToken,
          refreshToken: tokens.refreshToken,
          expiresIn: tokens.expiresIn,
          scopes: tokens.scopes,
          clientId,
          clientSecret,
        });

        await createDefaultSyncJobs(tx, updated.id);
        return updated;
      });

      return { connector, redirectUrl };
    } catch (error) {
      await setConnectorError(
        prisma,
        connectorId,
        error instanceof Error ? error.message : "OAuth failed"
      );
      throw error;
    }
  }
}
