...
export class GoogleCalendarAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      googleCalendarApp.auth.type === AuthType.OAUTH2
        ? googleCalendarApp.auth.config.redirectPath
        : "/connectors/setup/google-calendar/oauth/callback";
  ...
  private async getCredentials(connectorId: string) {
    ...
    const config = connector.config as GoogleCalendarConfig;
