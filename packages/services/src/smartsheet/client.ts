...
export function createSmartsheetClient(
  ...
  async function fetchJson<T>(
    ...
    const url = new URL(`${SMARTSHEET_API_BASE}${path}`);
    if (params) {
    ...
    if (!response.ok) {
      ...
      if (response.status >= 500 && attempt < DEFAULT_RETRY_ATTEMPTS) {
        ...
        logger.warn(
          ...
          "Smartsheet API server error, retrying"
        );
        await sleep(delay);
        return fetchJson<T>(path, params, attempt + 1);
      }
      throw new SmartsheetApiError({
        message: `Smartsheet API ${response.status}: ${body}`,
        code: "API_ERROR",
  ...
  async function postJson<T>(path: string, body: unknown): Promise<T> {
    ...
    const url = `${SMARTSHEET_API_BASE}${path}`;
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), timeout);
    ...
    if (!response.ok) {
      ...
      throw new SmartsheetApiError({
        message: `Smartsheet API POST ${response.status}: ${text}`,
  ...
  async function putJson<T>(path: string, body: unknown): Promise<T> {
    ...
    if (!response.ok) {
      const text = await response.text().catch(() => "");
      throw new SmartsheetApiError({
        message: `Smartsheet API PUT ${response.status}: ${text}`,
        code: response.status === 429 ? "RATE_LIMITED" : "API_ERROR",
        statusCode: response.status,
        retryable: response.status >= 500,
      });
  ...
  async function healthCheck(): Promise<boolean> {
    try {
      ...
      if (
        error instanceof SmartsheetApiError &&
        SmartsheetApiError.isAuthError(error.code)
      ) {
        return false;
      }
      try {
        await fetchJson("/sheets", { pageSize: "1" });
        return true;
      } catch {
        return false;
      }
