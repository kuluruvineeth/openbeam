...
export class MiroAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      miroApp.auth.type === AuthType.OAUTH2
        ? miroApp.auth.config.redirectPath
        : "/connectors/setup/miro/oauth/callback";
  ...
  private async getCredentials(connectorId: string) {
    ...
    const config = connector.config as MiroConfig;

    if (!(config.client_id && config.client_secret)) {
      throw new Error("Miro OAuth credentials not configured");
    }
  ...
  private getOAuthConfig() {
    if (miroApp.auth.type !== AuthType.OAUTH2) {
      throw new Error("Miro app is not configured for OAuth2");
    }
    return miroApp.auth.config;
  }
      ...
      const connector = await prisma.$transaction(async (tx) => {
        ...
        const updated = await activateConnector(tx, connectorId, {
          ...
          name: tokens.userEmail
            ? `Miro (${tokens.userEmail})`
            : `Miro (${tokens.userName})`,
          config: {
            ...(current.config as object),
            miroTeamId: tokens.teamId,
            miroUserId: tokens.userId,
