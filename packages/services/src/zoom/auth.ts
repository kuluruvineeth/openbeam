...
export class ZoomAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      zoomApp.auth.type === AuthType.OAUTH2
        ? zoomApp.auth.config.redirectPath
        : "/connectors/setup/zoom/oauth/callback";
  ...
  private async getCredentials(connectorId: string) {
    ...
    const config = connector.config as ZoomConfig;
  ...
  async complete(ctx: AuthCompleteContext): Promise<ConnectorResult> {
    ...
    try {
      ...
      const connector = await prisma.$transaction(async (tx) => {
        ...
        const displayName = tokens.displayName
          ? `Zoom (${tokens.displayName})`
          : `Zoom (${tokens.email})`;

        const updated = await activateConnector(tx, connectorId, {
          workspaceExternalId: tokens.accountId ?? tokens.userId,
          name: displayName,
          config: {
