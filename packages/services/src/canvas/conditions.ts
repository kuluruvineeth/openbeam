...
function evaluateCondition(condition: SingleCondition, data: unknown): boolean {
...
function evaluateGroup(group: ConditionGroup, data: unknown): boolean {
  return evaluateConditions(group.conditions, group.logic, data);
}

function evaluateBranch(branch: ConditionBranch, data: unknown): boolean {
...
export function selectConditionBranch(
  ...
  if (evaluationOrder === "parallel") {
    const results = branches.map((branch) => evaluateBranch(branch, data));
    const index = results.findIndex(Boolean);
    return index >= 0 ? (branches[index]?.id ?? null) : null;
  }
