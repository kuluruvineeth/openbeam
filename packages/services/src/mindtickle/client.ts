...
const BASE_URL = "https://api.mindtickle.com/v2";
...
export function createMindtickleClient(
  ...
  async function fetchJson<T>(
    ...
    if (!response.ok) {
      ...
      throw new MindtickleApiError({
        message: `Mindtickle API ${response.status}: ${body}`,
        code: "API_ERROR",
  ...
  async function postJson<T>(path: string, body: unknown): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new MindtickleApiError({
        message: `Mindtickle API POST ${response.status}: ${text}`,
  ...
  async function putJson<T>(path: string, body: unknown): Promise<T> {
    ...
    if (!response.ok) {
      const text = await response.text().catch(() => "");
      throw new MindtickleApiError({
        message: `Mindtickle API PUT ${response.status}: ${text}`,
        code: response.status === 429 ? "RATE_LIMITED" : "API_ERROR",
        statusCode: response.status,
        retryable: response.status >= 500,
      });
  ...
  async function healthCheck(): Promise<boolean> {
    try {
      await fetchJson<{ courses: unknown[] }>("/courses", {
        page: "1",
        limit: "1",
      });
      return true;
