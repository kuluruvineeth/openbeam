...
import type { FifteenFiveClientConfig } from "@openbeam/types/services/connectors/fifteen-five";
import { logger } from "../lib/logger";
import { FifteenFiveApiError } from "./types";
...
export interface FifteenFiveClient {
...
export function createFifteenFiveClient(
  config: FifteenFiveClientConfig
): FifteenFiveClient {
    ...
    if (!response.ok) {
      ...
      if (response.status >= 500 && attempt < DEFAULT_RETRY_ATTEMPTS) {
        ...
        logger.warn(
          ...
          "15Five API server error, retrying"
        );
        await sleep(delay);
        return fetchJson<T>(path, params, attempt + 1);
      }
      throw new FifteenFiveApiError({
        message: `15Five API ${response.status}: ${body}`,
        code: "API_ERROR",
  ...
  async function postJson<T>(path: string, body: unknown): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new FifteenFiveApiError({
        message: `15Five API POST ${response.status}: ${text}`,
  ...
  async function healthCheck(): Promise<boolean> {
    try {
      await fetchJson<{ results: unknown[] }>("/user/", {
        page_size: "1",
      });
      return true;
    } catch {
