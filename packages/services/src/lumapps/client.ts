...
export function createLumAppsClient(
  config: LumAppsClientConfig
): LumAppsClient {
  const {
    connectorId,
    apiToken,
    ...
    if (!response.ok) {
      ...
      if (response.status >= 500 && attempt < DEFAULT_RETRY_ATTEMPTS) {
        ...
        logger.warn(
          ...
          "LumApps API server error, retrying"
        );
        await sleep(delay);
        return fetchJson<T>(path, params, attempt + 1);
      }
      throw new LumAppsApiError({
        message: `LumApps API ${response.status}: ${body}`,
        code: "API_ERROR",
  ...
  async function postJson<T>(path: string, body: unknown): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new LumAppsApiError({
        message: `LumApps API POST ${response.status}: ${text}`,
  ...
  async function putJson<T>(path: string, body: unknown): Promise<T> {
    ...
    if (!response.ok) {
      const text = await response.text().catch(() => "");
      throw new LumAppsApiError({
        message: `LumApps API PUT ${response.status}: ${text}`,
        code: response.status === 429 ? "RATE_LIMITED" : "API_ERROR",
        statusCode: response.status,
        retryable: response.status >= 500,
      });
  ...
  async function healthCheck(): Promise<boolean> {
    try {
      ...
      if (
        error instanceof LumAppsApiError &&
        LumAppsApiError.isAuthError(error.code)
      ) {
        return false;
      }
      try {
        await fetchJson("/communities", { maxResults: "1" });
        return true;
      } catch {
        return false;
      }
