...
export class ProcoreAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      procoreApp.auth.type === AuthType.OAUTH2
        ? procoreApp.auth.config.redirectPath
        : "/connectors/setup/procore/oauth/callback";
  ...
  private async getCredentials(connectorId: string) {
    ...
    const config = connector.config as ProcoreConfig;
  ...
  private getOAuthConfig() {
    if (procoreApp.auth.type !== AuthType.OAUTH2) {
      throw new Error("Procore app is not configured for OAuth2");
    }
    return procoreApp.auth.config;
  }
  ...
  async complete(ctx: AuthCompleteContext): Promise<ConnectorResult> {
    ...
    try {
      ...
      const tokens = await exchangeProcoreCode({
        config: this.getOAuthConfig(),
        clientId,
        clientSecret,
        code: ctx.code,
      ...
      const connector = await prisma.$transaction(async (tx) => {
        ...
        const updated = await activateConnector(tx, connectorId, {
          ...
          name: tokens.userName
            ? `Procore (${tokens.userName})`
            : `Procore (${tokens.companyName})`,
          config: {
