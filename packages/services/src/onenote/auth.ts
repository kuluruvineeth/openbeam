...
export class OneNoteAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      onenoteApp.auth.type === AuthType.OAUTH2
        ? onenoteApp.auth.config.redirectPath
        : "/connectors/setup/onenote/oauth/callback";
  ...
  private async getCredentials(connectorId: string) {
    ...
    const config = connector.config as OneNoteConfig;

    if (!(config.client_id && config.client_secret)) {
      throw new Error("OneNote OAuth credentials not configured");
    }
  ...
  async complete(ctx: AuthCompleteContext): Promise<ConnectorResult> {
    ...
    try {
      ...
      const tokens = await exchangeOneNoteCode({
        clientId,
        clientSecret,
        code: ctx.code,
        redirectUri: this.getRedirectUri(),
