...
export class TeamsAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      teamsApp.auth.type === AuthType.OAUTH2
        ? teamsApp.auth.config.redirectPath
        : "/connectors/setup/teams/oauth/callback";
  ...
  async complete(ctx: AuthCompleteContext): Promise<ConnectorResult> {
    ...
    try {
      ...
      const tokens = await exchangeTeamsCode({
        clientId,
        clientSecret,
        code: ctx.code,
        redirectUri: this.getRedirectUri(),
