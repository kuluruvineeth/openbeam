...
export class SalesforceAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      salesforceApp.auth.type === AuthType.OAUTH2
        ? salesforceApp.auth.config.redirectPath
        : "/connectors/setup/salesforce/oauth/callback";
  ...
  private async getCredentials(connectorId: string) {
    ...
    const config = connector.config as SalesforceConfig;
