...
export class SeismicAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      seismicApp.auth.type === AuthType.OAUTH2
        ? seismicApp.auth.config.redirectPath
        : "/connectors/setup/seismic/oauth/callback";
  ...
  private async getCredentials(connectorId: string) {
    ...
    const config = connector.config as SeismicConfig;

    if (!(config.client_id && config.client_secret)) {
      throw new Error("Seismic OAuth credentials not configured");
    }
  ...
  private getOAuthConfig() {
    if (seismicApp.auth.type !== AuthType.OAUTH2) {
      throw new Error("Seismic app is not configured for OAuth2");
    }
    return seismicApp.auth.config;
  }
  ...
  async complete(ctx: AuthCompleteContext): Promise<ConnectorResult> {
    ...
    try {
      ...
      const tokens = await exchangeSeismicCode({
        config: this.getOAuthConfig(),
        clientId,
        clientSecret,
        code: ctx.code,
      ...
      const connector = await prisma.$transaction(async (tx) => {
        ...
        const updated = await activateConnector(tx, connectorId, {
          workspaceExternalId: tokens.tenantId ?? tokens.userId,
          name: tokens.userEmail
            ? `Seismic (${tokens.userEmail})`
            : `Seismic (${tokens.tenantName ?? tokens.userId})`,
          config: {
