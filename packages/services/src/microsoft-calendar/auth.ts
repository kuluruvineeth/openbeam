...
export class MicrosoftCalendarAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      microsoftCalendarApp.auth.type === AuthType.OAUTH2
        ? microsoftCalendarApp.auth.config.redirectPath
        : "/connectors/setup/microsoft-calendar/oauth/callback";
  ...
  private async getCredentials(connectorId: string) {
    ...
    const config = connector.config as MicrosoftCalendarConfig;
  ...
  async start(ctx: AuthStartContext): Promise<string> {
    ...
    return generateMicrosoftCalendarAuthUrl({
      clientId,
    ...
    try {
      ...
      const connector = await prisma.$transaction(async (tx) => {
        ...
        await upsertOAuthProvider(tx, {
          ...
          app: AppType.MICROSOFT_CALENDAR,
