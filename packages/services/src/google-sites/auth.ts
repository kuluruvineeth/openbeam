...
export class GoogleSitesAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      googleSitesApp.auth.type === AuthType.OAUTH2
        ? googleSitesApp.auth.config.redirectPath
        : "/connectors/setup/google-sites/oauth/callback";
  ...
  private async getCredentials(connectorId: string) {
    ...
    const config = connector.config as GoogleSitesConfig;
    ...
    if (!(config.client_id && config.client_secret)) {
      throw new Error("Google Sites OAuth credentials not configured");
    }
  ...
  async start(ctx: AuthStartContext): Promise<string> {
    ...
    return generateGoogleSitesAuthUrl({
      clientId,
  ...
  async complete(ctx: AuthCompleteContext): Promise<ConnectorResult> {
    ...
    try {
      ...
      const tokens = await exchangeGoogleSitesCode({
        clientId,
        clientSecret,
        code: ctx.code,
        redirectUri: this.getRedirectUri(),
      ...
      const connector = await prisma.$transaction(async (tx) => {
        ...
        const updated = await activateConnector(tx, connectorId, {
          ...
          name: `Google Sites (${tokens.userEmail})`,
          config: {
