...
import type {
  GoogleSitesClientConfig,
  GoogleSitesRateLimitState,
} from "@openbeam/types/services/connectors/google-sites";
import { logger } from "../lib/logger";
import { getValidAccessToken } from "../lib/token-refresh";
import { GoogleSitesApiError, GoogleSitesErrorCodes } from "./types";
...
export interface GoogleSitesClient {
  ...
  getRateLimitState(): Promise<GoogleSitesRateLimitState>;
...
export function createGoogleSitesClient(
  config: GoogleSitesClientConfig
): GoogleSitesClient {
  const {
    connectorId,
  ...
  async function checkRateLimit(method: string): Promise<void> {
    ...
    if (!allowed) {
      if (debug) {
        logger.debug({ method, reason }, "Google Sites rate limit hit");
      }
  ...
  async function executeRequest<T>(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw GoogleSitesApiError.fromResponse(response.status, errorBody);
      }
  ...
  function shouldRetry(
    ...
    if (error instanceof GoogleSitesApiError) {
      if (GoogleSitesApiError.isAuthError(error.code)) {
        return { retry: false };
      }
  ...
  async function handleRetryableError<T>(
    ...
    throw error instanceof GoogleSitesApiError
      ? error
      : new Error(
          `Google Sites GET ${ctx.path} failed for connector ${connectorId}`,
          { cause: error }
        );
  }
  ...
  async function getRateLimitState(): Promise<GoogleSitesRateLimitState> {
  ...
  async function healthCheck(): Promise<boolean> {
    try {
      ...
      logger.debug({ error, connectorId }, "Google Sites health check failed");
      return false;
    }
  }
