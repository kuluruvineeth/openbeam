...
export class LucidAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      lucidApp.auth.type === AuthType.OAUTH2
        ? lucidApp.auth.config.redirectPath
        : "/connectors/setup/lucid/oauth/callback";
  ...
  private async getCredentials(connectorId: string) {
    ...
    const config = connector.config as LucidConfig;

    if (!(config.client_id && config.client_secret)) {
      throw new Error("Lucid OAuth credentials not configured");
    }
  ...
  private getOAuthConfig() {
    if (lucidApp.auth.type !== AuthType.OAUTH2) {
      throw new Error("Lucid app is not configured for OAuth2");
    }
    return lucidApp.auth.config;
  }
  ...
  async complete(ctx: AuthCompleteContext): Promise<ConnectorResult> {
    ...
    try {
      ...
      const tokens = await exchangeLucidCode({
        config: this.getOAuthConfig(),
        clientId,
        clientSecret,
        code: ctx.code,
      ...
      const connector = await prisma.$transaction(async (tx) => {
        ...
        const updated = await activateConnector(tx, connectorId, {
          ...
          name: tokens.userEmail
            ? `Lucid (${tokens.userEmail})`
            : `Lucid (${tokens.accountId})`,
          config: {
