...
export interface SearchByAuthorOptions {
  limit?: number;
  documentTypes?: string[];
  fromDate?: number;
  toDate?: number;
...
export async function search(params: SearchParams): Promise<SearchResult> {
  ...
  const documents = mapVespaHitsToSummaries(result.items, params.snippetLength);
  ...
  if (params.includeAggregations) {
    aggregations = buildAggregations(documents, result.total);
  }
...
export async function autocomplete(
  ...
  const vespaTypes = types.length > 0 ? mapSuggestionTypes(types) : undefined;

  // Use vespaSearchService for unified suggestions
  const suggestions = await vespaSearchService.getSuggestions(prefix, teamId, {
    limit,
    types: vespaTypes,
  });
  ...
  const result: AutocompleteSuggestion[] = suggestions.map((s) => ({
    text: s.text,
    type: mapVespaSuggestionType(s.type),
...
export async function getRecentDocuments(
  ...
  const connectorType =
    connectorTypes && connectorTypes.length > 0 ? connectorTypes[0] : undefined;
  ...
  return mapVespaHitsToSummaries(filtered);
}
...
export async function searchThread(
  ...
  const filtered = filterByAccessControl(results, accessControlIds);
  const documents = mapVespaHitsToSummaries(filtered);
...
export async function findSimilar(
  ...
  // Map and filter
  const sourceDocument = mapDocumentToSummary(sourceDoc);
  const similarDocuments = filterByAccessControl(similarHits, accessControlIds)
    .slice(0, limit)
    .map((hit) => ({
      ...mapVespaHitToSummary(hit),
...
export async function searchByAuthor(
  ...
  let filtered = filterByAccessControl(result.items, accessControlIds);
  ...
  if (toDate) {
    filtered = filtered.filter((hit) => (hit.fields.created_at || 0) <= toDate);
  }

  const documents = mapVespaHitsToSummaries(filtered.slice(0, limit));
 ...
 */
export async function unifiedSearch(options: UnifiedSearchOptions) {
...
function mapVespaSuggestionType(
  type: string
...
function filterByAccessControl(
...
function mapVespaHitsToSummaries(
  hits: VespaHit<OpenPlaneDocument>[],
  snippetLength = 200
): DocumentSummary[] {
  return hits.map((hit) => mapVespaHitToSummary(hit, snippetLength));
}
...
function mapVespaHitToSummary(
  ...
  return mapDocumentToSummary(hit.fields, hit.relevance, snippetLength);
}

function mapDocumentToSummary(
  ...
  return {
    ...
    snippet: generateSnippet(fields.content, snippetLength),
