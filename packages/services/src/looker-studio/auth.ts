...
export class LookerStudioAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      lookerStudioApp.auth.type === AuthType.OAUTH2
        ? lookerStudioApp.auth.config.redirectPath
        : "/connectors/setup/looker-studio/oauth/callback";
  ...
  private async getCredentials(connectorId: string) {
    ...
    const config = connector.config as LookerStudioConfig;
    ...
    if (!(config.client_id && config.client_secret)) {
      throw new Error("Looker Studio OAuth credentials not configured");
    }
  ...
  async start(ctx: AuthStartContext): Promise<string> {
    ...
    return generateLookerStudioAuthUrl({
      clientId,
  ...
  async complete(ctx: AuthCompleteContext): Promise<ConnectorResult> {
    ...
    try {
      ...
      const tokens = await exchangeLookerStudioCode({
        clientId,
        clientSecret,
        code: ctx.code,
        redirectUri: this.getRedirectUri(),
      ...
      const connector = await prisma.$transaction(async (tx) => {
        ...
        const updated = await activateConnector(tx, connectorId, {
          ...
          name: `Looker Studio (${tokens.userEmail})`,
          config: {
