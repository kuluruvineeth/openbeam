...
export class ConfluenceAuth implements IntegrationAuth {
  private getRedirectUri(): string {
    const webUrl = process.env.WEB_URL || "http://localhost:3001";
    const redirectPath =
      confluenceApp.auth.type === AuthType.OAUTH2
        ? confluenceApp.auth.config.redirectPath
        : "/connectors/setup/confluence/oauth/callback";
  ...
  private async getCredentials(connectorId: string) {
    ...
    const config = connector.config as ConfluenceConfig;

    if (!(config.client_id && config.client_secret)) {
      throw new Error("Confluence OAuth credentials not configured");
    }
  ...
  async start(ctx: AuthStartContext): Promise<string> {
    ...
    return generateConfluenceAuthUrl({
      clientId,
