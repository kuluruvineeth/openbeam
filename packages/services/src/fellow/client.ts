...
import type { FellowClientConfig } from "@openbeam/types/services/connectors/fellow";
import { logger } from "../lib/logger";
import { FellowApiError } from "./types";
...
export function createFellowClient(config: FellowClientConfig): FellowClient {
    ...
    if (!response.ok) {
      ...
      if (response.status >= 500 && attempt < DEFAULT_RETRY_ATTEMPTS) {
        ...
        logger.warn(
          ...
          "Fellow API server error, retrying"
        );
        await sleep(delay);
        return fetchJson<T>(path, params, attempt + 1);
      }
      throw new FellowApiError({
        message: `Fellow API ${response.status}: ${body}`,
        code: "API_ERROR",
  ...
  async function postJson<T>(path: string, body: unknown): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new FellowApiError({
        message: `Fellow API POST ${response.status}: ${text}`,
  ...
  async function putJson<T>(path: string, body: unknown): Promise<T> {
    ...
    if (!response.ok) {
      const text = await response.text().catch(() => "");
      throw new FellowApiError({
        message: `Fellow API PUT ${response.status}: ${text}`,
        code: response.status === 429 ? "RATE_LIMITED" : "API_ERROR",
        statusCode: response.status,
        retryable: response.status >= 500,
      });
  ...
  async function healthCheck(): Promise<boolean> {
    try {
      ...
      if (
        error instanceof FellowApiError &&
        FellowApiError.isAuthError(error.code)
      ) {
        return false;
      }
      return false;
