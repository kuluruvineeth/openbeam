...
export const GOOGLE_DRIVE_SERVICE_ACCOUNT_SCOPES = [
...
export async function getGoogleDriveServiceAccountToken(
  ...
  if (!res.ok) {
    throw new Error(
      `Google Drive service account auth failed: ${await res.text()}`
    );
  }
