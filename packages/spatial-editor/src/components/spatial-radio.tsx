...
export function SpatialRadio() {
  const [shuffledPlaylist] = useState(() => shuffleArray(PLAYLIST));
  const [currentTrackIndex, setCurrentTrackIndex] = useState(0);
  const { masterVolume, radioVolume, muted, isRadioPlaying, setRadioPlaying } =
    useAudio();
    ...
    >
      <div className="flex items-center justify-between gap-2 px-3 py-2 font-medium text-sm">
        <div className="flex items-center gap-2">
          <Disc3
            className={cn("h-4 w-4 shrink-0", isRadioPlaying && "animate-spin")}
          />
