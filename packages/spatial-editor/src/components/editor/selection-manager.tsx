...
const isNodeInCurrentLevel = (node: AnyNode): boolean => {
  ...
  if (!currentLevelId) {
    return true; // No level selected, allow all
  }
...
const computeNextIds = (
  ...
  const isMeta =
    event?.metaKey || event?.nativeEvent?.metaKey || modifierKeys?.meta;
  const isCtrl =
    event?.ctrlKey || event?.nativeEvent?.ctrlKey || modifierKeys?.ctrl;
...
export const SelectionManager = () => {
  ...
  useEffect(() => {
    ...
    const onClick = (event: NodeEvent) => {
      ...
      if (
        (currentPhase === "structure" || currentPhase === "furnish") &&
        ...
        if (
          ...
          const item = node as ItemNode;
          if (
            item.asset.category === "door" ||
            item.asset.category === "window"
          ) {
            targetPhase = "structure";
          } else {
            targetPhase = "furnish";
        ...
        if (targetPhase !== currentPhase) {
          ...
          if (
            targetPhase === "structure" &&
            useEditor.getState().structureLayer === "zones"
          ) {
  ...
  useEffect(() => {
    ...
    const onEnter = (event: NodeEvent) => {
      ...
      if (
        (node.type === "building" || node.type === "site") &&
        (currentPhase === "structure" || currentPhase === "furnish")
      ) {
        return;
      }
