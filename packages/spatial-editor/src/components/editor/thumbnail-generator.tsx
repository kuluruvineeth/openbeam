...
import { emitter, sceneRegistry, useScene } from "@openbeam/spatial-core";
import { snapLevelsToTruePositions } from "@openbeam/spatial-viewer";
...
export const ThumbnailGenerator = ({
  ...
  const generate = useCallback(() => {
    ...
    if (!onThumbnailCaptureRef.current) {
      return;
    }
    ...
    try {
      ...
      restoreLevels();
      for (const [id, wasVisible] of visibilitySnapshot) {
        const obj = sceneRegistry.nodes.get(id);
        if (obj) {
  ...
  useEffect(() => {
    ...
    return () =>
      emitter.off(
        "camera-controls:generate-thumbnail",
