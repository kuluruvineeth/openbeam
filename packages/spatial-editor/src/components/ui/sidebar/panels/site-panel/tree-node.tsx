  ...
  if (e.shiftKey && selectedIds.length > 0) {
    ...
    if (lastSelectedId) {
      ...
      const nodeIds = nodes.map(
        (n) => n.getAttribute("data-treenode-id") as string
      );
...
export function TreeNode({ nodeId, depth = 0, isLast }: TreeNodeProps) {
  ...
  switch (node.type) {
    case "building":
      return (
        // biome-ignore lint/suspicious/noExplicitAny: type cast
        <BuildingTreeNode depth={depth} isLast={isLast} node={node as any} />
      );
    case "ceiling":
      return (
        // biome-ignore lint/suspicious/noExplicitAny: type cast
        <CeilingTreeNode depth={depth} isLast={isLast} node={node as any} />
      );
    case "level":
      // biome-ignore lint/suspicious/noExplicitAny: type cast
    ...
    case "window":
      return (
        // biome-ignore lint/suspicious/noExplicitAny: type cast
        <WindowTreeNode depth={depth} isLast={isLast} node={node as any} />
      );
    case "zone":
      // biome-ignore lint/suspicious/noExplicitAny: type cast
