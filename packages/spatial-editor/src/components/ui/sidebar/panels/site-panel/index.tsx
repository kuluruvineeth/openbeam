...
import { Popover, PopoverContent, PopoverTrigger } from "@openbeam/ui";
import {
  Camera,
  ChevronDown,
  Loader2,
            ...
            {points.map((point, index) => (
              ...
              <div className="flex items-center gap-1.5 text-xs" key={index}>
                ...
                <input
                  ...
                  value={point[0]}
                />
                <span className="shrink-0 text-muted-foreground">Z</span>
                <input
...
function LevelReferences({
  ...
  const references = Object.values(nodes).filter(
    (node): node is ScanNode | GuideNode =>
      (node.type === "scan" || node.type === "guide") &&
  ...
  const handleAddAsset = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!projectId) {
      ...
      useUploadStore
        .getState()
        .setError(levelId, "No active project. Please open a project first.");
      return;
    }
    ...
    <div className="relative flex flex-col">
      ...
      {rows.map((row, i) => {
        ...
        if (row.type === "upload") {
          return (
            <div
              className="group/ref relative border-border/50 border-b"
              key="upload"
            >
              <div
                className={cn(
      ...
      >
        ...
        <div
          className={cn(
            ...
            isSelected
              ? "bg-accent/50"
              : "bg-background group-hover/level:bg-accent/30"
          )}
        />
        ...
        <div className="relative z-20 flex h-8 items-center pr-1 pl-[28px]">
          ...
          >
            {isExpanded ? (
              ...
              <ChevronDown className="-rotate-90 h-3 w-3 text-muted-foreground" />
            )}
          </button>
        </div>
...
function ContentSection() {
    ...
    return (
      <div className="flex flex-col">
        {levelZones.map((zone, index) => (
          <ZoneItem
            isLast={index === levelZones.length - 1}
            key={zone.id}
            zone={zone}
          />
        ))}
  ...
  const elementChildren = level.children.filter((childId) => {
    ...
    if (!childNode || childNode.type === "zone") {
      return false;
    }
  ...
  return (
    <motion.div
      className={cn(
        ...
        isBuildingActive && "min-h-0 flex-1"
      )}
      layout
...
export function SitePanel({
  ...
  const [buildingCameraOpen, setBuildingCameraOpen] = useState<string | null>(
    null
  );
