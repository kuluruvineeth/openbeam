...
import { resolveCdnUrl } from "@openbeam/spatial-viewer";
...
export function ItemCatalog({ category }: { category: CatalogCategory }) {
  ...
  const [activePlacementTag, setActivePlacementTag] = useState<string | null>(
    null
  );
  const [activeFunctionalTag, setActiveFunctionalTag] = useState<string | null>(
    null
  );
  ...
  const placementCount = (tag: string | null) =>
    categoryItems.filter((item) => {
      ...
      if (activeFunctionalTag && !tags.includes(activeFunctionalTag)) {
        return false;
      }
      return true;
    }).length;
  ...
  const functionalCount = (tag: string) =>
    categoryItems.filter((item) => {
      ...
      if (activePlacementTag && !tags.includes(activePlacementTag)) {
        return false;
      }
      return true;
    }).length;
              ...
              {functionalTags.map((tag) => {
                ...
                return (
                  <button
                    ...
                    key={tag}
                    onClick={() =>
                      setActiveFunctionalTag(isActive ? null : tag)
                    }
                    type="button"
                  >
                    {tag}
