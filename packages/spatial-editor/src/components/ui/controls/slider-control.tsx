  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent) => {
      ...
      const handlePointerUp = (upEvent: PointerEvent) => {
        if ((upEvent.target as HTMLElement).closest("button")) {
          return;
        }
  ...
  const startPercent =
    dragStartValue !== null
      ? Math.max(
          ...
          Math.min(
            100,
            ((dragStartValue - currentMin) / (currentMax - currentMin)) * 100
          )
        )
      : null;

  return (
    // biome-ignore lint/a11y/noNoninteractiveElementInteractions: mouse tracking for UX
    // biome-ignore lint/a11y/noStaticElementInteractions: acceptable
    <div
      className={cn(
