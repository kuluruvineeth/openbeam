...
export function DoorPanel() {
  ...
  const setSegmentHeightRatio = (segIdx: number, newVal: number) => {
    ...
    const updated = node.segments.map((s, idx) => ({
      ...s,
      // biome-ignore lint/style/noNonNullAssertion: geometry access
      heightRatio: newRatios[idx]!,
    }));
    handleUpdate({ segments: updated });
  };

  const setSegmentColumnRatio = (
    segIdx: number,
    colIdx: number,
    newVal: number
  ) => {
  ...
  const handleSavePreset = useCallback(
    async (name: string) => {
      ...
      if (presetId) {
        emitter.emit("preset:generate-thumbnail", {
          presetId,
          nodeId: selectedId,
        });
      }
  ...
  if (!node || node.type !== "door" || selectedIds.length !== 1) {
    return null;
  }
    ...
    >
      ...
      <PanelSection title="Content Padding">
        <SliderControl
          ...
          min={0}
          onChange={(v) =>
            handleUpdate({ contentPadding: [v, node.contentPadding[1]] })
          }
          precision={3}
          step={0.005}
          unit="m"
        ...
        <SliderControl
          ...
          min={0}
          onChange={(v) =>
            handleUpdate({ contentPadding: [node.contentPadding[0], v] })
          }
          precision={3}
          step={0.005}
          unit="m"
            ...
            <div className="mb-2 flex flex-col gap-1" key={i}>
              ...
              <SliderControl
                ...
                onChange={(v) => {
                  ...
                  const updated = node.segments.map((s, idx) =>
                    idx === i
                      ? { ...s, columnRatios: new Array(n).fill(1 / n) }
                      : s
                  );
                  handleUpdate({ segments: updated });
                }}
