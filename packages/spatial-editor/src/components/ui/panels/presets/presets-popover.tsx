...
export function PresetsPopover({
  ...
  useEffect(() => {
    const handler = ({
      ...
      setPresets((prev) =>
        prev.map((p) =>
          p.id === presetId ? { ...p, thumbnail_url: thumbnailUrl } : p
        )
      );
    };
  ...
  const handleToggleCommunity = async (id: string, current: boolean) => {
    ...
    setPresets((prev) =>
      prev.map((p) => (p.id === id ? { ...p, is_community: !current } : p))
    );
  };
    ...
    <Popover onOpenChange={setOpen} open={open}>
      ...
      >
        ...
        <div className="no-scrollbar max-h-72 overflow-y-auto">
          ...
          ) : // biome-ignore lint/style/noNestedTernary: readable conditional rendering
          presets.length === 0 ? (
            ...
            <ul className="divide-y divide-border/30">
              {presets.map((preset) => (
                <PresetRow
                  ...
                  onToggleCommunity={() =>
                    handleToggleCommunity(preset.id, preset.is_community)
                  }
    ...
    <div className="flex flex-col items-center justify-center gap-2 px-4 py-8 text-center">
      ...
      <p className="text-muted-foreground text-xs">
        {tab === "community"
          ...
          : // biome-ignore lint/style/noNestedTernary: readable conditional text
            isAuthenticated
