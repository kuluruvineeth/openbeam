...
export const MoveWindowTool: React.FC<{ node: WindowNode }> = ({
  node: movingWindowNode,
}) => {
  ...
  useEffect(() => {
    ...
    const meta =
      typeof movingWindowNode.metadata === "object" &&
      movingWindowNode.metadata !== null
    ...
    const getLevelYOffset = () => {
      const id = getLevelId();
      return id
        ? (sceneRegistry.nodes.get(id as AnyNodeId)?.position.y ?? 0)
        : 0;
    };
    ...
    const onWallEnter = (event: WallEvent) => {
      ...
      if (prevWallId && prevWallId !== event.node.id) {
        markWallDirty(prevWallId);
      }
    ...
    return () => {
      const current = useScene.getState().nodes[
        movingWindowNode.id as AnyNodeId
      ] as WindowNode | undefined;
      const currentMeta = current?.metadata as
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: acceptable
