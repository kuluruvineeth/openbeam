...
const commitCeilingDrawing = (
  ...
  const ceilingCount = Object.values(nodes).filter(
    (n) => n.type === "ceiling"
  ).length;
...
export const CeilingTool: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const onGridMove = (event: GridEvent) => {
      if (!(cursorRef.current && gridCursorRef.current)) {
        return;
      }
      ...
      gridCursorRef.current.position.set(
        displayPoint[0],
        gridY,
        displayPoint[1]
      );
      ...
      if (verticalLineRef.current) {
        verticalLineRef.current.position.set(
          displayPoint[0],
          gridY,
          displayPoint[1]
        );
  ...
  useEffect(() => {
    ...
    const groundLinePoints: Vector3[] = points.map(
      ([x, z]) => new Vector3(x, gridY, z)
    );
    ...
    if (linePoints.length >= 2) {
      ...
      groundMainLineRef.current.geometry = new BufferGeometry().setFromPoints(
        groundLinePoints
      );
    ...
    if (points.length >= 2 && firstPoint) {
      ...
      groundClosingLineRef.current.geometry =
        new BufferGeometry().setFromPoints(groundClosingPoints);
