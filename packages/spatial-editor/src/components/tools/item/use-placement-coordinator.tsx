...
export function usePlacementCoordinator(
  config: PlacementCoordinatorConfig
): React.ReactNode {
  ...
  const { canPlaceOnFloor, canPlaceOnWall, canPlaceOnCeiling } =
    useSpatialQuery();
  ...
  useEffect(() => {
    ...
    const revalidate = (): boolean => {
      const placeable =
        shiftFreeRef.current || checkCanPlace(getContext(), validators);
      const color = placeable ? 0x22_c5_5e : 0xef_44_44;
    ...
    if (draftNode.current) {
      ...
      if (mesh) {
        ...
        cursorGroupRef.current.rotation.y = new Euler().setFromQuaternion(
          q,
          "YXZ"
        ).y;
      } else {
    ...
    const onGridClick = (event: GridEvent) => {
      const result = floorStrategy.click(
        getContext(),
        event,
        getActiveValidators()
      );
    ...
    const onWallEnter = (event: WallEvent) => {
      ...
      if (!draftNode.current) {
        ...
        if (result.stateUpdate.wallId) {
          useScene
            .getState()
            .dirtyNodes.add(result.stateUpdate.wallId as AnyNodeId);
        }
      }
    };
    ...
    const onWallMove = (event: WallEvent) => {
      ...
      if (ctx.state.surface !== "wall") {
        ...
        if (draftNode.current && enterResult.nodeUpdate.parentId) {
          useScene
            .getState()
            .updateNode(draftNode.current.id, enterResult.nodeUpdate);
          if (enterResult.stateUpdate.wallId) {
            useScene
              .getState()
              .dirtyNodes.add(enterResult.stateUpdate.wallId as AnyNodeId);
          }
        }
        return;
      }
      ...
      if (draft && result.nodeUpdate) {
        if ("side" in result.nodeUpdate) {
          draft.side = result.nodeUpdate.side;
        }
        if ("rotation" in result.nodeUpdate) {
          draft.rotation = result.nodeUpdate.rotation as [
            number,
            number,
            number,
          ];
    ...
    const onWallClick = (event: WallEvent) => {
      const result = wallStrategy.click(
        getContext(),
        event,
        getActiveValidators()
      );
    ...
    const onItemMove = (event: ItemEvent) => {
      ...
      if (draft) {
        ...
        const mesh = sceneRegistry.nodes.get(draft.id);
        if (mesh) {
          mesh.position.set(...result.gridPosition);
        }
      }
    ...
    const onItemLeave = (event: ItemEvent) => {
      ...
      if (placementState.current.surface !== "item-surface") {
        return;
      }
    ...
    const onCeilingEnter = (event: CeilingEvent) => {
      const nodes = useScene.getState().nodes;
      const result = ceilingStrategy.enter(
        getContext(),
        event,
        resolveLevelId,
        nodes
      ...
      if (!draftNode.current) {
        ...
        if (result.stateUpdate.ceilingId) {
          useScene
            .getState()
            .dirtyNodes.add(result.stateUpdate.ceilingId as AnyNodeId);
        }
      }
    };
    ...
    const onCeilingMove = (event: CeilingEvent) => {
      if (!draftNode.current && placementState.current.surface === "ceiling") {
        const nodes = useScene.getState().nodes;
        const setup = ceilingStrategy.enter(
          getContext(),
          event,
          resolveLevelId,
          nodes
    ...
    const onCeilingClick = (event: CeilingEvent) => {
      const result = ceilingStrategy.click(
        getContext(),
        event,
        getActiveValidators()
      );
  ...
  useEffect(() => {
    const draft = draftNode.current;
    if (!(draft && viewerLevelId) || asset.attachTo) {
      return;
    }
    if (draft.parentId === viewerLevelId) {
      return;
    }
    draft.parentId = viewerLevelId;
    useScene
      .getState()
      .updateNode(draft.id as AnyNodeId, { parentId: viewerLevelId });
  ...
  useFrame((_, delta) => {
    ...
    if (placementState.current.surface === "floor") {
      ...
      const dist = mesh.position.distanceToSquared(gridPosition.current);
      if (dist > 1) {
        mesh.position.copy(gridPosition.current);
      } else {
      ...
      if (!asset.attachTo) {
        ...
        cursorGroupRef.current.position.y =
          slabElevation + (levelGroup?.position.y ?? 0);
      }
    }
  });
  ...
  basePlaneGeometry.rotateX(-Math.PI / 2);
  basePlaneGeometry.translate(0, 0.01, 0);
