import { type AnyNodeId, emitter, useScene } from "@openbeam/spatial-core";
...
export const useKeyboard = () => {
  useEffect(() => {
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: acceptable
      ...
      if (e.key === "v" && !e.metaKey && !e.ctrlKey) {
        ...
        if (buildingId) {
          ...
          if (
            ...
            const currentIdx = levelId
              ? // biome-ignore lint/suspicious/noExplicitAny: type cast
                building.children.indexOf(levelId as any)
              : -1;
            const nextIdx =
              currentIdx < building.children.length - 1
                ? currentIdx + 1
                : currentIdx;
            if (nextIdx !== -1 && nextIdx !== currentIdx) {
              useViewer
                .getState()
                // biome-ignore lint/suspicious/noExplicitAny: type cast
                .setSelection({ levelId: building.children[nextIdx] as any });
            } else if (currentIdx === -1) {
              useViewer
                .getState()
                // biome-ignore lint/suspicious/noExplicitAny: type cast
                .setSelection({ levelId: building.children[0] as any });
            }
          }
        }
          ...
          if (
            ...
            if (prevIdx !== -1 && prevIdx !== currentIdx) {
              useViewer
                .getState()
                // biome-ignore lint/suspicious/noExplicitAny: type cast
                .setSelection({ levelId: building.children[prevIdx] as any });
            } else if (currentIdx === -1) {
              useViewer
                .getState()
                // biome-ignore lint/suspicious/noExplicitAny: type cast
                .setSelection({ levelId: building.children.at(-1) as any });
            }
          }
        }
        ...
        const selectedNodeIds = useViewer.getState().selection
