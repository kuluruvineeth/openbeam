...
export class RelayDurableObject {
  ...
  private nudgeOrResetControlForClient(clientId: string): void {
    ...
    setTimeout(() => {
      if (!this.hasClientSocket(clientId)) {
        return;
      }
      ...
      this.notifyControls({
        type: "sync",
        clientIds: this.listConnectedClientIds(),
      });
  ...
  private fetchV2(
    ...
    if (isServerControl) {
      ...
      roleSuffix = `(data:${clientId})`;
    } else if (role === "client") {
      roleSuffix = `(${clientId})`;
    }
    console.log(
      `[Relay DO] v2:${role}${roleSuffix} connected to session ${serverId}`
    );
  ...
  fetch(request: Request): Response {
    ...
    if (!version) {
      return new Response("Invalid v parameter (expected 1 or 2)", {
        status: 400,
      });
    }
  ...
  webSocketMessage(ws: WebSocket, message: string | ArrayBuffer): void {
    ...
    if (version === LEGACY_RELAY_VERSION) {
      ...
      for (const target of targets) {
        try {
          ...
          console.error(
            `[Relay DO] Failed to forward to ${targetRole}: ${error instanceof Error ? error.message : "unknown"}`
          );
        }
      }
      return;
    ...
    if (role === "client") {
      ...
      for (const target of servers) {
        try {
          ...
          console.error(
            `[Relay DO] Failed to forward client->server(${clientId}): ${error instanceof Error ? error.message : "unknown"}`
          );
        }
      }
      return;
    ...
    for (const target of targets) {
      try {
        ...
        console.error(
          `[Relay DO] Failed to forward server->client(${clientId}): ${error instanceof Error ? error.message : "unknown"}`
        );
      }
    }
  }
  ...
  webSocketClose(
    ...
    if (attachment.role === "client" && attachment.clientId) {
      ...
      for (const serverWs of this.state.getWebSockets(
        `server:${attachment.clientId}`
      )) {
        try {
  ...
  webSocketError(ws: WebSocket, error: unknown): void {
    ...
    console.error(
      `[Relay DO] WebSocket error for ${attachment?.role ?? "unknown"}: ${error instanceof Error ? error.message : "unknown"}`
    );
  }
}
