...
const previewOpacity = smoothstep(
  0.42,
  0.55,
  positionLocal.y.add(time.mul(-0.2)).mul(10).fract()
);
  ...
  return (
    <>
      <Clone
        ...
        scale={multiplyScales(
          node.asset.scale || [1, 1, 1],
          node.scale || [1, 1, 1]
        )}
        {...handlers}
      />
      ...
      {interactive &&
        lightEffects.map((effect) => (
          <ItemLight
            ...
            key={`${node.id}-${effect.kind}-${effect.color}`}
            nodeId={node.id}
          />
        ))}
    </>
...
const ItemAnimation = ({
  ...
  const targetClip = useInteractive((s) => {
    ...
    const isOn =
      toggleIndex !== undefined && toggleIndex >= 0
        ? Boolean(values?.[toggleIndex])
        : false;
    return isOn
  ...
  useFrame((_, delta) => {
    if (fadingOutRef.current) {
      ...
      action.timeScale = MathUtils.lerp(
        action.timeScale,
        0,
        Math.min(delta * 5, 1)
      );
...
const ItemLight = ({
  ...
  const sliderControl =
    sliderIndex >= 0
      ? (interactive.controls[sliderIndex] as SliderControl)
      : null;
