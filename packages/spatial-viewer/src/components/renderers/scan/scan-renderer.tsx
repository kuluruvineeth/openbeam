import { type ScanNode, useRegistry } from "@openbeam/spatial-core";
...
const ScanModel = ({ url, opacity }: { url: string; opacity: number }) => {
  const gltf = useGLTFKTX2(url) as unknown as { scene: import("three").Group };
  ...
  useMemo(() => {
    ...
    scene.traverse((child: unknown) => {
      if ((child as Mesh).isMesh) {
        ...
        mesh.raycast = Function.prototype as () => void;
