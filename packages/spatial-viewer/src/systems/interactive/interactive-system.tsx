  ...
  const interactiveNodeIds = useScene(
    useShallow((state) =>
      Object.values(state.nodes)
        .filter(
          (n): n is ItemNode => n.type === "item" && n.asset.interactive != null
        )
        .map((n) => n.id)
    )
  );
...
const ItemControlsOverlay = ({ nodeId }: { nodeId: AnyNodeId }) => {
  ...
  const controlValues = useInteractive(
    useShallow((state) => state.items[nodeId]?.controlValues)
  );
    ...
    >
      ...
      >
        {controls.map((control, i) => (
          <ControlWidget
            control={control}
            key={`${control.kind}-${control.label ?? i}`}
            onChange={(val) => setControlValue(nodeId, i, val)}
...
const ControlWidget = ({
  ...
  if (control.kind === "temperature") {
    return (
      <label style={labelStyle}>
        <span>
          {control.label}: {value}&deg;{control.unit}
        </span>
        <input
          max={control.max}
          min={control.min}
