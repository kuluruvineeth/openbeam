...
export class MediaIndexer {
  private readonly embeddingService: MediaEmbeddingService;

  constructor(options: MediaIndexerOptions = {}) {
    this.embeddingService = new MediaEmbeddingService(options);
  }

  async indexMedia(params: MediaIndexParams): Promise<MediaDocument> {
    const processed = await this.embeddingService.processMedia(
      params.mediaUrl,
      { indexId: params.indexId, inputType: params.inputType }
    );

    const mediaDoc = this.buildMediaDocument(params, processed);
    await this.feedMediaDocument(mediaDoc);

    return mediaDoc;
  }

  private buildMediaDocument(
    ...
    return {
      id: `media_${params.connectorId}_${params.externalId}`,
      team_id: params.teamId,
      connector_id: params.connectorId,
      external_id: params.externalId,
      title: params.title,
      description: params.description,
      media_summary: processed.metadata.summary,
      media_keywords: processed.metadata.keywords,
      ...
      media_type: processed.metadata.mediaType,
      language: processed.metadata.language,
    };
  }

  private async feedMediaDocument(doc: MediaDocument): Promise<void> {
    const vespaDoc = this.formatMediaForVespa(doc);
    const documentPath = `/document/v1/default/media_document/docid/${doc.id}`;
    ...
    if (!response.ok) {
      ...
      throw new Error(`Vespa media feed error: ${error}`);
    }
  }

  private formatMediaForVespa(doc: MediaDocument): VespaMediaDocument {
    const vespaDoc = this.buildVespaBaseFields(doc);
    this.addTensorFields(doc, vespaDoc);
    return vespaDoc;
  }

  private buildVespaBaseFields(doc: MediaDocument): VespaMediaDocument {
    return {
  ...
  private getIdentityFields(doc: MediaDocument) {
    return {
      id: doc.id,
      team_id: doc.team_id,
  ...
  private getContentFields(doc: MediaDocument) {
    return {
      title: doc.title,
  ...
  private getSourceFields(doc: MediaDocument) {
    return {
  ...
  private getMetaFields(doc: MediaDocument) {
    return {
  ...
  private getEngagementFields(doc: MediaDocument) {
    return {
  ...
  private getRelationFields(doc: MediaDocument) {
    return {
  ...
  async deleteMedia(mediaId: string): Promise<void> {
    const vespaUrl = process.env.VESPA_URL || "http://localhost:8080";
    const documentPath = `/document/v1/default/media_document/docid/${mediaId}`;
    ...
    if (!response.ok) {
      const error = await response.text();
      throw new Error(`Vespa media delete error: ${error}`);
    }
  }

  async updateMediaEngagement(
    mediaId: string,
    engagement: {
      viewCount?: number;
    ...
    if (!response.ok) {
      const error = await response.text();
      throw new Error(`Vespa media update error: ${error}`);
    }
  }
...
export const mediaIndexer = new MediaIndexer();
